...
function wbsCCRows(w,lv,page,isLast){
  ...
  var wbsCls=(window._wbsIdx%2===0)?'wbs-even':'wbs-odd';
  window._wbsIdx++;
  h+='<tr class="wbs-sep-row"><td colspan="'+(8+NM)+'"></td></tr>';
  ...
  /* Monthly cells: Bud / Act / Var stacked */
  for(var m=0;m<NM;m++){
    ...
    if(m<page){
      av=reel(w,m);
      avStyle='color:var(--tx2)';
    } else if(m===page){
      av=reel(w,m);
      avStyle='color:var(--pur);font-weight:700';
    } else {
    ...
    var varM=+(bv-av).toFixed(1);
    var varCol=varM>0?'var(--green)':varM<0?'var(--red)':'var(--tx3)';
    var _ccMonthLocked=isCCMonthLocked(m);
    h+='<td class="'+cls+'" style="padding:2px 4px;vertical-align:middle">';
    if(bv>0||av>0||varM!==0){
      h+='<div style="display:flex;flex-direction:column;align-items:flex-end;gap:0px;font-family:var(--mono);font-size:10px;line-height:1.5">';
      /* Budget row */
      h+='<div style="color:var(--blue)">'+(bv?bv.toFixed(1):'0')+'</div>';
      /* Actual/ETC row — editable for future */
      if(m>page&&!_ccMonthLocked){
        h+='<div style="'+avStyle+';cursor:pointer;text-decoration:underline dotted" data-wid="'+wid+'" data-m="'+m+'" onclick="ecCC(this.parentElement.parentElement.parentElement,\''+wid+'\','+m+',\'rad\','+av+')">'+av.toFixed(1)+avExtra+'</div>';
      } else {
        h+='<div style="'+avStyle+'">'+av.toFixed(1)+avExtra+'</div>';
      }
      /* Variance row */
      h+='<div style="font-weight:700;color:'+varCol+'">'+( varM>0?'+':'')+varM.toFixed(1)+'</div>';
      h+='</div>';
    }
    h+='</td>';
  }
  ...
  /* ── EUR row (compact) ── */
  var budEurMonth=getBudEur(w,page);
  var actEurMonth=getReelEur(w,page);
  var budEurTot=addBudgetEurSlots(w);
  var actEurTot=reelEurCum(w,page);
  /* Also sum contract/subcontract billings */
  var scEurMonth=0,scEurTot=0;
  (w.subcontracts||[]).forEach(function(sc){
    scEurMonth+=+(sc.billing[mkey(page)]||0);
    Object.keys(sc.billing||{}).forEach(function(k){scEurTot+=+(sc.billing[k]||0);});
  });
  (S.contracts||[]).filter(function(ct){return ct.wbsId===wid;}).forEach(function(ct){
    var fcast=+(ct.adjustedForecast||ct.monthlyForecast);
    scEurMonth+=page<=S.ccM?(+(ct.billing[mkey(page)]||fcast)):fcast;
    Object.keys(ct.billing||{}).forEach(function(k){scEurTot+=+(ct.billing[k]||0);});
  });
  var totalBudEurM=budEurMonth+((w.subcontracts||[]).length>0||((S.contracts||[]).filter(function(ct){return ct.wbsId===wid;}).length>0)?0:0);
  var totalActEurM=actEurMonth+scEurMonth;
  var varEurM=+(budEurMonth-totalActEurM).toFixed(0);
  /* Show EUR row if PU set, any billing exists, OR contracts/subcontracts attached */
  ...
  if(hasEurData){
    ...
    h+='<td class="p1" style="padding-left:'+(ind+12)+'px;border-right:2px solid var(--border2)">';
    h+='<span class="tp" style="background:#eff6ff;color:#1d4ed8;font-size:9px">&#128176; EUR</span></td>';
    h+='<td class="p2" style="border-right:1px solid var(--border2)"></td>';
    h+='<td class="p3" style="border-right:1px solid var(--border2)"></td>';
    /* p4: Budget EUR month */
    h+='<td class="p4 num" style="border-right:1px solid var(--border2);line-height:1.4">';
    h+='<div style="color:#1d4ed8;font-size:10px">'+(budEurMonth?budEurMonth.toLocaleString():'—')+'</div>';
    if(budEurTot>0)h+='<div style="color:var(--tx3);font-size:9px">tot:'+(budEurTot/1000).toFixed(0)+'k</div>';
    h+='</td>';
    /* p5: Actual EUR month */
    h+='<td class="p5 num" style="border-right:1px solid var(--border2);line-height:1.4">';
    h+='<div style="color:#15803d;font-size:10px">'+(totalActEurM?totalActEurM.toLocaleString():'—')+'</div>';
    if(actEurTot>0||scEurTot>0)h+='<div style="color:var(--tx3);font-size:9px">cum:'+(((actEurTot+scEurTot)/1000).toFixed(0))+'k</div>';
    h+='</td>';
    /* Var.M EUR */
    var vEC=varEurM>0?'var(--green)':varEurM<0?'var(--red)':'var(--tx3)';
    h+='<td class="num" style="font-weight:700;border-right:1px solid var(--border2);color:'+vEC+';font-size:10px">'+(varEurM!==0?(varEurM>0?'+':'')+varEurM.toLocaleString():'')+'</td>';
    ...
    for(var m=0;m<NM;m++){
      ...
      /* Add contract billings for this month */
      (S.contracts||[]).filter(function(ct){return ct.wbsId===wid;}).forEach(function(ct){
        var fcast2=+(ct.adjustedForecast||ct.monthlyForecast);
        aEm+=m<=S.ccM?(+(ct.billing[mkey(m)]||0)):fcast2;
      });
      (w.subcontracts||[]).forEach(function(sc){
        aEm+=+(sc.billing[mkey(m)]||0);
      });
      var vEM=+(bEm-aEm).toFixed(0);
      var vEMc=vEM>0?'var(--green)':vEM<0?'var(--red)':'var(--tx3)';
      h+='<td class="'+cls+'" style="padding:2px 4px;vertical-align:middle">';
      if(bEm>0||aEm>0){
        h+='<div style="display:flex;flex-direction:column;align-items:flex-end;font-family:var(--mono);font-size:9px;line-height:1.5;color:var(--tx3)">';
        h+='<div style="color:#1d4ed8">'+(bEm?(bEm/1000).toFixed(1)+'k':'—')+'</div>';
        /* Actual EUR — editable for past/cur months */
        /* Act EUR auto-computed from assignments × 26 × 8 × PU — read only */
        var _eurSty='color:#15803d'+(m>S.ccM?';font-style:italic':'');
        h+='<div style="'+_eurSty+'">'+(aEm?(aEm/1000).toFixed(1)+'k':'—')+'</div>';
        h+='<div style="font-weight:700;color:'+vEMc+'">'+(vEM!==0?(vEM>0?'+':'')+Math.round(vEM/1000)+'k':'')+'</div>';
        h+='</div>';
      }
      h+='</td>';
    }
  ...
  /* ── Subcontract summary rows ── */
  (w.subcontracts||[]).forEach(function(sc){
    var scBilled=0;
    Object.keys(sc.billing||{}).forEach(function(k){scBilled+=+(sc.billing[k]||0);});
    var scReste=+(sc.amt-scBilled).toFixed(0);
    h+='<tr class="'+wbsCls+'"><td class="p1" colspan="8" style="padding:2px 6px 2px '+(ind+12)+'px;border-right:2px solid var(--border2);background:#eff6ff">';
  ...
  (S.contracts||[]).filter(function(ct){return ct.wbsId===wid;}).forEach(function(ct){
    ...
    h+='<tr class="'+wbsCls+'"><td class="p1" colspan="8" style="padding:2px 6px 2px '+(ind+12)+'px;border-right:2px solid var(--border3);background:#faf5ff">';
  ...
  if(isLast)h+='<tr style="height:3px"><td colspan="'+(8+NM)+'" style="background:var(--bg);border-top:var(--group-sep)"></td></tr>';
  return h;
}
