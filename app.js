...
function ccGrandTotalRow(page){
  ...
  S.wbs.forEach(function(w){tB+=addBudget(w);tBM+=bud(w,page);tRC+=reelCum(w,page);tETC+=etcHMFromAssign(w.id,page);tCumBud+=budgetCumAtMonth(w,page);tCostM+=reel(w,page);});
...
function wbsCCRows(w,lv,page){
  ...
  var varEAC=+(bt-eac).toFixed(1);   /* Budget Total - EAC */
  var varEOS=+(bt-eac).toFixed(1);    /* Budget Total - EAC (=VarEAC) */
  var wid=w.id;
