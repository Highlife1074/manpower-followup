...
function ccHead(page){
  ...
  h+='<th colspan="2" style="padding:4px 4px;border-right:'+SEP+';background:#f8fafc"></th>'; /* label + note col */
  for(var m=0;m<NM;m++){
  ...
  h+='<th style="min-width:32px;border-right:1px solid var(--border2);background:#f8fafc"></th>'; /* label col */
  h+='<th style="min-width:80px;text-align:left;border-right:'+SEP+';font-size:9px;background:#f8fafc">Note</th>';
  for(var m=0;m<NM;m++){
...
function ccGrandTotalRow(page){
  ...
  h+='<td style="border-right:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.08);padding:2px 4px"><span style="font-size:8px;color:rgba(255,255,255,.5)">Σ</span></td>';
  h+='<td style="border-right:'+SEP+';background:rgba(255,255,255,.05)"></td>'; /* note col */
  /* monthly */
  for(var m=0;m<NM;m++){
...
function ccGrpRow(g,allW,page){
  ...
  h+='<td class="p3" style="border-right:1px solid var(--border2)"></td>';
  h+=_ccFixedCols(tB,tBM,tCostM,tVarM,tCumBud,tRC,tCumVar,tEAC,tVarEAC,tVarEOS,SEP,';vertical-align:top;');
  h+='<td style="border-right:1px solid var(--border2);background:#f0f9ff"></td>'; /* label */
  h+='<td style="border-right:'+SEP+';background:#f0f9ff"></td>'; /* note */
  for(var m=0;m<NM;m++){
...
function wbsCCRows(w,lv,page){
  ...
  if(hasSubRows){
    ...
    h+='<td style="border-right:'+SEP+';background:#fffbeb;padding:2px 4px;font-size:9px;color:var(--tx2);cursor:pointer;max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" onclick="editCCNote(this.dataset.wid)" data-wid="'+w.id+'" title="'+_noteR.replace(/"/g,'&quot;')+'">'+(  _noteR||'<span style="color:var(--border2)">+</span>')+'</td>';
    /* Monthly recap cells */
    for(var m=0;m<NM;m++){
    ...
    h+='<td style="border-right:'+SEP+';background:#fffbeb;padding:2px 4px;font-size:9px;color:var(--tx2);cursor:pointer;max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" onclick="editCCNote(this.dataset.wid)" data-wid="'+w.id+'" title="'+_noteS.replace(/"/g,'&quot;')+'">'+(  _noteS||'<span style="color:var(--border2)">+</span>')+'</td>';
    for(var m=0;m<NM;m++){
  ...
  (w.subcontracts||[]).forEach(function(sc){
    ...
    /* Label col */
    h+='<td style="border-right:1px solid var(--border2);background:#e0f2fe;padding:2px 5px"><div style="font-size:8px;color:var(--blue);text-align:right;line-height:2"><div>€ Bud</div><div>€ Act</div><div>Var</div></div></td>';
    h+='<td style="border-right:'+SEP+';background:#e0f2fe"></td>'; /* note */
  ...
  (S.contracts||[]).filter(function(ct){return ct.wbsId===wid;}).forEach(function(ct){
    ...
    /* Label col */
    h+='<td style="border-right:1px solid var(--border2);background:#f3e8ff;padding:2px 5px"><div style="font-size:8px;color:var(--pur);text-align:right;line-height:2"><div>€ Bud</div><div>€ Act</div><div>Var</div></div></td>';
    h+='<td style="border-right:'+SEP+';background:#f3e8ff"></td>'; /* note */
...
function _ccSpecRow(w, spec, companyId, pu, budFn, sr, alt, indLevel, page, wid, SEP, showSummary){
  ...
  h+='<td style="border-right:1px solid var(--border2);background:#fafafa"></td>'; /* label */
  h+='<td style="border-right:'+SEP+';background:#fafafa"></td>'; /* note */
  for(var m=0;m<NM;m++){
...
function _ccUpdateCellInPlace(td, wid, m, type, v){
  /* Just restore the 3 divs (Bud/Real/Var) in the cell */
  var w=null; for(var i=0;i<S.wbs.length;i++) if(S.wbs[i].id===wid){w=S.wbs[i];break;}
  if(!w) return;
  var bv=budTotal(w,m);
  var av=v; /* the value just entered */
...
function _wfCurveChart(months){
  var W=700, H=180, PAD_L=40, PAD_R=10, PAD_T=15, PAD_B=30;
  var plotW=W-PAD_L-PAD_R, plotH=H-PAD_T-PAD_B;
  var n=months.length; if(!n) return '';
  var maxV=1;
  months.forEach(function(mo){
    maxV=Math.max(maxV,mo.budHM,mo.eac,mo.assigned);
  });
  ...
  var svg='<svg viewBox="0 0 '+W+' '+H+'" style="width:100%;height:auto;font-family:var(--sans);background:var(--bg2);border-radius:8px">';

  /* Grid lines */
  for(var gi=0;gi<=4;gi++){
    var gy=PAD_T+plotH*(1-gi/4);
    var gv=Math.round(maxV*gi/4);
    svg+='<line x1="'+PAD_L+'" y1="'+gy+'" x2="'+(W-PAD_R)+'" y2="'+gy+'" stroke="#e2e8f0" stroke-width="1"/>';
    svg+='<text x="'+(PAD_L-4)+'" y="'+(gy+3)+'" text-anchor="end" font-size="8" fill="#94a3b8">'+gv+'</text>';
  }

  /* CC month vertical line */
  var ccIdx=-1;
  months.forEach(function(mo,i){if(mo.isCur)ccIdx=i;});
  if(ccIdx>=0){
    svg+='<line x1="'+x(ccIdx)+'" y1="'+PAD_T+'" x2="'+x(ccIdx)+'" y2="'+(H-PAD_B)+'" stroke="#7c3aed" stroke-width="1.5" stroke-dasharray="4,3"/>';
    svg+='<text x="'+x(ccIdx)+'" y="'+(PAD_T-4)+'" text-anchor="middle" font-size="8" fill="#7c3aed" font-weight="700">CC</text>';
  }

  /* Build path data */
  function makePath(data,key){
    var pts=[];
    for(var i=0;i<n;i++) pts.push(x(i)+','+y(data[i][key]));
    return 'M'+pts.join(' L');
  }

  /* Budget curve (dashed) */
  if(_wfShowBudget){
    svg+='<path d="'+makePath(months,'budHM')+'" fill="none" stroke="#93c5fd" stroke-width="2" stroke-dasharray="5,3"/>';
  }
  /* EAC/Real curve (solid) */
  svg+='<path d="'+makePath(months,'eac')+'" fill="none" stroke="#7c3aed" stroke-width="2.5"/>';
  /* Assigned curve */
  svg+='<path d="'+makePath(months,'assigned')+'" fill="none" stroke="#22c55e" stroke-width="2"/>';

  /* Fill area between EAC and Assigned */
  var areaUp='', areaDown='';
  for(var i=0;i<n;i++){
    areaUp+=x(i)+','+y(months[i].eac)+' ';
    areaDown=x(i)+','+y(months[i].assigned)+' '+areaDown;
  }
  svg+='<polygon points="'+areaUp+areaDown+'" fill="rgba(124,58,237,.08)"/>';
  ...
  /* X axis labels — show every 1-3 months depending on count */
  var step=n>18?3:n>10?2:1;
  months.forEach(function(mo,i){
    if(i%step!==0&&i!==n-1) return;
    svg+='<text x="'+x(i)+'" y="'+(H-5)+'" text-anchor="middle" font-size="8" fill="'+(mo.isCur?'#7c3aed':'#94a3b8')+'" '+(mo.isCur?'font-weight="700"':'')+'>'+mo.label+'</text>';
  });

  /* Legend */
  var lx=PAD_L+8, ly=PAD_T+8;
  if(_wfShowBudget){
    svg+='<line x1="'+lx+'" y1="'+ly+'" x2="'+(lx+16)+'" y2="'+ly+'" stroke="#93c5fd" stroke-width="2" stroke-dasharray="4,2"/>';
    svg+='<text x="'+(lx+20)+'" y="'+(ly+3)+'" font-size="8" fill="#3b82f6">Budget</text>';
    ly+=13;
  }
  svg+='<line x1="'+lx+'" y1="'+ly+'" x2="'+(lx+16)+'" y2="'+ly+'" stroke="#7c3aed" stroke-width="2.5"/>';
  svg+='<text x="'+(lx+20)+'" y="'+(ly+3)+'" font-size="8" fill="#7c3aed">EAC/Real</text>';
  ly+=13;
  svg+='<line x1="'+lx+'" y1="'+ly+'" x2="'+(lx+16)+'" y2="'+ly+'" stroke="#22c55e" stroke-width="2"/>';
  svg+='<text x="'+(lx+20)+'" y="'+(ly+3)+'" font-size="8" fill="#22c55e">Assigned</text>';
