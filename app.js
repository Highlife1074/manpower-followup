...
function renderGroupDetail(page){
  ...
  /* Columns:
     Group | Bud. mois | Act. mois | Var. mois | Bud. restant | Dép. prévue | Var. finale */
  h+='<div style="overflow-x:auto">';
  h+='<table style="border-collapse:collapse;font-size:11px;min-width:100%">';
  h+='<thead><tr style="background:var(--bg2)">';
  h+='<th style="text-align:left;padding:5px 10px;border:1px solid var(--border2);min-width:180px">Group / Subgroup</th>';
  /* CC month block */
  h+='<th colspan="3" style="padding:4px 8px;border:1px solid var(--border2);text-align:center;background:var(--pl);color:var(--pur)">CC Month — '+(ML[page]?ML[page].short:MS[page])+'</th>';
  /* At completion block */
  h+='<th colspan="3" style="padding:4px 8px;border:1px solid var(--border2);text-align:center;background:var(--yl);color:var(--yel)">At Completion</th>';
  h+='</tr>';
  h+='<tr style="background:var(--bg2)">';
  h+='<th style="border:1px solid var(--border2)"></th>';
  h+='<th style="padding:4px 8px;border:1px solid var(--border2);text-align:right;color:var(--amber)">Bud. mois</th>';
  h+='<th style="padding:4px 8px;border:1px solid var(--border2);text-align:right;color:var(--green)">Act. mois</th>';
  h+='<th style="padding:4px 8px;border:1px solid var(--border2);text-align:right;border-right:2px solid var(--border3)">Écart mois</th>';
  h+='<th style="padding:4px 8px;border:1px solid var(--border2);text-align:right;color:var(--amber)">Bud. restant</th>';
  h+='<th style="padding:4px 8px;border:1px solid var(--border2);text-align:right;color:var(--ora)">Dép. prévue</th>';
  h+='<th style="padding:4px 8px;border:1px solid var(--border2);text-align:right">Var. finale</th>';
  h+='</tr></thead><tbody>';

  var totBM=0,totAM=0,totBR=0,totDP=0;

  /* Helper: stats for a list of WBS */
  function grpStats(wbsList){
    /* CC month budget */
    var budMois=0;
    wbsList.forEach(function(w){budMois+=bud(w,page);});
    /* CC month actual (assignments) */
    var actMois=0;
    wbsList.forEach(function(w){actMois+=reelHMFromAssign(w.id,page);});
    /* Budget restant = sum of budget from month page+1 to NM-1 */
    var budRestant=0;
    wbsList.forEach(function(w){for(var m=page+1;m<NM;m++)budRestant+=bud(w,m);});
    /* Dépense prévue = ETC = workers assigned on future months */
    var depPrevue=0;
    wbsList.forEach(function(w){depPrevue+=etcHMFromAssign(w.id,page);});
    return{budMois:+budMois.toFixed(1),actMois:+actMois.toFixed(1),
           budRestant:+budRestant.toFixed(1),depPrevue:+depPrevue.toFixed(1)};
  }

  function renderRow(label, st, indent, rowStyle){
    var varMois=+(st.budMois-st.actMois).toFixed(1);
    var varFin=+(st.budRestant-st.depPrevue).toFixed(1);
    var h='<tr style="'+(rowStyle||'')+'">';
    h+='<td style="padding:5px 10px;border:1px solid var(--border2);font-weight:700;'+(indent?'padding-left:'+indent+'px;font-size:11px;color:#4a3a80;font-weight:700':'')+'">'+label+'</td>';
    h+='<td style="font-family:var(--mono);text-align:right;padding:5px 8px;border:1px solid var(--border2);color:var(--amber)">'+st.budMois.toFixed(1)+'</td>';
    h+='<td style="font-family:var(--mono);text-align:right;padding:5px 8px;border:1px solid var(--border2);color:var(--green)">'+st.actMois.toFixed(1)+'</td>';
    h+='<td style="font-family:var(--mono);text-align:right;padding:5px 8px;border:1px solid var(--border2);border-right:2px solid var(--border3);font-weight:700" class="'+pnc(varMois)+'">'+(varMois>0?'+':'')+varMois.toFixed(1)+'</td>';
    h+='<td style="font-family:var(--mono);text-align:right;padding:5px 8px;border:1px solid var(--border2);color:var(--amber)">'+st.budRestant.toFixed(1)+'</td>';
    h+='<td style="font-family:var(--mono);text-align:right;padding:5px 8px;border:1px solid var(--border2);color:var(--ora)">'+st.depPrevue.toFixed(1)+'</td>';
    h+='<td style="font-family:var(--mono);text-align:right;padding:5px 8px;border:1px solid var(--border2);font-weight:700" class="'+pnc(varFin)+'">'+(varFin>0?'+':'')+varFin.toFixed(1)+'</td>';
    h+='</tr>';
    return h;
  }
  ...
  for(var gi=0;gi<S.groups.length;gi++){
    ...
    h+=renderRow(g.name,gSt,null,'background:rgba(240,90,0,.07)');

    for(var si=0;si<g.sub.length;si++){
      var sg=g.sub[si];
      var sgWBS=S.wbs.filter(function(w){return w.parentId===sg.id;});
      if(!sgWBS.length) continue;
      var sSt=grpStats(sgWBS);
      h+=renderRow('◆ '+sg.name,sSt,22,'');
    }
  }
  ...
  h+='<td style="padding:5px 10px;border:1px solid #C04800">PROJECT TOTAL</td>';
  h+='<td style="font-family:var(--mono);text-align:right;padding:5px 8px;border:1px solid #C04800">'+totBM.toFixed(1)+'</td>';
  h+='<td style="font-family:var(--mono);text-align:right;padding:5px 8px;border:1px solid #C04800">'+totAM.toFixed(1)+'</td>';
  h+='<td style="font-family:var(--mono);text-align:right;padding:5px 8px;border:1px solid #C04800;border-right:2px solid #C04800;font-weight:800" class="'+pnc(tVm)+'">'+(tVm>0?'+':'')+tVm.toFixed(1)+'</td>';
  h+='<td style="font-family:var(--mono);text-align:right;padding:5px 8px;border:1px solid #C04800">'+totBR.toFixed(1)+'</td>';
  h+='<td style="font-family:var(--mono);text-align:right;padding:5px 8px;border:1px solid #C04800">'+totDP.toFixed(1)+'</td>';
  h+='<td style="font-family:var(--mono);text-align:right;padding:5px 8px;border:1px solid #C04800;font-weight:800" class="'+pnc(tVf)+'">'+(tVf>0?'+':'')+tVf.toFixed(1)+'</td>';
  h+='</tr>';
...
function renderAssignmentsPage(){
  ...
  html += '<th style="position:sticky;left:220px;z-index:20;background:var(--bg2);min-width:65px;padding:5px 8px;border-bottom:2px solid var(--border2);border-right:2px solid var(--border2);font-size:10px;font-weight:700;color:var(--tx3);text-align:right;white-space:nowrap;text-transform:uppercase;">Budget</th>';
  ...
  for(var gi = 0; gi < S.groups.length; gi++){
    ...
    /* Group header row */
    html += '<tr style="background:#d8d2c8;">';
    html += '<td colspan="' + (2 + nCols) + '" style="position:sticky;left:0;padding:6px 12px;font-weight:800;font-size:12px;letter-spacing:.01em;text-transform:uppercase;border-bottom:2px solid var(--border3);">';
    html += '<span style="cursor:pointer;margin-right:6px;font-size:11px;" onclick="tog(\'G\',\'' + g.id + '\')">' + (g.col ? '▶' : '▼') + '</span>';
    html += g.name;
    html += '</td></tr>';
    ...
    for(var si = 0; si < g.sub.length; si++){
      ...
      /* Subgroup header */
      html += '<tr style="background:#e2ddd4;">';
      html += '<td colspan="' + (2 + nCols) + '" style="position:sticky;left:0;padding:5px 12px 5px 24px;font-weight:800;font-size:12px;color:#4a3a80;border-bottom:1px solid var(--border3);">';
      html += '<span style="cursor:pointer;margin-right:6px;" onclick="tog(\'SG\',\'' + sg.id + '\')">' + (sg.col ? '▶' : '▼') + '</span>';
      html += '◆ ' + sg.name;
      html += '</td></tr>';
      if(sg.col) continue;
...
function asWBSRow(wbs, ind, startCol, page, nCols){
  ...
  h += '</td>';
  /* Sticky budget cell */
  h += '<td style="position:sticky;left:220px;z-index:5;background:var(--white);padding:5px 8px;font-family:var(--mono);font-size:11px;color:var(--blue);text-align:right;border-bottom:1px solid var(--border);border-right:2px solid var(--border2);white-space:nowrap;">';
  h += budTotal ? budTotal.toFixed(1) : '';
  h += '</td>';
  ...
  for(var m = startCol; m < NM; m++){
    ...
    if(budHM > 0){
      ...
      else if(nW < Math.ceil(budHM)) alertStyle = 'background:rgba(140,97,36,.08)!important;border-left:3px solid var(--ora)!important;';
      else alertStyle = 'background:rgba(58,125,84,.07)!important;border-left:3px solid var(--green)!important;';
    }
    if(isSel) alertStyle = 'background:rgba(61,90,128,.15)!important;outline:2px solid var(--blue)!important;';
    ...
    if(budHM > 0){
      ...
      countHtml += '<span style="color:' + (nW >= need ? 'var(--green)' : 'var(--red)') + ';">' + nW + '</span>';
      countHtml += '<span style="color:var(--tx3);font-weight:400;">/</span>';
      countHtml += '<span>' + need + '</span>';
      countHtml += '</div>';
    } else if(nW > 0){
    ...
    h += '<div style="min-width:80px;min-height:38px;padding:4px 6px;display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:' + cursorStyle + ';' + alertStyle + '"';
