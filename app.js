...
function openModal(type,id){
  ...
  h+='<label>Foreman</label><select id="mfmid">'+svOpts2+'</select>';
    h+='</div>';
    /* Responsible select */
...
function addWBS(){
  ...
  var svid=gv('msvid');var fmid=gv('mfmid');
  ...
  w.supervisorId=svid;w.foremanId=fmid||'';
  w.eurOnly=!!eurOnly;
...
function renderOrgChartPage(){
  ...
  var sups={}; /* supervisorId → {name, foremen:{foremanId → {name, wbs:[{name,workers}]}}} */
  S.wbs.forEach(function(w){
    if(S.ccNeutral&&S.ccNeutral[w.id]) return;
    var si=w.supervisorId||'_none', fi=w.foremanId||'_none';
    if(!sups[si]){var sw=si!=='_none'&&S.workers.find(function(x){return x.id===si;});sups[si]={name:sw?sw.name:'No Supervisor',foremen:{}};}
    if(!sups[si].foremen[fi]){var fw=fi!=='_none'&&S.workers.find(function(x){return x.id===fi;});sups[si].foremen[fi]={name:fw?fw.name:'No Foreman',wbs:[]};}
    var wkrs=[];
    S.assignments.forEach(function(a){
      if(a.wbsId!==w.id) return;
      var wk=S.workers.find(function(x){return x.id===a.workerId;});
      if(!wk||wkrs.some(function(x){return x.id===wk.id;})) return;
      var lastM=-1;for(var m=0;m<NM;m++){var sl=getAssignmentSlot(wk.id,m);if(sl&&sl.wbsId===w.id)lastM=m;}
      wkrs.push({id:wk.id,name:wk.name,spec:wk.spec||'',end:lastM>=0&&ML[lastM]?ML[lastM].short:'—'});
    });
    sups[si].foremen[fi].wbs.push({name:w.name,code:w.code||'',spec:w.spec||'',workers:wkrs});
  });
  var h='<div style="padding:16px;overflow-x:auto"><h2 style="font-size:16px;font-weight:700;margin:0 0 20px;color:var(--tx)">&#127970; Organization Chart</h2>';
  Object.keys(sups).sort(function(a,b){return sups[a].name.localeCompare(sups[b].name);}).forEach(function(si){
    var sup=sups[si];
    h+='<div style="margin-bottom:32px">';
    h+='<div style="text-align:center;margin-bottom:6px"><div style="display:inline-block;background:#1e3a5f;color:#fff;padding:8px 24px;border-radius:10px;font-weight:700;font-size:13px;box-shadow:0 2px 8px rgba(0,0,0,.15)">&#128119; '+sup.name+'</div></div>';
    var fks=Object.keys(sup.foremen).sort(function(a,b){return sup.foremen[a].name.localeCompare(sup.foremen[b].name);});
    h+='<div style="display:flex;justify-content:center;gap:28px;flex-wrap:wrap;padding-top:4px">';
    fks.forEach(function(fi){
      var fm=sup.foremen[fi];
      h+='<div style="display:flex;flex-direction:column;align-items:center;min-width:170px">';
      h+='<div style="width:2px;height:18px;background:#94a3b8"></div>';
      ...
      fm.wbs.forEach(function(wb){
        ...
        h+='<div style="font-size:9px;padding:2px 8px;background:'+c+'18;color:'+c+';border:1px solid '+c+'44;border-radius:4px;margin-bottom:2px;max-width:190px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="'+wb.name+'">'+(wb.code?wb.code+' — ':'')+wb.name+'</div>';
      });
      h+='<div style="background:#f59e0b;color:#fff;padding:6px 18px;border-radius:8px;font-weight:700;font-size:12px;margin:6px 0;box-shadow:0 1px 4px rgba(0,0,0,.1)">&#128736; '+fm.name+'</div>';
      var allW=[];
      fm.wbs.forEach(function(wb){wb.workers.forEach(function(wk){if(!allW.some(function(x){return x.id===wk.id;}))allW.push(wk);});});
      if(allW.length){
        h+='<div style="border-left:2px solid #e2e8f0;padding-left:10px;margin-top:2px">';
        allW.forEach(function(wk){
          ...
          h+='<span>'+wk.name+'</span>';
          h+='<span style="font-size:9px;color:var(--tx3);margin-left:auto;white-space:nowrap">→ '+wk.end+'</span>';
          h+='</div>';
        });
        h+='</div>';
