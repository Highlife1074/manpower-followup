...
function reelAtSlot(w,i){
  ...
  /* Assignments are the single source of truth for actual HM — 1 worker = 1 HM */
...
var _showEurRows=false; /* toggle EUR rows in CC */
...
function renderTopbarR(){
  ...
  /* Worker import */
  h+='<button class="btn soft" onclick="triggerXLSX()" title="Import workers from Excel">&#128101; Import Workers</button>';
  ...
  h+=_userRole?('&#128275; '+_userRole):'&#128274; Login';
  h+='</button>';
  /* Profile logins (for granular permissions) */
  _activeProfiles.forEach(function(p){
    h+='<span style="font-size:11px;background:var(--bl);border:1px solid var(--bm);border-radius:6px;padding:3px 8px;color:var(--blue);font-weight:700;cursor:pointer" onclick="logoutProfile(this.dataset.p)" data-p="'+p+'">&#128100; '+p+' &times;</span>';
  });
  if(!_activeProfiles.length){
    var _hasPrf=Object.keys((S.permissions&&S.permissions.profiles)||{}).length>0;
    if(_hasPrf) h+='<button class="btn" style="font-size:11px" onclick="loginProfile()">&#128100; Profile</button>';
  }
  if(_userRole==='admin'){
...
function renderCCPage(page){
  ...
  for(var i=0;i<S.wbs.length;i++){
    var w=S.wbs[i];
    tB+=addBudget(w);
    tBcum+=budgetCumAtMonth(w,page);
    /* Actual: from assignments (1 worker = 1 HM) */
    for(var m=0;m<=page;m++) tRC+=reelHMFromAssign(w.id,m);
    tRCeur+=reelEURFromAssign(w.id,page); /* current month EUR */
    /* ETC: assignments on future slots */
    tETC+=etcHMFromAssign(w.id,page);
    tETCeur+=etcEURFromAssign(w.id,page);
  }
  tB=+tB.toFixed(1); tBcum=+tBcum.toFixed(1);
  tRC=+tRC.toFixed(1); tETC=+tETC.toFixed(1);
  var eac=+(tRC+tETC).toFixed(1);
  ...
  banner+='<button class="btn" onclick="_showEurRows=!_showEurRows;renderCCPage('+page+')" style="font-size:11px;padding:4px 10px;background:'+(_showEurRows?'var(--ambl)':'var(--white)')+';border-color:'+(_showEurRows?'var(--amber)':'var(--border2)')+';">'+(_showEurRows?'&#128176; Hide EUR':'&#128176; Show EUR')+'</button>';
  ...
  document.getElementById('pageContent').innerHTML=banner+botHtml+tableHtml;
}

function ccHead(page){
  var h='<tr><th class="p1" style="min-width:220px;text-align:left;border-right:2px solid var(--border2)">Code / WBS</th>';
  h+='<th class="p2" style="min-width:90px;text-align:left;border-right:1px solid var(--border2)">Company</th>';
  h+='<th class="p3" style="min-width:90px;text-align:left;border-right:1px solid var(--border2)">Specialty</th>';
  h+='<th class="p4" style="min-width:62px;border-right:1px solid var(--border2)">Bud.</th>';
  h+='<th class="p5" style="min-width:62px;border-right:1px solid var(--border2)">Actual</th>';
  h+='<th style="min-width:60px;border-right:1px solid var(--border2);position:sticky;left:564px;z-index:12;background:var(--bg2)">Var.M</th>';
  h+='<th style="min-width:62px;border-right:2px solid var(--border3);position:sticky;left:624px;z-index:12;background:var(--bg2)">Var.Fin</th>';
  for(var m=0;m<NM;m++){
...
function ccGrandTotal(page){
  ...
  tB=+tB.toFixed(1);tR=+tR.toFixed(1);var gV=+(tB-tR).toFixed(1); /* Budget-Cost */
  var h='';
  /* Budget */
  h+='<tr class="rg"><td class="p1" style="padding-left:10px;border-right:2px solid #C04800;background:#2563eb"><span style="color:#1a1a1a;font-size:12px;font-weight:800">PROJECT TOTAL</span></td>';
  h+='<td class="p2" style="border-right:1px solid #C04800;background:#2563eb"><span class="tp tpb">Budget</span></td>';
  h+='<td class="p3" style="border-right:1px solid #C04800;background:#2563eb"></td>';
  h+='<td class="p4" style="border-right:2px solid #C04800;background:#2563eb"><span class="num" style="color:#1a1a1a;font-weight:800">'+tB.toFixed(1)+'</span></td>';
  h+='<td class="num" style="color:#1a1a1a;font-weight:800;border-right:2px solid #C04800">--</td>';
  h+='<td style="border-right:2px solid #C04800;background:#2563eb"></td>';
  for(var m=0;m<NM;m++){var cls=m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td';h+='<td class="'+cls+' num" style="color:#1a1a1a;font-weight:700;background:#2563eb">'+f1(mB[m])+'</td>';}
  h+='</tr>';
  /* Actual */
  ...
  h+='<td class="p5 num" style="color:#1a1a1a;font-weight:800;border-right:1px solid #2a5d3e;background:#3a7d54">'+tR.toFixed(1)+'</td><td style="border-right:1px solid #2a5d3e;background:#3a7d54"></td><td style="border-right:2px solid #1a4a2e;background:#3a7d54"></td>';
  ...
  h+='</tr>';
  /* Variance */
  var vc=gV>0?'var(--green)':'var(--red)'; /* Budget-Cost: positive=green */
  ...
  for(var m=0;m<NM;m++){var bv=mB[m],rv=mR[m],va2=+(bv-rv).toFixed(1); /* Budget-Cost */var cls=m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td';var vc2=va2>0?'var(--green)':va2<0?'var(--red)':'var(--tx3)'; /* Budget-Cost */h+='<td class="'+cls+' num" style="color:'+vc2+';background:#f8f4ef">'+(va2!==0?(va2>0?'+':'')+va2.toFixed(1):'')+'</td>';}
...
function ccGrpRow(g,allW,page){
  ...
  var va=+(tB-tRC).toFixed(1); /* Budget-Cost */
  ...
  var _gETC=0;allW.forEach(function(w){_gETC+=etcHMFromAssign(w.id,page);});var _gBT=0;allW.forEach(function(w){_gBT+=addBudget(w);});var _gVF=+(_gBT-(tRC+_gETC)).toFixed(1);
  h+='<td class="num '+pncBC(va)+'" style="font-weight:700;border-right:1px solid var(--border2)">'+(va>0?'+':'')+va.toFixed(1)+'</td>';
  h+='<td class="num '+pncBC(_gVF)+'" style="font-weight:800;border-right:2px solid var(--border3)">'+(+_gVF>0?'+':'')+_gVF.toFixed(1)+'</td>';
...
function ccSgRow(sg,sgW,page){
  ...
  h+='<td class="num '+pncBC(va)+'" style="border-right:2px solid var(--border2)">'+(va>0?'+':'')+va.toFixed(1)+'</td>';
...
function ecCCCell(el,wid,m,type,cur){
  ...
  var td=el.closest('td')||el.parentElement;
  ecCC(td,wid,m,type,cur);
}

function wbsCCRows(w,lv,page,isLast){
  var ind=lv===1?16:36;
  var bt=addBudget(w);
  var rc2=reelCum(w,page);
  var va=+(bt-rc2).toFixed(1);
  ...
  var budMonth=bud(w,page);
  var reelMonth=reel(w,page);
  var _etc=etcHMFromAssign(w.id,page);
  var _eac=+(rc2+_etc).toFixed(1);
  var _vaFin=+(bt-_eac).toFixed(1);

  h+='<tr class="wbs-sep-row"><td colspan="'+(7+NM)+'"></td></tr>';

  /* ── Row 1: WBS header (sticky cols only, no monthly values) ── */
  h+='<tr class="rwbs '+wbsCls+'">';
  h+='<td class="p1" style="padding-left:'+ind+'px;border-right:2px solid var(--border2)" rowspan="3"><div class="ar">';
  if(w.code)h+='<span class="codb">'+w.code+'</span>';
  h+='<div style="display:flex;flex-direction:column">';
  h+='<span style="font-weight:600;font-size:12px">'+w.name+'</span>';
  if(w.responsible)h+='<span style="font-size:10px;color:var(--blue)">&#128203; '+w.responsible+'</span>';
  h+='</div></div></td>';
  h+='<td class="p2" style="border-right:1px solid var(--border2)" rowspan="3">'+(cname?'<span class="cobadge" style="background:'+cbg+';color:'+cco+'">'+cname+'</span>':'')+'</td>';
  h+='<td class="p3" style="border-right:1px solid var(--border2)" rowspan="3">'+(sl?'<span class="spsel sk-'+w.spec+'" style="cursor:default">'+sl+'</span>':'')+'</td>';
  /* p4: Budget month + total — empty if no budget */
  h+='<td class="p4 num" style="border-right:1px solid var(--border2);line-height:1.4">';
  if(budMonth>0){
    h+='<div style="color:var(--blue);font-weight:700;font-size:11px">'+budMonth.toFixed(1)+'</div>';
    if(bt>budMonth)h+='<div style="color:var(--tx3);font-size:9px">tot:'+bt.toFixed(0)+'</div>';
  }
  h+='</td>';
  /* p5: Actual month + cum */
  h+='<td class="p5 num" style="border-right:1px solid var(--border2);line-height:1.4">';
  h+='<div style="color:var(--green);font-weight:700;font-size:11px">'+reelMonth.toFixed(1)+'</div>';
  if(rc2>0)h+='<div style="color:var(--tx3);font-size:9px">cum:'+rc2.toFixed(0)+'</div>';
  h+='</td>';
  /* Var.M — empty when no budget and no actual */
  h+='<td class="num '+pncBC(va)+'" style="font-weight:700;border-right:1px solid var(--border2)">'+(bt>0||rc2>0?(va>0?'+':'')+va.toFixed(1):'')+'</td>';
  /* Var.Fin */
  h+='<td class="num '+pncBC(_vaFin)+'" style="font-weight:800;font-size:12px;border-right:2px solid var(--border3)">'+(bt>0?((_vaFin>0?'+':'')+_vaFin.toFixed(1)):'')+'</td>';
  /* Budget monthly cells */
  for(var m=0;m<NM;m++){
    var cls=m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td';
    var bv=bud(w,m);
    h+='<td class="'+cls+' lk num" style="color:var(--blue);font-size:10px">'+(bv?bv.toFixed(1):'')+'</td>';
  }
  h+='</tr>';

  /* ── Row 2: Actual/ETC ── */
  h+='<tr class="rre '+wbsCls+'">';
  h+='<td class="p4 num" style="border-right:1px solid var(--border2);line-height:1.4" rowspan="2">';
  /* empty — already filled in row 1 with rowspan could work but keep simple */
  h+='</td><td class="p5 num" style="border-right:1px solid var(--border2)"></td>';
  h+='<td style="border-right:1px solid var(--border2)"></td>';
  h+='<td style="border-right:2px solid var(--border3)"></td>';
  for(var m=0;m<NM;m++){
    var cls=m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td';
    var av,avStyle,avExtra='';
    if(m<=page){
      av=reel(w,m);
      avStyle=m===page?'color:var(--pur);font-weight:700':'color:var(--tx2)';
    } else {
      var hasManualRad=ccSlot(m).rad[w.id]!==undefined;
      var _asgFut=reelHMFromAssign(w.id,m);
      av=hasManualRad?+ccSlot(m).rad[w.id]:_asgFut;
      avStyle=hasManualRad?'color:#c2410c;font-weight:700':'color:var(--tx2);font-style:italic';
      var _delta=hasManualRad?+(av-_asgFut).toFixed(1):0;
      if(_delta!==0)avExtra='<span style="font-size:8px;color:'+(_delta>0?'#15803d':'#b91c1c')+'"> ('+(_delta>0?'+':'')+_delta+')</span>';
    }
    var _ccMonthLocked=isCCMonthLocked(m);
    h+='<td class="'+cls+' num" style="'+avStyle+';">';
    if(m>page&&!_ccMonthLocked){
      h+='<span style="cursor:pointer;text-decoration:underline dotted" onclick="ecCCCell(this,\''+wid+'\','+m+',\'rad\','+av+')">'+av.toFixed(1)+avExtra+'</span>';
    } else {
      h+=av.toFixed(1)+avExtra;
    }
    h+='</td>';
  }
  h+='</tr>';
  ...
  /* ── Row 3: Variance ── */
  h+='<tr class="rvar '+wbsCls+'">';
  h+='<td style="border-right:1px solid var(--border2)"></td>';
  h+='<td style="border-right:1px solid var(--border2)"></td>';
  h+='<td style="border-right:2px solid var(--border3)"></td>';
  for(var m=0;m<NM;m++){
    var bv=bud(w,m);
    var rv=m<=page?reel(w,m):rad(w,m);
    var va2=+(bv-rv).toFixed(1);
    var cls=m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td';
    /* Only show variance if there's budget or actual */
    h+='<td class="'+cls+' num '+pncBC(va2)+'" style="font-size:10px">'+(( bv>0||rv>0)&&va2!==0?(va2>0?'+':'')+va2.toFixed(1):'')+'</td>';
  }
  h+='</tr>';

  /* ── Row EUR (compact, read-only) ── */
  var budEurTot=addBudgetEurSlots(w);
  var actEurTot=reelEurCum(w,page);
  var scEurTot=0;
  (w.subcontracts||[]).forEach(function(sc){Object.keys(sc.billing||{}).forEach(function(k){scEurTot+=+(sc.billing[k]||0);});});
  (S.contracts||[]).filter(function(ct){return ct.wbsId===wid;}).forEach(function(ct){Object.keys(ct.billing||{}).forEach(function(k){scEurTot+=+(ct.billing[k]||0);});});
  var _hasContracts=(w.subcontracts&&w.subcontracts.length>0)||((S.contracts||[]).some(function(ct){return ct.wbsId===wid;}));
  var hasEurData=budEurTot>0||actEurTot>0||scEurTot>0||_hasContracts;

  if(hasEurData){
    /* EUR Budget row */
    h+='<tr class="rbud '+wbsCls+'" style="opacity:.85">';
    h+='<td class="p1" style="padding-left:'+(ind+12)+'px;border-right:2px solid var(--border2);font-size:9px;color:#1d4ed8">&#128176; Bud EUR</td>';
    h+='<td class="p2" style="border-right:1px solid var(--border2)"></td>';
    h+='<td class="p3" style="border-right:1px solid var(--border2)"></td>';
    var budEurM=getBudEur(w,page);
    var budEurTotV=addBudgetEurSlots(w);
    h+='<td class="p4 num" style="color:#1d4ed8;font-size:10px;border-right:1px solid var(--border2)">'+(budEurM?(budEurM/1000).toFixed(1)+'k':'')+'</td>';
    h+='<td class="p5 num" style="border-right:1px solid var(--border2);font-size:9px;color:var(--tx3)">'+(budEurTotV?(budEurTotV/1000).toFixed(0)+'k tot':'')+'</td>';
    h+='<td style="border-right:1px solid var(--border2)"></td>';
    h+='<td style="border-right:2px solid var(--border3)"></td>';
    for(var m=0;m<NM;m++){
      var cls=m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td';
      var bEm=getBudEur(w,m);
      h+='<td class="'+cls+' lk num" style="color:#1d4ed8;font-size:9px">'+(bEm?(bEm/1000).toFixed(1)+'k':'')+'</td>';
    }
    h+='</tr>';

    /* EUR Actual row */
    h+='<tr class="rbud '+wbsCls+'" style="opacity:.85">';
    h+='<td class="p1" style="padding-left:'+(ind+12)+'px;border-right:2px solid var(--border2);font-size:9px;color:#15803d">&#128176; Act EUR</td>';
    h+='<td class="p2" style="border-right:1px solid var(--border2)"></td>';
    h+='<td class="p3" style="border-right:1px solid var(--border2)"></td>';
    var actEurM=getReelEur(w,page);
    /* Add contract billings */
    var scEM=0;
    (w.subcontracts||[]).forEach(function(sc){scEM+=+(sc.billing[mkey(page)]||0);});
    (S.contracts||[]).filter(function(ct){return ct.wbsId===wid;}).forEach(function(ct){
      var fcast=+(ct.adjustedForecast||ct.monthlyForecast);
      scEM+=page<=S.ccM?(+(ct.billing[mkey(page)]||0)):fcast;
    });
    var totalActEurM=actEurM+scEM;
    var totalActEurTot=actEurTot+scEurTot;
    h+='<td class="p4 num" style="color:#15803d;font-size:10px;border-right:1px solid var(--border2)">'+(totalActEurM?(totalActEurM/1000).toFixed(1)+'k':'')+'</td>';
    h+='<td class="p5 num" style="border-right:1px solid var(--border2);font-size:9px;color:var(--tx3)">'+(totalActEurTot?(totalActEurTot/1000).toFixed(0)+'k cum':'')+'</td>';
    var varEurM=+(budEurM-totalActEurM).toFixed(0);
    var vEC=varEurM>0?'var(--green)':varEurM<0?'var(--red)':'var(--tx3)';
    h+='<td class="num" style="font-weight:700;border-right:1px solid var(--border2);color:'+vEC+';font-size:10px">'+(varEurM!==0?(varEurM>0?'+':'')+Math.round(varEurM/1000)+'k':'')+'</td>';
    /* Var.Fin EUR = Budget EUR total - EAC EUR */
    var _eacEurAct=reelEurCum(w,page);
    var _eacEurFut=0;
    for(var _mf=page+1;_mf<NM;_mf++){
      _eacEurFut+=getReelEur(w,_mf);
      (S.contracts||[]).filter(function(ct){return ct.wbsId===wid;}).forEach(function(ct){_eacEurFut+=+(ct.adjustedForecast||ct.monthlyForecast);});
    }
    var _eacEurTotal=+(_eacEurAct+_eacEurFut).toFixed(0);
    var _varFinEur=+(budEurTotV-_eacEurTotal).toFixed(0);
    var _vFEC=_varFinEur>0?'var(--green)':_varFinEur<0?'var(--red)':'var(--tx3)';
    h+='<td class="num" style="font-weight:800;border-right:2px solid var(--border3);color:'+_vFEC+';font-size:10px">'+(budEurTotV>0&&_varFinEur!==0?(_varFinEur>0?'+':'')+Math.round(_varFinEur/1000)+'k':'')+'</td>';
    for(var m=0;m<NM;m++){
      var cls=m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td';
      var aEm=getReelEur(w,m);
      (w.subcontracts||[]).forEach(function(sc){aEm+=+(sc.billing[mkey(m)]||0);});
      (S.contracts||[]).filter(function(ct){return ct.wbsId===wid;}).forEach(function(ct){
        var fcast2=+(ct.adjustedForecast||ct.monthlyForecast);
        aEm+=m<=S.ccM?(+(ct.billing[mkey(m)]||0)):fcast2;
      });
      h+='<td class="'+cls+' lk num" style="color:#15803d;font-size:9px;'+(m>S.ccM?'font-style:italic':'')+';">'+(aEm?(aEm/1000).toFixed(1)+'k':'')+'</td>';
    }
    h+='</tr>';
  }
  ...
  (w.subcontracts||[]).forEach(function(sc){
    var scBilled=0;
    Object.keys(sc.billing||{}).forEach(function(k){scBilled+=+(sc.billing[k]||0);});
    var scReste=+(sc.amt-scBilled).toFixed(0);
    h+='<tr class="'+wbsCls+'"><td class="p1" colspan="7" style="padding:2px 6px 2px '+(ind+12)+'px;border-right:2px solid var(--border2);background:#eff6ff">';
    h+='<span style="font-size:10px;color:var(--blue)">&#128196; <strong>'+sc.ref+'</strong>';
    if(sc.company)h+=' — '+sc.company;
    h+=' &nbsp;·&nbsp; Bud: <strong>'+fkv(sc.amt)+'€</strong>';
    h+=' &nbsp;·&nbsp; Fact: <strong>'+fkv(scBilled)+'€</strong>';
    h+=' &nbsp;·&nbsp; Reste: <strong style="color:'+(scReste<0?'var(--red)':'var(--green)')+'">'+fkv(scReste)+'€</strong>';
    h+='</span></td>';
    for(var m=0;m<NM;m++){
      var k2=mkey(m);var sv=+(sc.billing[k2]||0);
      var cls2=m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td';
      var _scLk=isCCMonthLocked(m);
      var _scPast=m<S.ccM; /* past months are locked */
      if(!_scLk&&!_scPast){
        var _scStyle='color:var(--blue);font-size:10px;cursor:pointer'+(m>S.ccM?';font-style:italic':'');
        h+='<td class="'+cls2+' edt num" style="'+_scStyle+'" onclick="openScBillingCell(this,this.dataset.wid,this.dataset.scid,this.dataset.k,'+sv+')" data-wid="'+w.id+'" data-scid="'+sc.id+'" data-k="'+k2+'">'+(sv?fkv(sv):(m>S.ccM?'<span style="color:var(--border2);font-size:9px">+</span>':''))+'</td>';
      } else {
        h+='<td class="'+cls2+' lk num" style="color:var(--blue);font-size:10px">'+(sv?fkv(sv):'')+'</td>';
      }
    }
    h+='</tr>';
  });
  ...
  /* ── Global contracts ── */
  (S.contracts||[]).filter(function(ct){return ct.wbsId===wid;}).forEach(function(ct){
    var _isCCLk=isCCMonthLocked(page);
    var _cbTotal=0;Object.keys(ct.billing||{}).forEach(function(k){_cbTotal+=+(ct.billing[k]||0);});
    var _cnM=contractActiveMonths(ct);
    var _ctBud=ct.totalAmount>0?ct.totalAmount:+(ct.monthlyForecast*_cnM).toFixed(0);
    var _cEAC=contractEAC(ct);
    var _cOver=_cEAC>_ctBud&&_ctBud>0;
    var _cExh=contractExhaustDate(ct);
    h+='<tr class="'+wbsCls+'"><td class="p1" colspan="5" style="padding:2px 6px 2px '+(ind+12)+'px;border-right:1px solid var(--border2);background:#faf5ff">';
    /* Compute spent so far (past + CC month actual) */
    var _cSpent=0;
    for(var _m=0;_m<=S.ccM;_m++){
      if(!contractMonthActive(ct,_m)) continue;
      _cSpent+=ct.billing&&ct.billing[mkey(_m)]!==undefined?+(ct.billing[mkey(_m)]):0;
    }
    /* Forecast remaining: sum of adjustedForecast for future months */
    var _cFcastRem=0;
    for(var _m=S.ccM+1;_m<NM;_m++){
      if(!contractMonthActive(ct,_m)) continue;
      _cFcastRem+=+(ct.adjustedForecast||ct.monthlyForecast);
    }
    var _cProjected=_cSpent+_cFcastRem;
    var _cOverrun=_ctBud>0&&_cProjected>_ctBud;
    var _cOverAmt=_cProjected-_ctBud;
    var _cFcastCur=+(ct.adjustedForecast||ct.monthlyForecast);
    var _alertHtml='';
    if(_ctBud>0&&_cOverrun){
      _alertHtml='<span style="display:inline-flex;align-items:center;gap:4px;margin-left:8px;background:#fee2e2;color:#b91c1c;font-weight:800;font-size:10px;padding:2px 8px;border-radius:6px;border:1px solid #fca5a5">'
        +'⚠ Dépassement +'+fkv(_cOverAmt)+'€'
        +(_cExh?' · épuisé '+_cExh:'')
        +'</span>';
    } else if(_ctBud>0){
      _alertHtml='<span style="display:inline-flex;align-items:center;gap:4px;margin-left:8px;background:#dcfce7;color:#15803d;font-size:10px;padding:2px 8px;border-radius:6px;border:1px solid #86efac">'
        +'✓ OK · reste '+fkv(_ctBud-_cProjected)+'€'
        +'</span>';
    }
    h+='<span style="font-size:10px;color:var(--pur)">&#128196; <strong>'+ct.ref+'</strong>';
    if(ct.company)h+=' — <span style="color:var(--tx2)">'+ct.company+'</span>';
    if(_ctBud>0){
      h+=' &nbsp;·&nbsp; Bud: <strong>'+fkv(_ctBud)+' €</strong>';
      h+=' &nbsp;·&nbsp; Dép: <strong style="color:var(--tx2)">'+fkv(_cSpent)+' €</strong>';
    }
    h+=_alertHtml;
    if(!_ctBud)h+=' <span style="color:var(--red);font-size:9px">(montant contrat non renseigné)</span>';
    h+='</span></td>';
    var _ctActMon=ct.billing&&ct.billing[mkey(page)]!==undefined?+(ct.billing[mkey(page)]):0;
    var _ctFcastMon=+(ct.adjustedForecast||ct.monthlyForecast);
    var _ctVarM=+(_ctFcastMon-_ctActMon).toFixed(0);
    var _ctVarMCol=_ctVarM>0?'var(--green)':_ctVarM<0?'var(--red)':'var(--tx3)';
    var _ctVarFin=_ctBud>0?+(_ctBud-_cProjected).toFixed(0):0;
    var _ctVarFinCol=_ctVarFin>0?'var(--green)':_ctVarFin<0?'var(--red)':'var(--tx3)';
    h+='<td class="num" style="font-size:10px;font-weight:700;color:'+_ctVarMCol+';border-right:1px solid var(--border2);background:#faf5ff">'+((_ctFcastMon>0||_ctActMon>0)&&_ctVarM!==0?(_ctVarM>0?'+':'')+fkv(_ctVarM):'')+'</td>';
    h+='<td class="num" style="font-size:10px;font-weight:800;color:'+_ctVarFinCol+';border-right:2px solid var(--border3);background:#faf5ff">'+(_ctBud>0&&_ctVarFin!==0?(_ctVarFin>0?'+':'')+fkv(_ctVarFin):'')+'</td>';
    for(var m=0;m<NM;m++){
      var _ccls2=m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td';
      if(!contractMonthActive(ct,m)){
        h+='<td class="'+_ccls2+'"></td>'; continue;
      }
      var _ck=mkey(m);
      var _cFcast=+(ct.adjustedForecast||ct.monthlyForecast);
      var _cIsReal=!!(ct.billing&&ct.billing[_ck]!==undefined);
      var _cBilled=_cIsReal?+(ct.billing[_ck]):0;
      var _ccLkM=isCCMonthLocked(m);
      var _isPast=m<S.ccM;

      /* What to display */
      var _cDisp, _cLabel, _cColor, _cItalic=false;
      if(m<S.ccM){
        /* Past: show actual billed (read-only) */
        _cDisp=_cIsReal?_cBilled:0;
        _cColor=_cIsReal?'#15803d':'var(--tx3)';
        _cLabel=_cDisp?fkv(_cDisp):'';
      } else if(m===S.ccM){
        /* CC month: show actual billed, editable */
        _cDisp=_cIsReal?_cBilled:0;
        _cColor=_cIsReal?'#15803d':'var(--tx3)';
        _cLabel=_cDisp?fkv(_cDisp):'';
      } else {
        /* Future: show per-month billing if set, else forecast */
        _cDisp=_cIsReal?_cBilled:_cFcast;
        _cColor=_cIsReal?'#15803d':'var(--tx3)';
        _cItalic=!_cIsReal;
        _cLabel=_cDisp?fkv(_cDisp):'';
      }
      ...
      /* Overrun check */
      var _cumToM=0;
      for(var _mx=0;_mx<=m;_mx++){
        if(!contractMonthActive(ct,_mx)) continue;
        var _mk2=mkey(_mx);
        if(_mx<=S.ccM){
          _cumToM+=ct.billing&&ct.billing[_mk2]!==undefined?+(ct.billing[_mk2]):0;
        } else {
          _cumToM+=ct.billing&&ct.billing[_mk2]!==undefined?+(ct.billing[_mk2]):_cFcast;
        }
      }
      var _cellOver=_ctBud>0&&_cumToM>_ctBud;
      var _overBg=_cellOver?';background:rgba(185,28,28,.08)':'';

      var _tdStyle='font-size:10px;color:'+_cColor+_overBg+(_cItalic?';font-style:italic':'')+';cursor:pointer';

      if(!_ccLkM&&!_isPast){
        /* Editable: CC month and future months */
        h+='<td class="'+_ccls2+' edt num" style="'+_tdStyle+'" onclick="openContractCellEdit(this)" data-cid="'+ct.id+'" data-m="'+m+'" data-v="'+_cDisp+'">'+_cLabel+'</td>';
      } else {
        /* Read-only: past months or locked */
        h+='<td class="'+_ccls2+' lk num" style="font-size:10px;color:'+_cColor+_overBg+'">'+_cLabel+'</td>';
      }
    }
    h+='</tr>';
  });

  if(isLast)h+='<tr style="height:3px"><td colspan="'+(7+NM)+'" style="background:var(--bg);border-top:var(--group-sep)"></td></tr>';
  return h;
}
...
function renderGroupDetail(page){
  ...
  function grpStats(wbsList){
    var budMois=0,actMois=0,budRestant=0,depPrevue=0;
    wbsList.forEach(function(w){
      budMois+=bud(w,page);
      actMois+=reelHMFromAssign(w.id,page);
      for(var m=page+1;m<NM;m++) budRestant+=bud(w,m);
      depPrevue+=etcHMFromAssign(w.id,page);
    });
    return{
      budMois:+budMois.toFixed(1),
      actMois:+actMois.toFixed(1),
      budRestant:+budRestant.toFixed(1),
      depPrevue:+depPrevue.toFixed(1)
    };
  }

  var totBM=0,totAM=0,totBR=0,totDP=0;
  var groupData=[];
  for(var gi=0;gi<S.groups.length;gi++){
    var g=S.groups[gi];
    ...
    totBM+=gSt.budMois; totAM+=gSt.actMois; totBR+=gSt.budRestant; totDP+=gSt.depPrevue;
    var subData=[];
    for(var si=0;si<g.sub.length;si++){
      var sg=g.sub[si];
  ...
  h+='<button class="btn" onclick="event.stopPropagation();printGroupDetail()" style="padding:4px 10px;font-size:11px">&#128438; Print</button>';
  ...
  for(var gi=0;gi<groupData.length;gi++){
    ...
    for(var si=0;si<gd.subs.length;si++){
      ...
      var sVf=+(sub.st.budRestant-sub.st.depPrevue).toFixed(1);
  ...
  function buildPicker(ft){
    ...
    function workerStatus(wk){
      ...
      pairs.forEach(function(p){
        ...
        if(!active){freePairs.push(p);return;}
    ...
    for(var i=0;i<workers.length;i++){
      ...
      h+='<div class="wc-name">'+wk.name+(wk.workerId?' <span style="font-size:9px;color:var(--tx3)">#'+wk.workerId+'</span>':'')+'</div>';
      h+='<div class="wc-meta">'+(coName||'—')+(sl2?' · <span class="spsel sk-'+wk.spec+'" style="cursor:default;font-size:9px;padding:1px 5px">'+sl2+'</span>':'')+'</div>';
      h+=statusHtml;
      h+='</div>';
    }
...
function openSettingsModal(){
  ...
  (S.wbsResponsibles||[]).forEach(function(r){
    ...
    h+=r+' <button class="bic del" style="width:14px;height:14px;font-size:8px" onclick="delWbsResponsible(this.dataset.r)" data-r="'+r+'">x</button>';
    h+='</span>';
  });
  h+='</div>';
...
function propagateFromPrevMonth(targetSlot){
  ...
  prevAssignments.forEach(function(a){
    ...
    if(!workerActiveInMonth(wk,tMo,tYr)) return;
...
function saveWorkerModal(id,isNew){
  ...
  var wk={
    ...
    rating:rating,comment:comment,category:category,responsible:responsible
  };
...
function renderWorkersPage(){
  ...
  for(var wi=0;wi<sorted.length;wi++){
    ...
    html+='<td><div class="co-sel-wrap" style="font-size:11px"><span class="spsel sk-'+wk.spec+'" style="cursor:pointer">'+(sl||'—')+'</span><select onchange="wkFieldSave(\''+wk.id+'\',\'spec\',this.value)">'+wspO+'</select></div></td>';
...
function asWBSRow(wbs, ind, startCol, page, nCols){
  ...
  for(var m = startCol; m < NM; m++){
    ...
    var clickAttr = canClick ? ' onclick="handleCellClick(event,\'' + cellKey + '\')"' : '';
...
function showChipPop(e, el){
  ...
  pop.innerHTML = '<div class="chip-pop-name">' + wk.name + '</div>' +
...
function getRolePwds(){
  if(!S.rolePwds) S.rolePwds={admin:'admin123',manager:'mgr456',viewer:'view789'};
  return S.rolePwds;
}

function canDo(action){
  /* action: 'edit_budget','lock_cc','edit_workers','view_only','admin' */
  if(!_userRole) return false;
  if(_userRole==='admin') return true;
  if(_userRole==='manager') return action!=='admin';
  if(_userRole==='viewer') return action==='view';
  return false;
}
...
function openLoginModal(){
  ...
  box.className='modal'; document.getElementById('mbg').className='mbg';
  var h='<h3>&#128274; Login</h3>';
  h+='<p style="font-size:12px;color:var(--tx2);margin-bottom:12px">Enter your access password to continue.</p>';
  h+='<label>Password</label><input id="loginPwd" type="password" placeholder="Enter password" autofocus>';
  h+='<div class="ma"><button class="btn" onclick="closeModal()">Cancel</button><button class="btn pri" onclick="tryLogin()">Login</button></div>';
  box.innerHTML=h; document.getElementById('mbg').style.display='flex';
...
function tryLogin(){
  ...
  if(pwd===pwds.admin){ _userRole='admin'; sessionStorage.setItem('manpower_role','admin'); }
  else if(pwd===pwds.manager){ _userRole='manager'; sessionStorage.setItem('manpower_role','manager'); }
  else if(pwd===pwds.viewer){ _userRole='viewer'; sessionStorage.setItem('manpower_role','viewer'); }
  else { showToast('Wrong password','warn'); return; }
  closeModal();
  showToast('Logged in as '+_userRole+' ✓','ok');
  render();
}
...
function openRoleSettings(){
  ...
  h+='<div><label>&#128203; Manager password<br><span style="font-size:10px;color:var(--tx3)">Edit workers, CC, assignments</span></label><input id="rpMgr" type="password" value="'+pwds.manager+'"></div>';
  h+='<div><label>&#128065; Viewer password<br><span style="font-size:10px;color:var(--tx3)">Read-only access</span></label><input id="rpViewer" type="password" value="'+pwds.viewer+'"></div>';
  h+='</div>';
...
function saveRolePwds(){
  ...
  S.rolePwds={admin:a,manager:m,viewer:v};
...
var PERMISSION_DEFS = [
  /* Workers */
  {key:'add_worker',      label:'Add / Delete workers',        tab:'Workers', view:true},
  {key:'edit_worker',     label:'Edit worker details',          tab:'Workers', view:true},
  {key:'paper_check',     label:'Paper Check column',           tab:'Workers', view:true},
  {key:'induction',       label:'Induction column (Safety)',    tab:'Workers', view:true},
  /* Budget */
  {key:'edit_budget',     label:'Edit Initial Budget',          tab:'Budget',  view:false},
  /* CC */
  {key:'lock_cc',         label:'Lock / Unlock CC months',      tab:'CC',      view:false},
  {key:'cc_actual',       label:'Enter CC actual values',       tab:'CC',      view:true},
  {key:'edit_wbs_resp',   label:'Set WBS Responsible',          tab:'CC/Budget',view:true},
  {key:'edit_etc',        label:'Edit ETC future months',       tab:'CC',      view:true},
  {key:'view_eur',        label:'View EUR rows',                tab:'CC',      view:true},
  /* Contracts */
  {key:'edit_contracts',  label:'Add / Edit Contracts',         tab:'Contracts',view:true},
  {key:'view_contracts',  label:'View Contracts tab',           tab:'Contracts',view:true},
  /* Assignments */
  {key:'edit_assignments',label:'Edit Assignments',             tab:'Assignments',view:true},
  /* Admin */
  {key:'settings',        label:'Access Settings',              tab:'Admin',   view:false},
  {key:'view_improvements',label:'View Improvement Log',        tab:'Admin',   view:true},
];
...
function hasRight(key){
  ...
  var allowed = perms.rights[key] || ['admin'];
  /* Check if any active profile has this right */
  for(var i=0;i<_activeProfiles.length;i++){
    if(allowed.indexOf(_activeProfiles[i])>=0) return true;
  }
  /* Check legacy roles */
  if(_userRole && allowed.indexOf(_userRole)>=0) return true;
  return false;
}
...
function openPermissionsModal(){
  ...
  var perms=getPermissions();
  var profiles=perms.profiles||{};
  var rights=perms.rights||{};

  var box=document.getElementById('mbox');
  box.className='modal modal-wide'; document.getElementById('mbg').className='mbg';

  var h='<h3>&#128100; Profiles &amp; Permissions</h3>';

  /* Profile list */
  h+='<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px">';
  h+='<div style="font-size:11px;font-weight:700;color:var(--tx3);text-transform:uppercase">Profiles</div>';
  h+='<button class="btn" style="font-size:10px" onclick="addProfilePrompt()">+ Add profile</button>';
  h+='</div>';
  h+='<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:14px">';
  Object.keys(profiles).forEach(function(p){
    h+='<span style="display:inline-flex;align-items:center;gap:6px;background:var(--bl);border:1.5px solid var(--bm);border-radius:8px;padding:4px 10px;font-size:11px;font-weight:700;color:var(--blue)">';
    h+=p;
    h+=' <button class="bic del" onclick="delProfile(\''+p+'\')" style="font-size:8px;width:14px;height:14px">x</button>';
    h+='</span>';
  });
  if(!Object.keys(profiles).length) h+='<span style="color:var(--tx3);font-size:11px">No profiles yet</span>';
  h+='</div>';

  /* Rights matrix */
  h+='<div style="font-size:11px;font-weight:700;color:var(--tx3);text-transform:uppercase;margin-bottom:6px">Permissions</div>';
  h+='<div style="overflow-x:auto;margin-bottom:14px">';
  h+='<table style="border-collapse:collapse;font-size:11px;min-width:400px">';
  h+='<thead><tr style="background:var(--bg2)">';
  h+='<th style="text-align:left;padding:5px 10px;border:1px solid var(--border2);min-width:220px">Permission</th>';
  h+='<th style="padding:5px 8px;border:1px solid var(--border2);min-width:80px;color:var(--blue)">Admin</th>';
  Object.keys(profiles).forEach(function(p){
    h+='<th style="padding:5px 8px;border:1px solid var(--border2);min-width:70px">'+p+'</th>';
  });
  h+='</tr></thead><tbody>';
  PERMISSION_DEFS.forEach(function(def){
    var allowed=rights[def.key]||['admin'];
    h+='<tr><td style="padding:5px 10px;border:1px solid var(--border2);font-weight:600">'+def.label+'<br><span style="font-size:9px;color:var(--tx3)">'+def.tab+'</span></td>';
    /* Admin always has access */
    h+='<td style="text-align:center;padding:5px;border:1px solid var(--border2)"><span style="color:#15803d;font-size:14px">&#10003;</span></td>';
    Object.keys(profiles).forEach(function(p){
      var has=allowed.indexOf(p)>=0;
      h+='<td style="text-align:center;padding:5px;border:1px solid var(--border2)">';
      h+='<input type="checkbox" '+(has?'checked':'')+' onchange="toggleRight(\''+def.key+'\',\''+p+'\',this.checked)" style="width:16px;height:16px;cursor:pointer">';
      h+='</td>';
    });
    h+='</tr>';
  });
...
function openTutorial(){
  ...
  var sections=[
    ...
    {icon:'🔒',title:'Access Control',text:'Login with admin/manager/viewer password. For granular access, use <strong>Profile login</strong>: create profiles (RH, Safety, etc.) in Permissions and assign rights per action. Active profiles are shown in the header.'},
...
function renderCCByResponsible(page){
  ...
  S.wbs.forEach(function(w){
    ...
    var budM=bud(w,page);
    var actM=reelHMFromAssign(w.id,page);
    var budR=0; for(var m=page+1;m<NM;m++) budR+=bud(w,m);
    var depP=etcHMFromAssign(w.id,page);
