...
function toggleCCNeutral(wid){
  ...
  renderCCPage(S.ccM); /* render immediately with new state */
  saveS();             /* save after render */
}
...
function ccGrandTotalRow(page){
  ...
  for(var m=0;m<NM;m++){
    ...
    S.wbs.forEach(function(w){if(S.ccNeutral&&S.ccNeutral[w.id])return;mB+=budTotal(w,m);mC+=m<=S.ccM?reel(w,m):rad(w,m);mEUR_B+=getBudEur(w,m);mEUR_C+=m<=S.ccM?getReelEur(w,m):Math.round((m<=S.ccM?reel(w,m):rad(w,m))*26*8*(w.pu||0));});
