...
function makeWBS(id,name,code,parentId,companyId,spec,pu){
  ...
  return{id:id,name:name,code:code||'',parentId:parentId,companyId:companyId||'',spec:spec||'',pu:pu||0,supervisorId:'',eurOnly:false,data:d,dataEur:{},subRows:[],contractRef:'',contractAmt:0,responsible:''};
}
...
function buildDefault(){
  ...
  return{
    ...
    settings:{startYear:2025,startMonth:0,endYear:2025,endMonth:11},asLocked:true,wbsResponsibles:[],workerCategories:[]
  };
}
...
function renderCCPage(page){
  ...
  banner+='<span style="font-size:11px;color:var(--tx3);margin-left:auto">Actual HM = workers assigned · ETC (future months) is editable</span>';
  banner+='</div>';
...
function wbsCCRows(w,lv,page,isLast){
  ...
  for(var m=0;m<NM;m++){
    ...
    if(cv.type==='past'){
      ...
      /* Future month ETC = editable (RAD manuel) */
