...
document.addEventListener('DOMContentLoaded', function(){
  ...
  setTimeout(function(){
    if(!_userRole && !_activeProfiles.length){
      openLoginModal();
    }
  }, 1200); /* Wait for Firebase + S to load */
...
function openLoginModal(){
  ...
  h+='<button id="ltProfile" class="btn" style="flex:1" onclick="switchLoginType(this.dataset.t)" data-t="profile"'+(profiles.length?'':' disabled')+'>&#128100; Profile</button>';
  h+='</div>';
  h+='<div id="loginAdmin">';
...
function switchLoginType(type){
  document.getElementById('loginAdmin').style.display=type==='admin'?'':'none';
  document.getElementById('loginProfile').style.display=type==='profile'?'':'none';
  document.getElementById('ltAdmin').className='btn'+(type==='admin'?' pri':'');
  document.getElementById('ltProfile').className='btn'+(type==='profile'?' pri':'');
  setTimeout(function(){
...
function tryLoginUnified(){
  ...
  if(isAdmin){
    ...
    var perms=S.permissions||{};
    if(!perms.profiles||!perms.profiles[profile]||perms.profiles[profile].pwd!==ppwd){
