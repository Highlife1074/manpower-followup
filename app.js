...
function ccGrpRow(g,allW,page){
  ...
  for(var m=0;m<NM;m++){
    ...
    h+='<td class="num" style="border-right:1px solid var(--border)">'+(mB?mB.toFixed(1):'')+'</td>';
    h+='<td class="num" style="border-right:1px solid var(--border);font-size:9px"></td>';
    h+='<td class="num" style="border-right:2px solid var(--border2);font-weight:700;color:'+vC+'">'+(mB>0||mC>0?(mVar>0?'+':'')+mVar.toFixed(1):'')+'</td>';
  }
  h+='</tr>';
  return h;
}
...
function ccSgRow(sg,sgW,page){
  ...
  for(var m=0;m<NM;m++){
    ...
    h+='<td class="num" style="border-right:1px solid var(--border)">'+(mB?mB.toFixed(1):'')+'</td>';
    h+='<td class="num" style="border-right:1px solid var(--border);font-size:9px"></td>';
    h+='<td class="num" style="border-right:2px solid var(--border2);color:'+vC+'">'+(mB>0||mC>0?(mVar>0?'+':'')+mVar.toFixed(1):'')+'</td>';
  }
  h+='</tr>';
  return h;
}
...
function wbsCCRows(w,lv,page,isLast){
  ...
  (w.subcontracts||[]).forEach(function(sc){
    ...
    for(var m=0;m<NM;m++){
      ...
      h+='<td colspan="3" style="text-align:right;padding:3px 6px;font-family:var(--mono);font-size:10px;background:#eff6ff;border-right:2px solid var(--border2);color:var(--blue);'+(!_scLk&&!_scPast?'cursor:pointer':'')+'"';
  ...
  (S.contracts||[]).filter(function(ct){return ct.wbsId===wid;}).forEach(function(ct){
    ...
    for(var m=0;m<NM;m++){
      ...
      h+='<td colspan="3" style="text-align:right;padding:3px 6px;font-family:var(--mono);font-size:10px;background:#faf5ff;border-right:2px solid var(--border2);color:'+(_cIsReal?'#15803d':'var(--tx3)')+(_cDisp&&m>S.ccM?';font-style:italic':'')+(!_ccLkM&&!_isPast?';cursor:pointer':'')+'"';
...
function ccGrandTotalRow(page){
  ...
  for(var m=0;m<NM;m++){
    ...
    h+='<td class="num" style="border-right:1px solid #1a3a9e">'+( mB?mB.toFixed(1):'')+'</td>';
    h+='<td class="num" style="border-right:1px solid #1a3a9e;font-size:9px">'+(mEUR_B?fkv(mEUR_B):'')+'</td>';
    h+='<td class="num" style="border-right:2px solid #1a3a9e;color:'+vC+'">'+( mB>0||mC>0?(mVar>0?'+':'')+mVar.toFixed(1):'')+'</td>';
  }
  h+='</tr>';
  return h;
}
