...
function wbsCCRows(w,lv,page){
  ...
  if(hasSubRows){
    ...
    for(var m=0;m<NM;m++){
      ...
      var _lk=isCCMonthLocked(m); var editable=(m>S.ccM&&!_lk); /* past+CC month no longer editable — driven by assignments */
...
function openPositionDialog(){
  ...
  h+='<div style="display:flex;gap:8px"><button class="btn pri" onclick="confirmOpenPositions('+JSON.stringify(pairs)+')">Set</button>';
  h+='<button class="btn" onclick="clearOpenPositions('+JSON.stringify(pairs)+');closeModal()">Clear all</button>';
