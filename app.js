...
function asWBSRow(wbs, ind, startCol, page, nCols){
  ...
  for(var m = startCol; m < NM; m++){
    ...
    if(budHM > 0 || _supObjCell || _hasFm){
      ...
      countHtml += '<span style="color:' + (_totalW >= need ? 'var(--green)' : (_totalW>0?'#c2410c':'var(--red)')) + ';">' + nW + (_openP>0?'<span style="color:var(--tx3);font-size:9px">+'+_openP+'◯</span>':'') + '</span>';
