...
function ccGrandTotalRow(page){
  ...
  for(var m=0;m<NM;m++){
    ...
    h+='<td style="text-align:right;padding:4px 4px;vertical-align:middle;'+bg+bl+'border-right:1px solid #1a3a9e">';
...
function renderGroupDetail(page){
  ...
  function grpStats(wbsList){
    ...
    wbsList.forEach(function(w){
      if(S.ccNeutral&&S.ccNeutral[w.id]) return; /* skip neutralized */
      var _wBt=addBudget(w); if(w.subRows)w.subRows.forEach(function(sr){_wBt+=addSubRowBudget(sr);});
      budMois+=budTotal(w,page);
      actMois+=reel(w,page); /* actual this month from CC */
      /* budRestant = remaining budget after current month */
      for(var m=page+1;m<NM;m++) budRestant+=budTotal(w,m);
      /* depPrevue = ETC using rad overrides */
      for(var m=page+1;m<NM;m++)
        depPrevue+=ccSlot(m).rad[w.id]!==undefined?+ccSlot(m).rad[w.id]:reelHMFromAssign(w.id,m);
    });
    return{
      budMois:+budMois.toFixed(1),
