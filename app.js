...
/* ============================================================
   ORG CHART — shows ALL workers for CC month
   ============================================================ */
function renderOrgChartPage(){
  var el=document.getElementById('pageContent');
  var m=S.ccM; /* current CC month */
  var supMap={}; /* supName → {foremen:{fmName→{wbs:[],workers:[]}}} */
  var usedWorkerIds={};
  
  S.wbs.forEach(function(w){
    if(S.ccNeutral&&S.ccNeutral[w.id]) return;
    /* Get supervisor for this WBS at CC month */
    var supId=getCCSupervisor(w.id,m);
    var supWk=supId?(S.supervisors||[]).find(function(x){return x.id===supId;})||null:null;
    var supName=supWk?supWk.name:'No Supervisor';
    if(supWk) usedWorkerIds[supWk.id]=true;
    if(!supMap[supName]) supMap[supName]={foremen:{}};
    
    /* Get workers assigned to this WBS for CC month */
    var fmList=[], wkList=[];
    S.assignments.forEach(function(a){
      if(a.wbsId!==w.id) return;
      var sl=getAssignmentSlot(a.workerId,m);
      if(!sl||sl.wbsId!==w.id) return; /* only CC month assignments */
      var wk=S.workers.find(function(x){return x.id===a.workerId;});
      if(!wk) return;
      usedWorkerIds[wk.id]=true;
      var lastM=-1;for(var i=m;i<NM;i++){var s2=getAssignmentSlot(wk.id,i);if(s2&&s2.wbsId===w.id)lastM=i;else break;}
      var info={id:wk.id,name:wk.name,spec:wk.spec||'',end:lastM>=0&&ML[lastM]?ML[lastM].short:'—'};
      if((wk.role||'')==='foreman') fmList.push(info);
      else wkList.push(info);
    });
    
    if(!fmList.length) fmList.push({id:'_none',name:'No Foreman',spec:'',end:''});
    
    fmList.forEach(function(fm){
      var fk=fm.name;
      if(!supMap[supName].foremen[fk]) supMap[supName].foremen[fk]={name:fm.name,spec:fm.spec,wbs:[],workers:[]};
      supMap[supName].foremen[fk].wbs.push({name:w.name,code:w.code||'',spec:w.spec||''});
      wkList.forEach(function(wk){
        if(!supMap[supName].foremen[fk].workers.some(function(x){return x.id===wk.id;}))
          supMap[supName].foremen[fk].workers.push(wk);
      });
    });
  });
  ...
  var h='<div style="padding:16px;overflow-x:auto">';
  h+='<h2 style="font-size:16px;font-weight:700;margin:0 0 4px;color:var(--tx)">&#127970; Organization Chart</h2>';
  h+='<p style="font-size:11px;color:var(--tx3);margin:0 0 20px">CC Month: <strong>'+(ML[m]?ML[m].label:'—')+'</strong> — '+S.workers.length+' workers total, '+unassigned.length+' unassigned</p>';
  
  Object.keys(supMap).sort().forEach(function(supName){
    var sup=supMap[supName];
    h+='<div style="margin-bottom:32px">';
    h+='<div style="text-align:center;margin-bottom:6px"><div style="display:inline-block;background:#1e3a5f;color:#fff;padding:10px 28px;border-radius:12px;font-weight:700;font-size:14px;box-shadow:0 2px 8px rgba(0,0,0,.15)">&#128119; '+supName+'</div></div>';
    var fks=Object.keys(sup.foremen).sort();
    h+='<div style="display:flex;justify-content:center;gap:28px;flex-wrap:wrap;padding-top:4px">';
    fks.forEach(function(fk){
      var fm=sup.foremen[fk];
      h+='<div style="display:flex;flex-direction:column;align-items:center;min-width:180px;max-width:220px">';
      h+='<div style="width:2px;height:20px;background:#94a3b8"></div>';
      h+='<div style="width:0;height:0;border-left:6px solid transparent;border-right:6px solid transparent;border-top:8px solid #94a3b8;margin-bottom:6px"></div>';
      fm.wbs.forEach(function(wb){
        var c=specColor(wb.spec);
        h+='<div style="font-size:9px;padding:2px 8px;background:'+c+'18;color:'+c+';border:1px solid '+c+'44;border-radius:4px;margin-bottom:2px;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="'+wb.name+'">'+(wb.code?wb.code+' — ':'')+wb.name+'</div>';
      });
      var fmCol=fm.spec?specColor(fm.spec):'#f59e0b';
      h+='<div style="background:'+fmCol+';color:#fff;padding:7px 18px;border-radius:8px;font-weight:700;font-size:12px;margin:6px 0;box-shadow:0 1px 4px rgba(0,0,0,.12)">&#128736; '+fm.name+'</div>';
      if(fm.workers.length){
        h+='<div style="border-left:2px solid #e2e8f0;padding-left:10px;margin-top:2px;width:100%">';
        fm.workers.forEach(function(wk){
          var c2=specColor(wk.spec);
          h+='<div style="display:flex;align-items:center;gap:6px;padding:3px 0;font-size:11px">';
          h+='<span style="width:8px;height:8px;border-radius:50%;background:'+c2+';flex-shrink:0"></span>';
          h+='<span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+wk.name+'</span>';
          h+='<span style="font-size:9px;color:var(--tx3);white-space:nowrap">→ '+wk.end+'</span>';
          h+='</div>';
        });
        h+='</div>';
      } else { h+='<div style="font-size:10px;color:var(--tx3);font-style:italic;margin-top:4px">No workers</div>'; }
      h+='</div>';
    });
    h+='</div></div>';
  });
  ...
  if(unassigned.length){
    h+='<div style="margin-top:24px;padding:16px;background:#fef3c7;border:1px solid #fcd34d;border-radius:10px">';
    h+='<h3 style="font-size:13px;font-weight:700;color:#92400e;margin:0 0 10px">&#9888; Unassigned Workers ('+unassigned.length+')</h3>';
    h+='<div style="display:flex;flex-wrap:wrap;gap:8px">';
    unassigned.forEach(function(wk){
      var c3=specColor(wk.spec||'');
      h+='<div style="display:flex;align-items:center;gap:5px;padding:4px 10px;background:#fff;border:1px solid #e2e8f0;border-radius:6px;font-size:11px">';
      h+='<span style="width:8px;height:8px;border-radius:50%;background:'+c3+'"></span>';
      h+=wk.name;
      h+='</div>';
    });
    h+='</div></div>';
