...
function wbsCCRows(w,lv,page,isLast){
  ...
  (S.contracts||[]).filter(function(ct){return ct.wbsId===wid;}).forEach(function(ct){
    ...
    for(var m=0;m<NM;m++){
      ...
      var _ck=mkey(m);
      var _cAct=m<=S.ccM?+(ct.billing[_ck]||0):null;
      var _cFcast=+(ct.adjustedForecast||ct.monthlyForecast);
      var _cDisp=_cAct!==null?_cAct:_cFcast;
      var _ccls2=m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td';
      var _cIsReal=_cAct!==null&&ct.billing[_ck]!==undefined;
      var _cSty='color:'+(_cIsReal?'var(--green)':'var(--tx3)')+';font-size:10px;';
      if(m<=S.ccM&&!_isCCLk){
        h+='<td class="'+_ccls2+' edt num" style="'+_cSty+';cursor:pointer" onclick="openContractCellEdit(this)" data-cid="'+ct.id+'" data-m="'+m+'" data-v="'+(_cAct||0)+'">'+(_cDisp?_cDisp.toLocaleString():'')+'</td>';
      } else {
        h+='<td class="'+_ccls2+' num" style="'+_cSty+'">'+(_cDisp?_cDisp.toLocaleString():'')+'</td>';
      }
    }
    h+='</tr>';
  });
...
function renderWorkersPage(){
  ...
  if(hasRight('add_worker')||_userRole==='admin') html+='<button class="btn pri" style="margin-left:auto" onclick="openWorkerModal(null)">&#10133; Add Worker</button>';
  html+='</div>';
  /* Table */
...
function renderAssignmentsPage(){
  ...
  for(var gi = 0; gi < S.groups.length; gi++){
    ...
    html = html.slice(0, html.lastIndexOf('<tr style="background:#dbeafe'));
...
var PERMISSION_DEFS = [
  ...
  {key:'paper_check',     label:'Paper Check column',          tab:'Workers'},
  {key:'induction',       label:'Induction column',            tab:'Workers'},
  {key:'edit_budget',     label:'Edit Initial Budget',         tab:'Budget'},
  {key:'lock_cc',         label:'Lock / Unlock CC months',     tab:'CC'},
  {key:'cc_actual',       label:'Enter CC actual values',      tab:'CC'},
  {key:'edit_wbs_resp',   label:'Set WBS Responsible',         tab:'CC/Budget'},
  ...
  {key:'settings',        label:'Access Settings',             tab:'Admin'},
];
...
function openPermissionsModal(){
  ...
  h+='<thead><tr style="background:var(--bg2)">';
  h+='<th style="text-align:left;padding:5px 10px;border:1px solid var(--border2);min-width:200px">Permission</th>';
  h+='<th style="padding:5px 8px;border:1px solid var(--border2);min-width:60px">Admin</th>';
  Object.keys(profiles).forEach(function(p){
