      ...
      unsubscribe = db.doc(DOC_PATH).onSnapshot(function(snap){
        ...
        if(snap.exists){
          ...
          if(migrated){
            ...
            var remoteHash=JSON.stringify(migrated).length;
            var localHash=JSON.stringify(S).length;
            S = migrated;
            applySettings();
            setSyncStatus('ok','Live');
            /* Only re-render if data actually changed (other user's update) */
            if(remoteHash!==localHash) render();
            else setSyncStatus('ok','Live');
          }
        } else {
...
function wbsCCRows(w,lv,page){
  ...
  /* EAC = sum of actual (past+CC) + rad/ETC for future months (respects manual overrides) */
  var eac=+rc2.toFixed(1);
  for(var _mf=page+1;_mf<NM;_mf++){
    eac+=ccSlot(_mf).rad[w.id]!==undefined?+ccSlot(_mf).rad[w.id]:reelHMFromAssign(w.id,_mf);
  }
  eac=+eac.toFixed(1);
  /* Var EAC = Var EOS = Total Budget - EAC (sum of all monthly variances) */
  var varEAC=+(bt-eac).toFixed(1);
  var varEOS=varEAC;
  var wid=w.id;
  ...
  for(var m=0;m<NM;m++){
    ...
    h+='<div style="font-size:10px;color:var(--blue)">'+(bv?bv.toFixed(1):'')+'</div>';
  ...
  if(w.subRows&&w.subRows.length){
    w.subRows.forEach(function(sr){
      ...
      var srSl=specLabel(sr.spec);
      var srPu=+(sr.pu||w.pu||0);
      /* Budget M and Cum Budget for this subRow */
      var srBudM=subRowBudAtSlot(sr,page);
      var srCumBud=0; for(var _m=0;_m<=page;_m++) srCumBud+=subRowBudAtSlot(sr,_m);
      /* subRows share actual HM with parent WBS — no separate actual */
      /* EUR budget only */
      var srBudEurM=Math.round(srBudM*26*8*srPu);
      var srBudEurTot=Math.round(srBt*26*8*srPu);

      h+='<tr class="rre '+alt+'">';
      /* p1: indent label */
      h+='<td class="p1" style="padding-left:'+(ind+16)+'px;border-right:1px solid var(--border2);font-size:10px;color:var(--tx2)">';
      h+='<span style="font-size:9px;color:var(--tx3)">+ Specialty</span>';
      h+='</td>';
      /* p2 company */
      ...
      /* Cost M — shared with parent, show empty */
      ...
      /* Cum Cost / Cum Var / EAC / Var EAC / Var EOS — empty for subrows */
      h+='<td style="border-right:1px solid var(--border2)"></td>';
      h+='<td style="border-right:'+SEP+'"></td>';
      h+='<td style="border-right:1px solid var(--border2)"></td>';
      h+='<td style="border-right:1px solid var(--border2)"></td>';
      ...
      h+='<div style="font-size:8px;color:var(--tx3);text-align:right;line-height:2"><div>Bud</div><div>—</div><div></div></div>';
      h+='</td>';
      /* Monthly: budget HM + EUR only */
      for(var m=0;m<NM;m++){
        var srBv=subRowBudAtSlot(sr,m);
        var srBEur=Math.round(srBv*26*8*srPu);
        var cls=m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td';
        var bl=m===page?'border-left:2px solid var(--pur);':'';
        h+='<td class="'+cls+' num" style="'+bl+'border-right:1px solid var(--border);vertical-align:top;padding:2px 4px">';
        h+='<div style="font-size:10px;color:var(--blue)">'+(srBv?srBv.toFixed(1):'')+'</div>';
        h+='<div style="font-size:10px;color:var(--tx3)">—</div>';
        h+='<div></div>';
        h+='</td>';
        h+='<td class="'+cls+' num" style="border-right:'+SEP+';vertical-align:top;padding:2px 4px">';
        h+='<div style="font-size:9px;color:#1d4ed8">'+(srBEur?fkv(srBEur):'')+'</div>';
        h+='<div style="font-size:9px;color:var(--tx3)">—</div>';
        h+='<div></div>';
        h+='</td>';
      }
      h+='</tr>';
...
function ccGrandTotalRow(page){
  ...
  var tVarEAC=+(tB-tEAC).toFixed(1); /* Budget Total - EAC */
