...
function reelAtSlot(w,i){
  ...
  /* Manual override takes priority; otherwise use assignments (1 worker = 1 HM) */
  var manual=ccSlot(i).reel[w.id];
  if(manual!==undefined) return+manual;
  return reelHMFromAssign(w.id,i);
}
function radAtSlot(w,i){
  /* Manual override; otherwise use assignments on future months */
  var manual=ccSlot(i).rad[w.id];
  if(manual!==undefined) return+manual;
  var assignHM=reelHMFromAssign(w.id,i);
  return assignHM>0?assignHM:wbsBudAtSlot(w,i);
}
...
function buildDefault(){
  ...
  return{
    ...
    settings:{startYear:2025,startMonth:0,endYear:2025,endMonth:11},asLocked:true,workerCategories:[]
  };
}
...
function renderCCPage(page){
  ...
  banner+='<span style="font-size:11px;color:var(--tx3);margin-left:auto"><em>Italic</em> = from assignments · <strong>Bold</strong> = manual override · Click to edit</span>';
  banner+='</div>';
...
function wbsCCRows(w,lv,page,isLast){
  ...
  h+='<tr class="rre '+wbsCls+'"><td class="p1" style="padding-left:'+(ind+12)+'px;border-right:2px solid var(--border2)"><span class="tp tpr" title="Values from assignments (italic) or manual entry (bold). Click to override.">✏ Actual/ETC</span><span style="font-size:8px;color:var(--tx3);margin-left:4px;font-style:italic">assign</span></td>';
  h+='<td class="p2" style="border-right:1px solid var(--border2)"></td><td class="p3" style="border-right:1px solid var(--border2)"></td><td class="p4" style="border-right:2px solid var(--border2)"></td>';
  ...
  for(var m=0;m<NM;m++){
    ...
    var wid=w.id;
    var hasManual=(m<=S.ccM)?(ccSlot(m).reel[w.id]!==undefined):(ccSlot(m).rad[w.id]!==undefined);
    if(cv.type==='past'){
      var pastStyle=hasManual?'color:var(--tx2)':'color:var(--tx2);font-style:italic';
      h+='<td class="'+cls+' num" style="'+pastStyle+'">'+f1(cv.v)+'</td>';
    } else if(cv.type==='cur'){
      var dv=cv.v?cv.v.toFixed(1):'0';
      var curStyle=hasManual?'color:var(--pur);font-weight:800':'color:var(--pur);font-weight:400;font-style:italic';
      h+='<td class="'+cls+' edt num" style="'+curStyle+'" data-wid="'+wid+'" data-key="r'+m+'" data-type="cc-reel" data-m="'+m+'" onclick="ecCC(this,\''+wid+'\','+m+',\'reel\','+cv.v+')">'+dv+'</td>';
    } else {
      var dv2=cv.v?cv.v.toFixed(1):'';
      var radStyle=hasManual?'color:var(--ora);font-weight:700':'color:var(--ora);font-style:italic';
      h+='<td class="'+cls+' edt num" style="'+radStyle+'" data-wid="'+wid+'" data-key="r'+m+'" data-type="cc-rad" data-m="'+m+'" onclick="ecCC(this,\''+wid+'\','+m+',\'rad\','+cv.v+')">'+dv2+'</td>';
    }
  }
  h+='</tr>';
  /* Variance */
...
function renderGroupDetail(page){
  ...
  h+='<div class="bot-title" style="cursor:pointer;display:flex;align-items:center;justify-content:space-between" onclick="_botCollapsed.grp=!(_botCollapsed.grp);render()">';
  h+='<span>&#128202; Cost Control by Group — '+(ML[page]?ML[page].label:(MS[page]+' '+YEAR))+'</span>';
  h+='<span style="font-size:14px;color:var(--tx3)">'+(collapsed?'▶':'▼')+'</span>';
  h+='</div>';
  if(collapsed){h+='</div>';return h;}

  h+='<table class="bot-table" style="width:100%">';
  h+='<thead><tr>';
  h+='<th>Group / Subgroup</th>';
  h+='<th>Budget HM</th><th>Actual HM</th><th>ETC HM</th><th>EAC</th><th>Variance (month)</th><th>Variance (final)</th>';
  h+='</tr></thead><tbody>';

  var totB=0,totRC=0,totETC=0,totBcum=0;

  for(var gi=0;gi<S.groups.length;gi++){
    var g=S.groups[gi];
    var allWBS=S.wbs.filter(function(w){return w.parentId===g.id;});
    /* Also include subgroup WBS */
    ...
    /* Group totals */
    var gB=0,gRC=0,gETC=0,gBcum=0;
    allWBS.forEach(function(w){
      gB+=addBudget(w);
      gBcum+=budgetCumAtMonth(w,page);
      for(var m=0;m<=page;m++) gRC+=reelHMFromAssign(w.id,m);
      gETC+=etcHMFromAssign(w.id,page);
    });
    gB=+gB.toFixed(1);gRC=+gRC.toFixed(1);gETC=+gETC.toFixed(1);gBcum=+gBcum.toFixed(1);
    var gEAC=+(gRC+gETC).toFixed(1);
    var gVm=+(gBcum-gRC).toFixed(1);
    var gVf=+(gB-gEAC).toFixed(1);
    totB+=gB;totRC+=gRC;totETC+=gETC;totBcum+=gBcum;

    h+='<tr style="background:rgba(240,90,0,.07)">';
    h+='<td style="font-weight:800;font-size:12px;color:var(--ora)">'+g.name+'</td>';
    h+='<td>'+gB.toFixed(1)+'</td>';
    h+='<td style="color:var(--green)">'+gRC.toFixed(1)+'</td>';
    h+='<td style="color:var(--ora)">'+gETC.toFixed(1)+'</td>';
    h+='<td style="color:var(--yel)">'+gEAC.toFixed(1)+'</td>';
    h+='<td class="'+pnc(gVm)+'" style="font-weight:700">'+(gVm>0?'+':'')+gVm.toFixed(1)+'</td>';
    h+='<td class="'+pnc(gVf)+'" style="font-weight:700">'+(gVf>0?'+':'')+gVf.toFixed(1)+'</td>';
    h+='</tr>';
    ...
    for(var si=0;si<g.sub.length;si++){
      ...
      var sB=0,sRC=0,sETC=0,sBcum=0;
      sgWBS.forEach(function(w){
        sB+=addBudget(w);sBcum+=budgetCumAtMonth(w,page);
        for(var m=0;m<=page;m++) sRC+=reelHMFromAssign(w.id,m);
        sETC+=etcHMFromAssign(w.id,page);
      });
      sB=+sB.toFixed(1);sRC=+sRC.toFixed(1);sETC=+sETC.toFixed(1);sBcum=+sBcum.toFixed(1);
      var sEAC=+(sRC+sETC).toFixed(1);
      var sVm=+(sBcum-sRC).toFixed(1);
      var sVf=+(sB-sEAC).toFixed(1);

      h+='<tr>';
      h+='<td style="padding-left:22px;font-size:11px;color:#4a3a80;font-weight:700">◆ '+sg.name+'</td>';
      h+='<td>'+sB.toFixed(1)+'</td>';
      h+='<td style="color:var(--green)">'+sRC.toFixed(1)+'</td>';
      h+='<td style="color:var(--ora)">'+sETC.toFixed(1)+'</td>';
      h+='<td style="color:var(--yel)">'+sEAC.toFixed(1)+'</td>';
      h+='<td class="'+pnc(sVm)+'">'+(sVm>0?'+':'')+sVm.toFixed(1)+'</td>';
      h+='<td class="'+pnc(sVf)+'">'+(sVf>0?'+':'')+sVf.toFixed(1)+'</td>';
      h+='</tr>';
    }
  }

  /* Total row */
  var tEAC=+(totRC+totETC).toFixed(1);
  var tVm=+(totBcum-totRC).toFixed(1);
  var tVf=+(totB-tEAC).toFixed(1);
  h+='<tr style="background:#F05A00;color:#1a1a1a;font-weight:800">';
  h+='<td>PROJECT TOTAL</td>';
  h+='<td>'+totB.toFixed(1)+'</td>';
  h+='<td>'+totRC.toFixed(1)+'</td>';
  h+='<td>'+totETC.toFixed(1)+'</td>';
  h+='<td>'+tEAC.toFixed(1)+'</td>';
  h+='<td class="'+pnc(tVm)+'">'+(tVm>0?'+':'')+tVm.toFixed(1)+'</td>';
  h+='<td class="'+pnc(tVf)+'">'+(tVf>0?'+':'')+tVf.toFixed(1)+'</td>';
  h+='</tr>';
  ...
  function buildModal(filterText){
    ...
    for(var i=0;i<workers.length;i++){
      ...
      if(outOfContract){
        ...
        clickable=true; /* allow reassignment */
      }
  ...
  function buildPicker(ft){
    ...
    function workerStatus(wk){
      ...
      pairs.forEach(function(p){
        ...
        if(!active){freePairs.push(p);return;} /* treat out-of-contract as assignable */
        var asgn=getAssignmentSlot(wk.id,p.month);
...
function syncAssignmentsToCC(wbsId,slotIdx){
  ...
  if(slotIdx<=S.ccM){
    /* Actual month: set reel = number of workers (as HM proxy) only if 0 */
    if(!slot.reel[wbsId]&&nW>0) slot.reel[wbsId]=+(nW).toFixed(1);
  } else {
    /* Future: update RAD = workers × 1 HM each */
    if(nW>0) slot.rad[wbsId]=+(nW).toFixed(1);
    else if(slot.rad[wbsId]&&slot.rad[wbsId]===+(nW).toFixed(1)) delete slot.rad[wbsId];
  }
}
...
function renderWorkersPage(){
  ...
  for(var wi=0;wi<sorted.length;wi++){
    ...
    html+='<td style="font-size:10px;color:var(--tx2);cursor:pointer;white-space:nowrap" onclick="wkEditField(event,\''+wk.id+'\',\'responsible\')">'+( wk.responsible||'<span style="color:var(--border2)">—</span>')+'</td>';
    ...
    for(var m=0;m<NM;m++){
      ...
      if(asgn2){
        ...
        cellContent='<span style="font-size:9px;color:var(--border2);cursor:pointer" onclick="wkCellClick(event,\''+wk.id+'\','+m+')" title="Assign to WBS">+</span>';
      } else {
...
function renderAssignmentsPage(){
  ...
  /* Always start from CC month — no manual page selection */
  var page=S.ccM>=0?S.ccM:0;
  if(page>=NM) page=NM-1;
  window._asPage=page;

  /* Sub-tabs: Assignments / Overview */
  ...
  /* No month tabs — view always starts from ccM */

  /* ── Header bar: propagation info + assign button ── */
  var slotLabel=ML[page]?ML[page].label:(MS[page]+' '+YEAR);
  var isPast=false; /* always start from CC month, never past */
  var isCur=true;
  var isFut=false;
  var prevLabel=page>0?(ML[page-1]?ML[page-1].short:MS[page-1]):'';
  var hasAssign=monthIsConfirmed(page);

  var totalSel=Object.keys(window._asSel||{}).reduce(function(n,wid){
    return n+Object.keys(window._asSel[wid]||{}).filter(function(k){return window._asSel[wid][k];}).length;
  },0);
  ...
  if(isPast) html+='<span style="font-size:11px;color:var(--tx3);background:var(--bg2);padding:2px 8px;border-radius:10px">Past — read only</span>';
  else if(isCur) html+='<span style="font-size:11px;color:var(--pur);font-weight:700">● CC Month</span>';
  else html+='<span style="font-size:11px;color:var(--bm);font-weight:600">◌ Future</span>';
  if(!isPast&&page>0&&!hasAssign) html+='<span style="font-size:10px;color:var(--green);background:var(--gl);padding:2px 8px;border-radius:10px">↓ Pre-filled from '+prevLabel+'</span>';

  if(!isPast){
    /* Always render these with ids so asUpdateTopBar() can show/hide without re-render */
    ...
    /* Reset/Clear this CC month */
    if(page>0){
      html+='<button class="btn soft" style="font-size:10px" onclick="asResetFromPrev('+page+')" title="Re-copy assignments from previous month">&#8635; Reset from '+prevLabel+'</button>';
      html+='<button class="btn" style="font-size:10px;color:var(--red);border-color:var(--red)" onclick="asClearMonth('+page+')">&#128465; Clear CC month</button>';
    }
  }
  html+='</div>';

  /* ── The grid: for the selected month, show from page to end ──
     Columns: WBS | Budget | [page] | [page+1] | ... | [NM-1]
     The page column is highlighted as "current assignment month" */
  var startCol=page; /* from CC month to end of project */

  window._cellMap={}; window._cellMapIdx=0; /* reset before rebuilding grid */
  html+='<div class="as-grid" style="flex:1;overflow:auto;min-height:0">';
  html+='<table class="as-table"><thead><tr>';
  html+='<th class="ap1" style="position:sticky;left:0;z-index:25;background:var(--bg2)">WBS</th>';
  html+='<th class="ap2" style="position:sticky;left:200px;z-index:25;background:var(--bg2);min-width:60px">Budget</th>';
  for(var m=startCol;m<NM;m++){
    var ml2=ML[m]?ML[m].short:MS[m];
    var isCur2=(m===page);
    var thBg=isCur2?'rgba(61,90,128,.12)':m<S.ccM?'var(--bg2)':'transparent';
    var thCol=isCur2?'var(--blue)':m<S.ccM?'var(--tx3)':'var(--tx2)';
    html+='<th style="min-width:80px;text-align:center;background:'+thBg+';color:'+thCol+';cursor:pointer;border-left:'+(isCur2?'2px solid var(--blue)':'')+'" onclick="asSelectCol('+m+')">';
    html+=ml2;
    if(m===S.ccM) html+='<br><span style="font-size:8px;color:var(--pur)">CC</span>';
    else if(m<S.ccM) html+='<br><span style="font-size:8px;color:var(--tx3)">past</span>';
    html+='</th>';
  }
  html+='</tr></thead><tbody>';
  ...
  for(var gi=0;gi<S.groups.length;gi++){
    var g=S.groups[gi];
    if(gi>0) html+='<tr class="group-sep-row"><td colspan="'+(2+(NM-startCol))+'"></td></tr>';
    html+='<tr class="rgroup"><td class="ap1" colspan="'+(2+(NM-startCol))+'" style="position:sticky;left:0">';
    html+='<span class="tog" onclick="toggleGroup(\''+g.id+'\')"><span>'+(g.col?'▶':'▼')+'</span></span> ';
    html+='<span style="font-weight:800;font-size:12px">'+g.name.toUpperCase()+'</span></td></tr>';
    if(g.col) continue;
    S.wbs.filter(function(w){return w.parentId===g.id;}).forEach(function(wbs){html+=asWBSRow(wbs,14,startCol,page);});
    for(var si=0;si<g.sub.length;si++){
      var sg=g.sub[si];
      var sgWBS=S.wbs.filter(function(w){return w.parentId===sg.id;});
      if(!sgWBS.length) continue;
      html+='<tr class="sg-sep-row"><td colspan="'+(2+(NM-startCol))+'"></td></tr>';
      html+='<tr class="rsg"><td class="ap1" colspan="'+(2+(NM-startCol))+'" style="position:sticky;left:0">';
      html+='<span class="tog" onclick="toggleSG(\''+sg.id+'\')">▼</span> ';
      html+='<span style="font-size:13px;font-weight:800;color:#4a3a80">◆ '+sg.name+'</span></td></tr>';
      if(sg.col) continue;
      sgWBS.forEach(function(wbs){html+=asWBSRow(wbs,32,startCol,page);});
    }
  }
  html+='</tbody></table></div>';
  document.getElementById('pageContent').innerHTML=html;
...
function asWBSRow(wbs,ind,startCol,page){
  var budTotal=wbsTotalBudget(wbs);
  var wbsSel=window._asSel&&window._asSel[wbs.id]?window._asSel[wbs.id]:{};
  var h='<tr>';
  h+='<td class="ap1" style="position:sticky;left:0;background:var(--white);z-index:10;padding-left:'+ind+'px;font-weight:600;font-size:11px">';
  if(wbs.code) h+='<span class="codb" style="font-size:9px">'+wbs.code+'</span> ';
  h+=wbs.name+'</td>';
  h+='<td class="ap2" style="position:sticky;left:200px;background:var(--white);z-index:10;font-family:var(--mono);font-size:11px;color:var(--blue);text-align:right">'+(budTotal?budTotal.toFixed(1):'')+'</td>';
  ...
  for(var m=startCol;m<NM;m++){
    ...
    /* Compare with prev month to show delta */
    var prevWorkers=(m>startCol&&m>0)?getWBSWorkers(wbs.id,m-1):[];
    var delta=nW-prevWorkers.length;
    var deltaHtml='';
    if(m>startCol&&delta!==0){
      deltaHtml='<span style="font-size:9px;font-weight:700;color:'+(delta>0?'var(--green)':'var(--red)')+';">'+(delta>0?'+':'')+delta+'</span>';
    }
    ...
    for(var k=0;k<workers.length;k++){
      ...
      tipLines.push((isFmWk?'★ ':'')+wk2.name+(wk2.workerId?' #'+wk2.workerId:'')+(isExp?' ⚠':''));
    }
    ...
    var countHtml='';
    if(budHM>0){
      countHtml='<div class="as-count"><span class="'+(nW>=need?'n-ass':'n-miss')+'">'+nW+'</span><span class="sep">/</span><span>'+need+'</span></div>';
    } else if(nW>0){
      countHtml='<div class="as-count"><span class="n-ass">'+nW+'</span></div>';
    }
    if(deltaHtml) countHtml+=deltaHtml;

    var colBg=isCurCol?'rgba(61,90,128,.06)':isPastCol?'var(--bg2)':'';
    var borderL=isCurCol?'border-left:2px solid var(--blue);':'';
    var canClick=isPastCol?(!!(!S.asLocked)):true;
    if(!window._cellMap) window._cellMap={};
    /* Use numeric index as key — avoids ALL special char issues */
    var cellKey='k'+(window._cellMapIdx=(window._cellMapIdx||0)+1);
    window._cellMap[cellKey]={wbsId:wbs.id,m:m};
    var onclickAttr=canClick?' onclick="handleCellClick(event,\''+cellKey+'\')"':'';
    h+='<td style="padding:0;'+(colBg?'background:'+colBg+';':'')+borderL+'">';
    h+='<div class="'+cellCls+'" data-cell-key="'+cellKey+'" data-cell-wbs="'+wbs.id+'" data-cell-m="'+m+'"'+(tip?' data-tip="'+tip+'"':'')+' style="cursor:'+(canClick?'pointer':'default')+'"'+onclickAttr+'>';
    h+=countHtml;
    h+='<div class="as-names">';
    for(var k=0;k<workers.length;k++){
      var wk3=workers[k];
      h+='<span class="as-chip" data-wid="'+wk3.id+'" data-wbs="'+wbs.id+'" data-m="'+m+'" onclick="event.stopPropagation();showChipPop(event,this)" style="display:none">'+wk3.name+'</span>';
    }
    h+='</div></div></td>';
  }
  h+='</tr>';
...
function asSetPage(m){
  /* Page is now driven by ccM — this function kept for compatibility */
  window._asPage=m;
  window._asSel={};
  renderAssignmentsPage();
}
...
function asClearAll(){
  window._asSel={};
  document.querySelectorAll('.as-cell-sel').forEach(function(el){el.classList.remove('as-cell-sel');});
  asUpdateTopBar();
}
function asClearSel(wbsId){if(window._asSel)window._asSel[wbsId]={};renderAssignmentsPage();}
...
function asDirectPick(e,wbsId,m){
  ...
  divs.forEach(function(d){d.classList.toggle('as-cell-sel',!!window._asSel[wbsId][m]);});
  asUpdateTopBar();
}
...
function asUpdateTopBar(){
  ...
  var clrBtn=document.getElementById('asClrBtn');
  if(btn){
  ...
  if(clrBtn) clrBtn.style.display=total>0?'inline-flex':'none';
}
function asToggleCell(wbsId,m,el){
  if(!window._asSel) window._asSel={};
  if(!window._asSel[wbsId]) window._asSel[wbsId]={};
  window._asSel[wbsId][m]=!window._asSel[wbsId][m];
  if(el) el.classList.toggle('as-cell-sel',!!window._asSel[wbsId][m]);
  renderAssignmentsPage();
}
...
function asToggleWBS(wbsId,m){
  if(!window._asSel) window._asSel={};
  if(!window._asSel[wbsId]) window._asSel[wbsId]={};
  var was=!!window._asSel[wbsId][m];
  window._asSel[wbsId][m]=!was;
  renderAssignmentsPage();
}
...
function remAssign(el){
  var wid=el.dataset.wid; var m=parseInt(el.dataset.m);
  var rYr=ML[m]?ML[m].year:YEAR; var rMo=ML[m]!==undefined?ML[m].month:m;
  S.assignments=S.assignments.filter(function(a){return !(a.workerId===wid&&a.month===rMo&&a.year===rYr);});
  saveS(); renderAssignmentsPage();
}
function removeAssignment(workerId,month){
  var rYr=ML[month]?ML[month].year:YEAR; var rMo=ML[month]!==undefined?ML[month].month:month;
...
function showChipPop(e,el){
  ...
  var sel2=window._asSel&&wbsId2&&window._asSel[wbsId2]?window._asSel[wbsId2]:{};
  var selM2=Object.keys(sel2).filter(function(k){return sel2[k];}).map(Number);
  var removeLabel=selM2.length>1?'&#128465; Remove from '+selM2.length+' selected months':'&#128465; Remove from this month';
  var pop=document.createElement('div');
  pop.id='chipPop'; pop.className='chip-pop';
  pop.innerHTML='<div class="chip-pop-name">'+wk.name+(wk.workerId?' <span style="color:var(--tx3);font-weight:400">#'+wk.workerId+'</span>':'')+'</div>'+
    '<button class="chip-pop-btn" onclick="chipRemove(\''+wid+'\','+m+',\''+wbsId2+'\')">'+removeLabel+'</button>';
...
function chipRemove(wid,m,wbsId){
  var old=document.getElementById('chipPop');if(old)old.remove();
  var sel=window._asSel&&wbsId&&window._asSel[wbsId]?window._asSel[wbsId]:{};
  var selM=Object.keys(sel).filter(function(k){return sel[k];}).map(Number);
  if(selM.length>1){
    selM.forEach(function(slotIdx){
      var rYr=ML[slotIdx]?ML[slotIdx].year:YEAR; var rMo=ML[slotIdx]!==undefined?ML[slotIdx].month:slotIdx;
      S.assignments=S.assignments.filter(function(a){return !(a.workerId===wid&&a.month===rMo&&a.year===rYr);});
    });
    saveS(); showToast('Removed from '+selM.length+' months','ok');
  } else {
    removeAssignment(wid,m);
  }
  renderAssignmentsPage();
}

function openWorkerPickerMultiWBS(pairs){
  /* Open picker for multiple WBS — assign workers across all pairs */
  var wbsIds=[...new Set(pairs.map(function(p){return p.wbsId;}))];
  /* Use first WBS as reference for display */
  var wbsId=wbsIds[0];
  var months=[...new Set(pairs.map(function(p){return p.month;}))];
  openWorkerPickerMulti(wbsId,months,pairs);
}
