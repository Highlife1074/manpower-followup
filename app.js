...
function wbsCCRows(w,lv,page,isLast){
  ...
  (S.contracts||[]).filter(function(ct){return ct.wbsId===wid;}).forEach(function(ct){
    ...
    h+='<tr class="'+wbsCls+'"><td class="p1" colspan="7" style="padding:2px 6px 2px '+(ind+12)+'px;border-right:2px solid var(--border3);background:#faf5ff">';
    ...
    for(var m=0;m<NM;m++){
      if(!contractMonthActive(ct,m)){
        h+='<td class="'+(m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td')+'"></td>'; continue;
      }
      var _ck=mkey(m);
      var _cFcast=+(ct.adjustedForecast||ct.monthlyForecast);
      var _cIsReal=ct.billing&&ct.billing[_ck]!==undefined;
      var _cAct=_cIsReal?+(ct.billing[_ck]):null;
      /* Past/CC = actual if set, else 0. Future = per-month billing if set, else forecast */
      var _cDispFut=_cIsReal?_cAct:_cFcast;
      var _cDisp=m<=S.ccM?(_cIsReal?_cAct:0):_cDispFut;
      var _ccls2=m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td';
      var _cSty='font-size:10px;color:'+(_cIsReal?'var(--green)':m>S.ccM?'var(--tx3)':'var(--tx3)');
      /* Check if cumul at this month exceeds contract budget */
      var _cumToM=0;
      for(var _mx=0;_mx<=m;_mx++){
        if(!contractMonthActive(ct,_mx)) continue;
        var _fv=+(ct.adjustedForecast||ct.monthlyForecast);
        _cumToM+=(_mx<=S.ccM&&ct.billing&&ct.billing[mkey(_mx)]!==undefined)?+(ct.billing[mkey(_mx)]):_fv;
      }
      var _cellOver=_ctBud>0&&_cumToM>_ctBud;
      var _ccLkM=isCCMonthLocked(m);
      /* Past months = read only (part of locked previous CC) */
      var _isPast=m<S.ccM;
      if(!_ccLkM && !_isPast){
        var _overBg=_cellOver?';background:rgba(185,28,28,.08)':'';
        h+='<td class="'+_ccls2+' edt num" style="'+_cSty+_overBg+';cursor:pointer'+(m>S.ccM?';font-style:italic':'')+'" onclick="openContractCellEdit(this)" data-cid="'+ct.id+'" data-m="'+m+'" data-v="'+(_cDisp||0)+'">'+(_cDisp?fkv(_cDisp):(m>S.ccM?'<span style="color:var(--border2);font-size:9px">+</span>':''))+'</td>';
      } else {
        var _overBg2=_cellOver?';background:rgba(185,28,28,.08)':'';
        h+='<td class="'+_ccls2+' lk num" style="'+_cSty+_overBg2+'">'+(_cDisp?fkv(_cDisp):'')+'</td>';
      }
    }
    h+='</tr>';
  });
