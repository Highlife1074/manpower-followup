...
function saveS(){
  ...
  localStorage.setItem('hht8',JSON.stringify(S)); /* always local immediately */
  if(_saveTimer) clearTimeout(_saveTimer);
  _saveTimer=setTimeout(function(){ _saveTimer=null; _doSave(); },600);
}
function _doSave(){
  var json='';
  try{
...
function ccHead(page){
  ...
  for(var m=0;m<NM;m++){
    ...
    h+='<th colspan="2" style="text-align:center;'+bg+col+';border-right:'+SEP+';padding:4px 6px;font-size:10px;font-weight:700">'+lbl+(m===S.ccM?' ●':'')+'</th>';
  }
  h+='</tr>';
  /* Row 2: column names */
  ...
  for(var m=0;m<NM;m++){
    ...
    h+='<th style="min-width:50px;text-align:right;'+bg+'border-right:'+SEP+';color:#1d4ed8;font-size:9px">EUR</th>';
  }
  h+='</tr>';
  return h;
}
...
function ccBody(page){
  ...
  for(var gi=0;gi<S.groups.length;gi++){
    ...
    if(g.col){h+='<tr class="wbs-sep-row"><td colspan="'+(14+NM*2)+'"></td></tr>';continue;}
    ...
    h+='<tr style="height:4px"><td colspan="'+(14+NM*2)+'" style="background:var(--bg);border-top:3px solid #8a8070"></td></tr>';
  }
  return h;
}
...
function ccGrandTotalRow(page){
  ...
  S.wbs.forEach(function(w){tB+=addBudget(w);tBM+=bud(w,page);tRC+=reelCum(w,page);tCumBud+=budgetCumAtMonth(w,page);tCostM+=reel(w,page);
    var _wEAC=0;
    for(var _mf=page+1;_mf<NM;_mf++) _wEAC+=ccSlot(_mf).rad[w.id]!==undefined?+ccSlot(_mf).rad[w.id]:reelHMFromAssign(w.id,_mf);
    tETC+=_wEAC;
  });
  ...
  h+=_ccFixedCols(tB,tBM,tCostM,tVarM,tCumBud,tRC,tCumVar,tEAC,tVarEAC,tVarEOS,SEP,';');
  /* label col */
  h+='<td style="border-right:'+SEP+';background:rgba(255,255,255,.08)"><div style="font-size:8px;line-height:2;text-align:right;color:rgba(255,255,255,.6)"><div>Bud</div><div>Real</div><div>Var</div></div></td>';
  /* monthly */
  for(var m=0;m<NM;m++){
    var mB=0,mC=0,mEB=0,mEC=0;
    S.wbs.forEach(function(w){
      mB+=bud(w,m);mC+=m<=S.ccM?reel(w,m):rad(w,m);
      mEB+=getBudEur(w,m);
    ...
    var isCur=(m===page); var bl=isCur?'border-left:2px solid rgba(255,255,255,.4);':'';
    var bg='background:rgba(255,255,255,.06);';
    h+='<td style="text-align:right;padding:2px 4px;vertical-align:top;'+bg+bl+'border-right:1px solid #1a3a9e">';
    h+='<div style="font-size:10px;color:#93c5fd">'+(mB?mB.toFixed(1):'')+'</div>';
    h+='<div style="font-size:10px;color:#fff">'+(mC?mC.toFixed(1):'')+'</div>';
...
function ccGrpRow(g,allW,page){
  var SEP='3px solid #94a3b8';
  var tB=0,tBM=0,tRC=0,tETC=0,tCumBud=0,tCostM=0;
  allW.forEach(function(w){tB+=addBudget(w);tBM+=bud(w,page);tRC+=reelCum(w,page);tCumBud+=budgetCumAtMonth(w,page);tCostM+=reel(w,page);
    /* EAC: rc2 + rad-aware future */
    var _wEAC=0;
    for(var _mf=page+1;_mf<NM;_mf++) _wEAC+=ccSlot(_mf).rad[w.id]!==undefined?+ccSlot(_mf).rad[w.id]:reelHMFromAssign(w.id,_mf);
    tETC+=_wEAC;
  });
  ...
  h+='<td class="p3" style="border-right:1px solid var(--border2)"></td>';
  h+=_ccFixedCols(tB,tBM,tCostM,tVarM,tCumBud,tRC,tCumVar,tEAC,tVarEAC,tVarEOS,SEP,';');
  h+='<td style="border-right:'+SEP+';background:#f0f9ff"></td>';
  for(var m=0;m<NM;m++){
    ...
    h+='<div style="font-size:10px;color:var(--blue)">'+(mB?mB.toFixed(1):'')+'</div>';
    h+='<div style="font-size:10px;color:var(--green)">'+(mC?mC.toFixed(1):'')+'</div>';
    h+='<div style="font-size:10px;font-weight:700;color:'+vC+'">'+(mB>0||mC>0?(mVar>0?'+':'')+mVar.toFixed(1):'')+'</div>';
    h+='</td>';
    h+='<td class="'+(m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td')+'" style="border-right:'+SEP+'"></td>';
  }
  h+='</tr>';
  return h;
}
...
function wbsCCRows(w,lv,page){
  ...
  var budM=bud(w,page); var costM=reel(w,page);
  var varM=+(budM-costM).toFixed(1);
  var cumVar=+(cumBud-rc2).toFixed(1);
  /* EAC = future HM only (months AFTER CC month) */
  var eac=0;
  for(var _mf=page+1;_mf<NM;_mf++){
    eac+=ccSlot(_mf).rad[w.id]!==undefined?+ccSlot(_mf).rad[w.id]:reelHMFromAssign(w.id,_mf);
  }
  eac=+eac.toFixed(1);
  /* Var EAC = Total Budget - Cum Bud - EAC */
  var varEAC=+(bt-cumBud-eac).toFixed(1);
  /* Var EOS = Budget Total - Cum Cost - EAC = Cum Var + Var EAC */
  var varEOS=+(bt-rc2-eac).toFixed(1);
  var wid=w.id;
  ...
  /* Fixed summary cols */
  h+=_ccFixedCols(bt,budM,costM,varM,cumBud,rc2,cumVar,eac,varEAC,varEOS,SEP,';');
  /* Label column: Bud / Real / Var */
  h+='<td style="border-right:'+SEP+';background:#f8fafc;vertical-align:middle;padding:2px 5px">';
  h+='<div style="font-size:8px;color:var(--tx3);line-height:2;text-align:right"><div>Bud</div><div>Real</div><div>Var</div></div>';
  h+='</td>';
  /* Monthly cells: HM col + EUR col */
  for(var m=0;m<NM;m++){
    var bv=bud(w,m); var bEur=getBudEur(w,m);
    ...
    if(m<S.ccM){
      av=reel(w,m); avStyle='color:var(--tx2)';
      ...
      av=reel(w,m); avStyle='color:var(--pur);font-weight:700';
      avEur=getReelEur(w,m);
      (w.subcontracts||[]).forEach(function(sc){avEur+=+(sc.billing[mkey(m)]||0);});
      (S.contracts||[]).filter(function(ct){return ct.wbsId===wid;}).forEach(function(ct){
        if(contractMonthActive(ct,m))avEur+=ct.billing&&ct.billing[mkey(m)]!==undefined?+(ct.billing[mkey(m)]):0;
    ...
    var _lk=isCCMonthLocked(m);
    var editable=(m>=S.ccM&&!_lk);
    /* HM col */
    h+='<td class="'+cls+(editable?' edt':'')+' num" style="'+bl+'border-right:1px solid var(--border);vertical-align:top;padding:2px 4px'+(editable?';cursor:pointer':'')+'"'
      +(editable?' onclick="ecCC(this,\''+wid+'\','+m+',\'rad\','+av+')"':'')+' title="'+(editable?'Click to edit ETC':'')+'\">';
    ...
    h+='<div style="font-size:10px;font-weight:700;color:'+vHc+'">'+(bv>0||av>0?(varHM>0?'+':'')+varHM.toFixed(1):'')+'</div>';
    h+='</td>';
    /* EUR col */
    h+='<td class="'+cls+' num" style="border-right:'+SEP+';vertical-align:top;padding:2px 4px">';
    h+='<div style="font-size:9px;color:#1d4ed8">'+(bEur?fkv(bEur):'')+'</div>';
    h+='<div style="font-size:9px;color:#15803d'+(m>S.ccM?';font-style:italic':'')+'">'+(avEur?fkv(avEur):'')+'</div>';
    h+='<div style="font-size:9px;font-weight:700;color:'+vEc+'">'+(bEur>0||avEur>0?(varEur>0?'+':'')+fkv(varEur):'')+'</div>';
    h+='</td>';
  }
  h+='</tr>';

  /* ══ SubRows (additional specialties) ══ */
  if(w.subRows&&w.subRows.length){
    w.subRows.forEach(function(sr){
      var srBt=addSubRowBudget(sr);
      var srC=getCompany(sr.companyId||w.companyId);
      var srCname=srC?srC.name:''; var srCbg=srC&&srC.bg?srC.bg:'var(--bl)'; var srCco=srC&&srC.co?srC.co:'var(--blue)';
      var srSl=specLabel(sr.spec); var srPu=+(sr.pu||w.pu||0);
      var srBudM=subRowBudAtSlot(sr,page);
      var srCumBud=0; for(var _m=0;_m<=page;_m++) srCumBud+=subRowBudAtSlot(sr,_m);
      h+='<tr class="rre '+alt+'">';
      h+='<td class="p1" style="padding-left:'+(ind+16)+'px;border-right:1px solid var(--border2);font-size:10px;color:var(--tx2)"><span style="font-size:9px;color:var(--tx3)">+ Specialty</span></td>';
      h+='<td class="p2" style="border-right:1px solid var(--border2)">'+(srCname?'<span class="cobadge" style="background:'+srCbg+';color:'+srCco+'">'+srCname+'</span>':''  )+'</td>';
      h+='<td class="p3" style="border-right:1px solid var(--border2)">'+(srSl?'<span class="spsel sk-'+sr.spec+'" style="cursor:default">'+srSl+'</span>':'')+'</td>';
      h+='<td class="num" style="border-right:'+SEP+';color:var(--blue)">'+(srBt?srBt.toFixed(1):'')+'</td>';
      h+='<td class="num" style="border-right:1px solid var(--border2);color:var(--blue)">'+(srBudM?srBudM.toFixed(1):'')+'</td>';
      h+='<td style="border-right:1px solid var(--border2)"></td>';
      h+='<td style="border-right:'+SEP+'"></td>';
      h+='<td class="num" style="border-right:1px solid var(--border2);color:var(--tx3)">'+(srCumBud?srCumBud.toFixed(1):'')+'</td>';
      h+='<td style="border-right:1px solid var(--border2)"></td>';
      h+='<td style="border-right:'+SEP+'"></td>';
      h+='<td class="num" style="border-right:1px solid var(--border2);color:var(--tx2)">'+(srBt?srBt.toFixed(1):'')+'</td>';
      h+='<td style="border-right:1px solid var(--border2)"></td>';
      h+='<td style="border-right:'+SEP+'"></td>';
      h+='<td style="border-right:'+SEP+';background:#f8fafc;padding:2px 5px"><div style="font-size:8px;color:var(--tx3);text-align:right;line-height:2"><div>Bud</div><div></div><div></div></div></td>';
      for(var m=0;m<NM;m++){
        var srBv=subRowBudAtSlot(sr,m); var srBEur=Math.round(srBv*26*8*srPu);
        var cls=m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td';
        var bl=m===page?'border-left:2px solid var(--pur);':'';
        h+='<td class="'+cls+' num" style="'+bl+'border-right:1px solid var(--border);vertical-align:top;padding:2px 4px">';
        h+='<div style="font-size:10px;color:var(--blue)">'+(srBv?srBv.toFixed(1):'')+'</div></td>';
        h+='<td class="'+cls+' num" style="border-right:'+SEP+';vertical-align:top;padding:2px 4px">';
        h+='<div style="font-size:9px;color:#1d4ed8">'+(srBEur?fkv(srBEur):'')+'</div></td>';
      }
      h+='</tr>';
    });
  }
  /* Separator */
  h+='<tr class="wbs-sep-row"><td colspan="'+(14+NM*2)+'"></td></tr>';
  ...
  (w.subcontracts||[]).forEach(function(sc){
    ...
    h+='<tr class="'+alt+'"><td class="p1" colspan="14" style="padding:2px 6px 2px '+(ind+10)+'px;border-right:'+SEP+';background:#eff6ff;font-size:10px;color:var(--blue)">';
    h+='&#128196; <strong>'+sc.ref+'</strong>'+(sc.company?' — '+sc.company:'');
    h+=' &nbsp;·&nbsp; Bud: '+fkv(sc.amt)+'€ &nbsp;·&nbsp; Fact: '+fkv(scBilled)+'€ &nbsp;·&nbsp; Reste: <span style="color:'+(scReste<0?'var(--red)':'var(--green)')+'">'+fkv(scReste)+'€</span>';
    h+='</td>';
    for(var m=0;m<NM;m++){
      ...
      h+='<td class="'+cls2+(!_lk&&!_past?' edt':'')+' num" style="border-right:1px solid var(--border);font-size:10px;color:var(--blue);'+(!_lk&&!_past?'cursor:pointer':'')+'"'
      ...
      h+='<td class="'+cls2+'" style="border-right:'+SEP+'"></td>';
    }
    h+='</tr>';
  });
  ...
  (S.contracts||[]).filter(function(ct){return ct.wbsId===wid;}).forEach(function(ct){
    ...
    var alertBadge=_cOver
      ?'<span style="background:#fee2e2;color:#b91c1c;font-weight:700;padding:1px 6px;border-radius:4px;margin-left:6px">⚠ +'+fkv(_cProj-_ctBud)+'€</span>'
      :(_ctBud>0?'<span style="background:#dcfce7;color:#15803d;padding:1px 6px;border-radius:4px;margin-left:6px">✓ reste '+fkv(_ctBud-_cProj)+'€</span>':'');
    h+='<tr class="'+alt+'"><td class="p1" colspan="14" style="padding:2px 6px 2px '+(ind+10)+'px;border-right:'+SEP+';background:#faf5ff;font-size:10px;color:var(--pur)">';
    h+='&#128196; <strong>'+ct.ref+'</strong>'+(ct.company?' — '+ct.company:'');
    if(_ctBud)h+=' &nbsp;·&nbsp; Bud: '+fkv(_ctBud)+'€';
    h+=' &nbsp;·&nbsp; Dép: '+fkv(_cSpent)+'€'+alertBadge;
    h+='</td>';
    for(var m=0;m<NM;m++){
      if(!contractMonthActive(ct,m)){h+='<td style="border-right:1px solid var(--border)"></td><td style="border-right:'+SEP+'"></td>';continue;}
      ...
      var _cDisp=m<=S.ccM?(_cIsReal?+(ct.billing[_ck]):0):(_cIsReal?+(ct.billing[_ck]):_cFcast);
      var cls2=m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td';
      h+='<td class="'+cls2+(!_lk2&&!_past2?' edt':'')+' num" colspan="2" style="border-right:'+SEP+';font-size:10px;color:'+(_cIsReal?'#15803d':'var(--tx3)')+(m>S.ccM?';font-style:italic':'')+(!_lk2&&!_past2?';cursor:pointer':'')+'"'
...
function ecCC(td,wid,m,type,cur){
  ...
  openInput(td,cur,borderCol,function(v){
    /* Explicit save for CC rad/reel types */
    if(type==='rad'){
      ccSlot(m).rad[wid]=+v;
      saveS();
      setTimeout(function(){ renderCCPage(S.ccM); },50);
    } else if(type==='reel'){
      ccSlot(m).reel[wid]=+v;
      saveS();
      setTimeout(function(){ renderCCPage(S.ccM); },50);
...
function ccGrandTotalRow(page){
  ...
  var tVarEAC=+(tB-tCumBud-tEAC).toFixed(1); /* Bud Total - CumBud - EAC */
  var tVarEOS=+(tB-tRC-tEAC).toFixed(1); /* Bud Total - CumCost - EAC */
  ...
  for(var m=0;m<NM;m++){
    ...
    h+='<td class="num" style="border-right:3px solid #1a3a9e;vertical-align:top;padding:2px 4px">'
      +'<div style="color:#93c5fd;font-size:10px">'+(mB?mB.toFixed(1):'')+'</div>'
      +'<div style="color:#fff;font-size:10px">'+(mC?mC.toFixed(1):'')+'</div>'
      +'<div style="font-weight:700;font-size:10px;color:'+vC+'">'+(mB>0||mC>0?(mVar>0?'+':'')+mVar.toFixed(1):'')+'</div>'
      +'</td>';
  }
  h+='</tr>';
  return h;
...
function renderWorkforcePage(){
  ...
  var months=[];
  for(var m=0;m<NM;m++){
    var slot={m:m,label:ML[m]?ML[m].short:'',isCur:m===S.ccM,isPast:m<S.ccM};
    /* Total budget HM */
    slot.budHM=0; S.wbs.forEach(function(w){slot.budHM+=bud(w,m);});
    /* Total assigned workers */
    slot.assigned=0;
    S.wbs.forEach(function(w){slot.assigned+=getWBSWorkers(w.id,m).length;});
    /* Delta = assigned - budget */
    slot.delta=+(slot.assigned-slot.budHM).toFixed(1);
    months.push(slot);
  }
  ...
  S.wbs.forEach(function(w){
    var sp=w.spec||'other';
    if(!specMap[sp]) specMap[sp]={spec:sp,label:specLabel(sp)||sp,months:[]};
  });
  Object.keys(specMap).forEach(function(sp){
    for(var m=0;m<NM;m++){
      var budHM=0,assigned=0;
      S.wbs.filter(function(w){return (w.spec||'other')===sp;}).forEach(function(w){
        budHM+=bud(w,m);
        assigned+=getWBSWorkers(w.id,m).length;
      });
      specMap[sp].months.push({budHM:budHM,assigned:assigned,delta:+(assigned-budHM).toFixed(1)});
    }
  });
  var specs=Object.values(specMap).filter(function(s){
    return s.months.some(function(mo){return mo.budHM>0||mo.assigned>0;});
  });

  /* ── Page header ── */
  h+='<div style="display:flex;align-items:center;gap:12px;padding:12px 16px;background:var(--white);border-bottom:1px solid var(--border2)">';
  h+='<span style="font-size:16px">&#128200;</span>';
  h+='<h2 style="font-size:14px;font-weight:700;margin:0">Workforce Plan</h2>';
  h+='<span style="font-size:11px;color:var(--tx3)">Budget vs Assigned workers by month</span>';
  h+='<div style="margin-left:auto;display:flex;gap:8px">';
  h+='<span style="font-size:10px;background:#dbeafe;color:#1d4ed8;padding:2px 8px;border-radius:4px">&#9646; Budget HM</span>';
  h+='<span style="font-size:10px;background:#dcfce7;color:#15803d;padding:2px 8px;border-radius:4px">&#9646; Assigned</span>';
  h+='<span style="font-size:10px;background:#fee2e2;color:#b91c1c;padding:2px 8px;border-radius:4px">&#9650; Overload</span>';
  h+='<span style="font-size:10px;background:#f0fdf4;color:#15803d;padding:2px 8px;border-radius:4px">&#9650; Available</span>';
  h+='</div></div>';

  /* ── Main chart: Total project ── */
  h+='<div style="padding:16px">';
  h+='<h3 style="font-size:12px;font-weight:700;margin:0 0 12px;color:var(--tx2)">PROJECT TOTAL — Budget vs Assigned</h3>';
  h+=_wfBarChart(months,'total');

  /* ── Delta summary cards ── */
  h+='<div style="display:flex;gap:8px;margin:16px 0;flex-wrap:wrap">';
  /* Next 3 months from CC */
  for(var m=S.ccM;m<Math.min(S.ccM+6,NM);m++){
    var mo=months[m];
    var dCol=mo.delta>0?'#b91c1c':mo.delta<0?'#15803d':'#64748b';
    var dBg=mo.delta>0?'#fee2e2':mo.delta<0?'#dcfce7':'#f1f5f9';
    var dLabel=mo.delta>0?'+'+mo.delta.toFixed(1)+' overload':mo.delta<0?mo.delta.toFixed(1)+' available':'balanced';
    h+='<div style="background:var(--white);border:1px solid var(--border2);border-radius:10px;padding:10px 14px;min-width:110px">';
    h+='<div style="font-size:10px;font-weight:700;color:'+(mo.isCur?'var(--pur)':'var(--tx3)')+'">'+mo.label+(mo.isCur?' ●':'')+'</div>';
    h+='<div style="font-size:13px;font-weight:800;margin:4px 0"><span style="color:#1d4ed8">'+mo.budHM.toFixed(0)+'</span> / <span style="color:#15803d">'+mo.assigned+'</span></div>';
    h+='<div style="font-size:10px;font-weight:700;background:'+dBg+';color:'+dCol+';padding:2px 6px;border-radius:4px">'+dLabel+'</div>';
    h+='</div>';
  }
  h+='</div>';
  ...
  /* ── By specialty ── */
  if(specs.length){
    h+='<h3 style="font-size:12px;font-weight:700;margin:16px 0 12px;color:var(--tx2)">BY SPECIALTY</h3>';
    ...
    specs.forEach(function(sp){
      ...
      h+='<div style="font-size:11px;font-weight:700;margin-bottom:8px;display:flex;align-items:center;gap:6px">';
      h+='<span class="spsel sk-'+sp.spec+'" style="cursor:default;font-size:10px">'+sp.label+'</span>';
      h+='</div>';
      h+=_wfBarChart(sp.months.map(function(mo,i){return{m:i,label:months[i].label,isCur:months[i].isCur,isPast:months[i].isPast,budHM:mo.budHM,assigned:mo.assigned,delta:mo.delta};}), sp.spec);
      h+='</div>';
    });
    h+='</div>';
  }
  ...
  /* ── By WBS group ── */
  h+='<h3 style="font-size:12px;font-weight:700;margin:16px 0 12px;color:var(--tx2)">BY GROUP</h3>';
  h+='<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(340px,1fr));gap:12px">';
  S.groups.forEach(function(g){
    var allW=S.wbs.filter(function(w){return w.parentId===g.id;}).slice();
    for(var si=0;si<g.sub.length;si++) S.wbs.filter(function(w){return w.parentId===g.sub[si].id;}).forEach(function(w){allW.push(w);});
    if(!allW.length) return;
    var gMonths=[];
    for(var m=0;m<NM;m++){
      var budHM=0,assigned=0;
      allW.forEach(function(w){budHM+=bud(w,m);assigned+=getWBSWorkers(w.id,m).length;});
      gMonths.push({m:m,label:months[m].label,isCur:months[m].isCur,isPast:months[m].isPast,budHM:budHM,assigned:assigned,delta:+(assigned-budHM).toFixed(1)});
    }
    if(!gMonths.some(function(mo){return mo.budHM>0||mo.assigned>0;})) return;
    h+='<div style="background:var(--white);border:1px solid var(--border2);border-radius:10px;padding:12px">';
    h+='<div style="font-size:11px;font-weight:700;margin-bottom:8px;color:#1d4ed8">'+g.name+'</div>';
    h+=_wfBarChart(gMonths,'g_'+g.id);
    h+='</div>';
  });
  h+='</div>';
  h+='</div>';
...
function _wfBarChart(months,key){
  ...
  var maxV=1;
  months.forEach(function(mo){maxV=Math.max(maxV,mo.budHM,mo.assigned);});
  var barH=60; /* max bar height px */
  var barW=Math.max(18,Math.min(40,Math.floor(700/months.length)-4));
  var h='<div style="overflow-x:auto"><div style="display:flex;align-items:flex-end;gap:2px;min-height:'+(barH+32)+'px;padding-bottom:0">';

  months.forEach(function(mo,i){
    var hBud=mo.budHM>0?Math.max(2,Math.round(mo.budHM/maxV*barH)):0;
    var hAsg=mo.assigned>0?Math.max(2,Math.round(mo.assigned/maxV*barH)):0;
    var borderL=mo.isCur?'border-left:2px solid var(--pur);':'';
    var isOver=mo.delta>0; var isUnder=mo.delta<0;
    var deltaStr=mo.delta!==0?(mo.delta>0?'+':'')+mo.delta.toFixed(0):'';
    var deltaCol=isOver?'#b91c1c':isUnder?'#15803d':'#94a3b8';

    h+='<div style="display:flex;flex-direction:column;align-items:center;flex-shrink:0;width:'+barW+'px;'+borderL+'">';
    /* Bars */
    h+='<div style="position:relative;display:flex;align-items:flex-end;gap:1px;height:'+barH+'px">';
    /* Budget bar */
    h+='<div style="width:'+(barW/2-1)+'px;height:'+hBud+'px;background:#93c5fd;border-radius:2px 2px 0 0" title="Bud: '+mo.budHM.toFixed(1)+'"></div>';
    /* Assigned bar */
    h+='<div style="width:'+(barW/2-1)+'px;height:'+hAsg+'px;background:'+(isOver?'#fca5a5':'#86efac')+';border-radius:2px 2px 0 0" title="Assigned: '+mo.assigned+'"></div>';
    h+='</div>';
    /* Delta indicator */
    h+='<div style="font-size:8px;font-weight:700;color:'+deltaCol+';height:12px;line-height:12px">'+deltaStr+'</div>';
    /* Month label */
    h+='<div style="font-size:8px;color:'+(mo.isCur?'var(--pur)':mo.isPast?'var(--tx3)':'var(--tx2)')+';white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:'+barW+'px;text-align:center">'+mo.label+'</div>';
    h+='</div>';
  });
  h+='</div></div>';
  return h;
