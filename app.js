...
function ecCC(td,wid,m,type,cur){
  ...
  openInput(td,cur,borderCol,function(v){
    ...
    saveS();
    /* Full re-render only after save timer fires — avoids scroll jump */
    if(!_ccRenderPending){
      _ccRenderPending=true;
      setTimeout(function(){
        _ccRenderPending=false;
        renderCCPage(S.ccM);
      },300);
    }
  });
...
function ccGrandTotalRow(page){
  ...
  S.wbs.forEach(function(w){
    tB+=addBudget(w);tBM+=bud(w,page);tRC+=reelCum(w,S.ccM);
  ...
  var tCostM=0; S.wbs.forEach(function(w){tCostM+=reel(w,S.ccM);});
