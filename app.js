...
function handleCellClick(e, cellKey){
  ...
  if(workers.length > 0){
    ...
    /* No workers — open picker directly */
    openWorkerPickerMulti(info.wbsId, [info.m]);
  }
}
...
function showCellWorkerPopup(e, wbsId, m, workers){
  ...
  h += '<div style="margin-top:8px;text-align:center"><button class="btn pri" style="font-size:10px;padding:3px 12px" onclick="var p=document.getElementById(\'chipPop\');if(p)p.remove();openWorkerPickerMulti(\''+wbsId+'\',['+m+'])">+ Add Worker</button></div>';
...
function renderOrgChartPage(){
  ...
  S.wbs.forEach(function(w){
    ...
    S.assignments.forEach(function(a){
      ...
      /* Check if this worker is foreman on this WBS this month (from assignment flag) */
      var _isFmA=false;
    ...
    supGroups[supName].push({wbs:w,foreman:foreman,workers:workers.sort(function(a,b){return a.name.localeCompare(b.name);})});
  });
  
  var unassigned=S.workers.filter(function(wk){return !usedWorkerIds[wk.id];});
  
  var h='<div style="padding:16px;overflow-x:auto">';
  h+='<h2 style="font-size:16px;font-weight:700;margin:0 0 4px">&#127970; Organization Chart</h2>';
  h+='<p style="font-size:11px;color:var(--tx3);margin:0 0 20px">'+(ML[m]?ML[m].label:'')+' — '+S.workers.length+' workers, '+unassigned.length+' unassigned</p>';
  ...
  Object.keys(supGroups).sort().forEach(function(supName){
    ...
    cards.forEach(function(card){
      var w=card.wbs,col=specColor(w.spec||''),n=(card.foreman?1:0)+card.workers.length;
      ...
      h+='<div style="background:'+col+';color:#fff;padding:8px 12px"><div style="font-size:9px;opacity:.8">'+(w.code||'WBS')+'</div><div style="font-weight:700;font-size:12px">'+w.name+'</div><div style="font-size:9px;opacity:.7">'+n+'/'+_bHM+' worker'+(n!==1?'s':'')+'</div></div>';
      if(card.foreman){
      ...
      var _assigned=card.workers.length+(card.foreman?1:0);
      var _budHM=Math.round(budTotal(card.wbs,m));
      var _open=Math.max(0,_budHM-_assigned);
      if(card.workers.length){
  ...
  if(unassigned.length){
    ...
    unassigned.forEach(function(wk){
      var c=specColor(wk.spec||'');
      h+='<div style="display:flex;align-items:center;gap:5px;padding:4px 10px;background:#fff;border:1px solid #e2e8f0;border-radius:6px;font-size:11px"><span style="width:8px;height:8px;border-radius:50%;background:'+c+'"></span>'+wk.name;
      if((wk.role||'')==='foreman') h+=' <span style="font-size:8px;background:#f59e0b;color:#fff;padding:1px 4px;border-radius:3px">FM</span>';
      h+='</div>';
    });
    h+='</div></div>';
  }
  h+='</div>';
  el.innerHTML=h;
  }catch(err){el.innerHTML='<div style="padding:20px;color:red">Org Chart Error: '+err.message+'<br><pre>'+err.stack+'</pre></div>';}
}
