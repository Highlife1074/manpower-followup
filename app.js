...
function ccHead(page){
  ...
  for(var m=0;m<NM;m++){
    ...
    h+='<th colspan="3" style="text-align:center;background:'+bg+';color:'+col+';'+sep1+'padding:4px 6px">'+lbl+(m===S.ccM?' ●':'')+'</th>';
  }
  h+='</tr>';
  /* Sub-header row */
  ...
  /* Monthly columns — groups of 3: HM | EUR | Var */
  for(var m=0;m<NM;m++){
    var lbl=ML[m]?ML[m].short:(MS[m]+' '+( ML[m]?ML[m].year:YEAR));
    var isCur=(m===page); var isPast=(m<page);
    var bg=isCur?'rgba(78,69,128,.10)':isPast?'var(--bg2)':'transparent';
    var col=isCur?'var(--pur)':isPast?'var(--tx3)':'var(--tx2)';
    var bl=isCur?'border-left:2px solid var(--pur);':'';
    h+='<th colspan="3" style="text-align:center;background:'+bg+';color:'+col+';'+bl+'border-right:2px solid var(--border2);padding:3px 6px;font-size:10px;font-weight:700">'+lbl+(m===S.ccM?' ●':'')+'</th>';
  }
  h+='</tr>';
  /* Sub-header: HM | EUR | Var for each month */
  h+='<tr style="background:var(--bg2);position:sticky;top:24px;z-index:14">';
  h+='<th colspan="13" style="border-right:2px solid var(--border3)"></th>';
  for(var m=0;m<NM;m++){
    var isCur=(m===page); var isPast=(m<page);
    var bg=isCur?'rgba(78,69,128,.06)':isPast?'var(--bg2)':'transparent';
    var bl=isCur?'border-left:2px solid var(--pur);':'';
    h+='<th style="font-size:9px;font-weight:600;color:var(--blue);background:'+bg+';'+bl+'border-right:1px solid var(--border)">HM</th>';
    h+='<th style="font-size:9px;font-weight:600;color:#1d4ed8;background:'+bg+';border-right:1px solid var(--border)">EUR</th>';
    h+='<th style="font-size:9px;font-weight:600;color:var(--tx3);background:'+bg+';border-right:2px solid var(--border2)">Var</th>';
  }
  h+='</tr>';
  return h;
}
...
function ccBody(page){
  ...
  for(var gi=0;gi<S.groups.length;gi++){
    ...
    if(g.col){h+='<tr class="wbs-sep-row"><td colspan="'+(13+NM*3)+'"></td></tr>'; continue;}
    ...
    h+='<tr style="height:4px"><td colspan="'+(13+NM*3)+'" style="background:var(--bg);border-top:3px solid #8a8070"></td></tr>';
  }
  return h;
}
...
function wbsCCRows(w,lv,page,isLast){
  ...
  /* Monthly cells: HM Budget | EUR Budget | Var HM (Budget-Cost) */
  for(var m=0;m<NM;m++){
    var isCur=(m===page); var isPast=(m<S.ccM);
    var bg=isCur?'rgba(78,69,128,.06)':isPast?'var(--bg2)':'';
    var bl=isCur?'border-left:2px solid var(--pur);':'';
    var bv=bud(w,m);
    var budEurM=getBudEur(w,m);
    /* Actual or ETC */
    var av,avEur;
    if(m<=S.ccM){
      av=reel(w,m);
      avEur=getReelEur(w,m);
      /* Add contract billings to EUR actual */
      ...
      var hasMRad=ccSlot(m).rad[w.id]!==undefined;
      var asgFut=reelHMFromAssign(w.id,m);
      av=hasMRad?+ccSlot(m).rad[w.id]:asgFut;
      /* Future EUR = HM × 26 × 8 × PU + contracts forecast */
      avEur=Math.round(av*26*8*(w.pu||0));
      (S.contracts||[]).filter(function(ct){return ct.wbsId===wid;}).forEach(function(ct){
        if(contractMonthActive(ct,m)){
          var _mk=mkey(m);
          avEur+=ct.billing&&ct.billing[_mk]!==undefined?+(ct.billing[_mk]):+(ct.adjustedForecast||ct.monthlyForecast);
        }
      });
    }
    var varHM=+(bv-av).toFixed(1);
    var varHMcol=varHM>0?'var(--green)':varHM<0?'var(--red)':'var(--tx3)';
    /* HM cell: past=read-only, cur=actual, future=editable ETC */
    var hmContent;
    if(m<=S.ccM){
      hmContent='<span style="color:'+(m===S.ccM?'var(--pur)':'var(--tx2)')+';font-weight:'+(m===S.ccM?'700':'400')+'">'+(av?av.toFixed(1):'0')+'</span>';
    } else {
      var hasMR=ccSlot(m).rad[w.id]!==undefined;
      var delta=hasMR?+(av-reelHMFromAssign(w.id,m)).toFixed(1):0;
      var dStr=delta!==0?'<span style="font-size:8px;color:'+(delta>0?'#15803d':'#b91c1c')+'"> ('+(delta>0?'+':'')+delta+')</span>':'';
      var _ccLkM=isCCMonthLocked(m);
      if(!_ccLkM){
        hmContent='<span style="color:'+(hasMR?'#c2410c':'var(--tx3)')+';font-style:'+(hasMR?'normal':'italic')+';cursor:pointer;text-decoration:underline dotted" onclick="ecCCCell(this,\''+wid+'\','+m+',\'rad\','+av+')">'+(av?av.toFixed(1):'0')+dStr+'</span>';
      } else {
        hmContent='<span style="color:var(--tx3);font-style:italic">'+(av?av.toFixed(1):'0')+'</span>';
      }
    }
    /* EUR cell */
    var eurContent;
    if(m<=S.ccM){
      var _eurV=getReelEur(w,m);
      var _eurStyle='color:#15803d;font-size:9px;cursor:pointer;text-decoration:underline dotted';
      eurContent='<span style="'+_eurStyle+'" onclick="ecCCCell(this,\''+wid+'\','+m+',\'cc-reel-eur\','+_eurV+')">'+(avEur?fkv(avEur):'')+'</span>';
    } else {
      eurContent='<span style="color:#1d4ed8;font-size:9px;font-style:italic">'+(avEur?fkv(avEur):'')+'</span>';
    }
    h+='<td style="text-align:right;padding:3px 4px;font-family:var(--mono);font-size:10px;'+bl+(bg?'background:'+bg+';':'')+';border-right:1px solid var(--border)">'+hmContent+'</td>';
    h+='<td style="text-align:right;padding:3px 4px;font-family:var(--mono);font-size:9px;border-right:1px solid var(--border);color:#1d4ed8">'+(budEurM?fkv(budEurM):'')+'</td>';
    h+='<td style="text-align:right;padding:3px 4px;font-family:var(--mono);font-size:10px;border-right:3px solid #94a3b8;font-weight:700;color:'+varHMcol+'">'+(bv>0||av>0?(varHM>0?'+':'')+varHM.toFixed(1):'')+'</td>';
  }
  h+='</tr>';
