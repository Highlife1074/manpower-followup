...
function getBudEur(w,i){var k=mkey(i);if(w.dataEur&&w.dataEur[k]!==undefined)return+(w.dataEur[k]);/* Auto-compute from HM budget × PU */var pu=+(w.pu||0);if(pu>0)return+((bud(w,i)*pu)).toFixed(0);return 0;}
...
function renderTabBar(){
  ...
  /* Single CC tab = current CC month */
  if(S.ccM>=0&&ML[S.ccM]){
    var _ccTabKey=ML[S.ccM].key;
    var _ccActive=(VIEW===_ccTabKey);
    h+='<div class="tab tab-cc'+(_ccActive?' active':'')+'" onclick="switchTab(\''+_ccTabKey+'\')" style="display:flex;align-items:center;gap:5px">';
...
function renderTopbarR(){
  ...
  h+='</button>';
  if(_userRole==='admin') h+='<button class="btn soft" style="font-size:11px" onclick="openRoleSettings()">&#128100; Roles</button>';
...
function wkApplyBulkEdit(ids){
  /* ids may be array or JSON string */
  ...
  S.workers.forEach(function(wk){
    ...
    if(resp!==undefined&&resp!=='') wk.responsible=resp; /* allow setting to non-empty */
