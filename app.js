...
function wbsCCRows(w,lv,page,isLast){
  ...
  for(var m=0;m<NM;m++){
    ...
    var hmCol='<div style="font-size:10px;color:var(--blue);line-height:1.6">'+(bv?bv.toFixed(1):'')+'</div>'
      +'<div style="font-size:10px;line-height:1.6"'+hmClickAttr+'>'+realHM+'</div>'
      +'<div style="font-size:10px;font-weight:700;color:'+vHMc+';line-height:1.6">'+(bv>0||av>0?(varHM>0?'+':'')+varHM.toFixed(1):'')+'</div>';
    /* EUR col: Budget EUR / Act EUR / Var EUR stacked */
    var _eurV=getReelEur(w,m);
    var eurClickAttr=m<=S.ccM&&!_ccLkM?' onclick="ecCCCell(this,\''+wid+'\','+m+',\'cc-reel-eur\',' +_eurV+')" style="cursor:pointer;text-decoration:underline dotted;color:#15803d;font-size:9px"':' style="color:'+(m>S.ccM?'#1d4ed8':'#15803d')+';font-size:9px;font-style:'+(m>S.ccM?'italic':'normal')+'"';
    var eurCol='<div style="font-size:9px;color:#1d4ed8;line-height:1.6">'+(budEurM?fkv(budEurM):'')+'</div>'
      +'<div'+eurClickAttr+'>'+(avEur?fkv(avEur):'')+'</div>'
      +'<div style="font-size:9px;font-weight:700;color:'+vEUc+';line-height:1.6">'+(budEurM>0||avEur>0?(varEur>0?'+':'')+fkv(varEur):'')+'</div>';
