...
function wbsCCRows(w,lv,page,isLast){
  ...
  if(bt>0)h+='<div style="color:var(--tx3);font-size:9px">tot:'+bt.toFixed(0)+'</div>';
  h+='</td>';
  /* p5: Actual month + cum */
  ...
  h+='<td class="num '+pncBC(va)+'" style="font-weight:700;border-right:1px solid var(--border2)">'+(va>0?'+':'')+va.toFixed(1)+'</td>';
  /* Var.Fin */
  h+='<td class="num '+pncBC(_vaFin)+'" style="font-weight:800;font-size:12px;border-right:2px solid var(--border3)">'+(+_vaFin>0?'+':'')+_vaFin.toFixed(1)+'</td>';
  ...
  for(var m=0;m<NM;m++){
    ...
    if(m<=page){
      ...
      avStyle=hasManualRad?'color:var(--ora);font-weight:700':'color:var(--tx2);font-style:italic';
  ...
  for(var m=0;m<NM;m++){
    ...
    var cls=m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td';
    h+='<td class="'+cls+' num '+pncBC(va2)+'" style="font-size:10px">'+(va2!==0?(va2>0?'+':'')+va2.toFixed(1):'')+'</td>';
  }
  h+='</tr>';
  ...
  (w.subcontracts||[]).forEach(function(sc){
    ...
    h+=' &nbsp;·&nbsp; Bud: <strong>'+sc.amt.toLocaleString()+' €</strong>';
    h+=' &nbsp;·&nbsp; Fact: <strong>'+scBilled.toLocaleString()+' €</strong>';
    h+=' &nbsp;·&nbsp; Reste: <strong style="color:'+(scReste<0?'var(--red)':'var(--green)')+'">'+scReste.toLocaleString()+' €</strong>';
    h+='</span></td>';
    for(var m=0;m<NM;m++){
      ...
      if(m<=S.ccM&&!_scLk){
        h+='<td class="'+cls2+' edt num" style="color:var(--blue);font-size:10px;cursor:pointer" onclick="openScBillingCell(this,this.dataset.wid,this.dataset.scid,this.dataset.k,'+sv+')" data-wid="'+w.id+'" data-scid="'+sc.id+'" data-k="'+k2+'">'+(sv?sv.toLocaleString():'')+'</td>';
      } else {
        h+='<td class="'+cls2+' lk num" style="color:var(--blue);font-size:10px">'+(sv?sv.toLocaleString():'')+'</td>';
      }
    }
    h+='</tr>';
  });
  ...
  (S.contracts||[]).filter(function(ct){return ct.wbsId===wid;}).forEach(function(ct){
    ...
    if(_ctBud>0&&_cOverrun){
      _alertHtml='<span style="display:inline-flex;align-items:center;gap:4px;margin-left:8px;background:#fee2e2;color:#b91c1c;font-weight:800;font-size:10px;padding:2px 8px;border-radius:6px;border:1px solid #fca5a5">'
        +'⚠ Dépassement +'+Math.round(_cOverAmt).toLocaleString()+' €'
        +(_cExh?' · épuisé '+_cExh:'')
        +'</span>';
    } else if(_ctBud>0){
      _alertHtml='<span style="display:inline-flex;align-items:center;gap:4px;margin-left:8px;background:#dcfce7;color:#15803d;font-size:10px;padding:2px 8px;border-radius:6px;border:1px solid #86efac">'
        +'✓ OK · reste '+Math.round(_ctBud-_cProjected).toLocaleString()+' €'
        +'</span>';
    }
    h+='<span style="font-size:10px;color:var(--pur)">&#128196; <strong>'+ct.ref+'</strong>';
    if(ct.company)h+=' — <span style="color:var(--tx2)">'+ct.company+'</span>';
    if(_ctBud>0){
      h+=' &nbsp;·&nbsp; Bud: <strong>'+_ctBud.toLocaleString()+' €</strong>';
      h+=' &nbsp;·&nbsp; Dép: <strong style="color:var(--tx2)">'+_cSpent.toLocaleString()+' €</strong>';
    }
    h+=_alertHtml;
    ...
    for(var m=0;m<NM;m++){
      ...
      /* Past/CC = actual if set, else 0. Future = forecast */
      var _cDisp=m<=S.ccM?(_cIsReal?_cAct:0):_cFcast;
      ...
      if(!_ccLkM && !_isPast){
        ...
        h+='<td class="'+_ccls2+' edt num" style="'+_cSty+_overBg+';cursor:pointer'+(m>S.ccM?';font-style:italic':'')+'" onclick="openContractCellEdit(this)" data-cid="'+ct.id+'" data-m="'+m+'" data-v="'+(_cDisp||0)+'">'+(_cDisp?_cDisp.toLocaleString():'')+'</td>';
      } else {
        var _overBg2=_cellOver?';background:rgba(185,28,28,.08)':'';
        h+='<td class="'+_ccls2+' lk num" style="'+_cSty+_overBg2+'">'+(_cDisp?_cDisp.toLocaleString():'')+'</td>';
      }
    }
    h+='</tr>';
  });
...
function renderAssignmentsPage(){
  ...
  for(var gi = 0; gi < S.groups.length; gi++){
    ...
    for(var m = startCol; m < NM; m++){
      ...
      var gDisp = '';
      if(_gn > 0 || _gt > 0){
        var gCol = _gn === 0 ? '#b91c1c' : _gt >= _gn ? '#15803d' : _gt > 0 ? '#1d4ed8' : '#b91c1c';
    ...
    for(var si = 0; si < g.sub.length; si++){
      ...
      for(var m = startCol; m < NM; m++){
        ...
        var sgDisp = '';
        if(_sn > 0 || _st > 0){
          var sgCol = _sn === 0 ? '#b91c1c' : _st >= _sn ? '#15803d' : _st > 0 ? '#1d4ed8' : '#b91c1c';
...
function asWBSRow(wbs, ind, startCol, page, nCols){
  ...
  for(var m = startCol; m < NM; m++){
    ...
    if(budHM > 0){
      ...
      countHtml += '<span style="color:' + (nW >= need ? 'var(--green)' : (nW>0?'var(--ora)':'var(--red)')) + ';">' + nW + '</span>';
...
function contractActiveMonths(c){
  /* Count ML months that fall within contract start/end */
  if(!c.startDate&&!c.endDate) return NM;
  var n=0;
  ML.forEach(function(slot){
    var d=new Date(slot.year,slot.month,1);
    var ok=true;
    if(c.startDate){ var sd=new Date(c.startDate); ok=ok&&d>=new Date(sd.getFullYear(),sd.getMonth(),1); }
    if(c.endDate){ var ed=new Date(c.endDate); ok=ok&&d<=new Date(ed.getFullYear(),ed.getMonth(),1); }
    if(ok) n++;
  });
  return n||1;
}
...
function onContractBillingEntry(contractId, m, newVal){
  ...
  if(!c) return;
  var k=mkey(m);
  var prevVal=+(c.billing[k]||c.adjustedForecast||c.monthlyForecast);
  c.billing[k]=newVal;

  /* Only prompt if value differs from forecast */
