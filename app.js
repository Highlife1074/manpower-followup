...
var _showEurCols=true; /* toggle EUR monthly columns */ /* toggle EUR rows in CC */
...
function ccBody(page){
  ...
  for(var gi=0;gi<S.groups.length;gi++){
    ...
    if(g.col){h+='<tr class="wbs-sep-row"><td colspan="'+(14+NM*(_showEurCols?2:1))+'"></td></tr>';continue;}
    ...
    h+='<tr style="height:4px"><td colspan="'+(14+NM*(_showEurCols?2:1))+'" style="background:var(--bg);border-top:3px solid #8a8070"></td></tr>';
  }
  return h;
}
...
function ccGrandTotalRow(page){
  ...
  S.wbs.forEach(function(w){
    if(S.ccNeutral&&S.ccNeutral[w.id]) return; /* neutralized */
    tB+=addBudget(w);tBM+=bud(w,page);tRC+=reelCum(w,page);tCumBud+=budgetCumAtMonth(w,page);tCostM+=reel(w,page);
    var _wEAC=0;
...
function wbsCCRows(w,lv,page){
  ...
  /* Separator */
  h+='<tr class="wbs-sep-row"><td colspan="'+(14+NM*(_showEurCols?2:1))+'"></td></tr>';
...
function renderGroupDetail(page){
  ...
  banner+='<button class="btn" onclick="_showEurCols=!_showEurCols;renderCCPage(S.ccM);" style="font-size:11px">&#128176; EUR '+ (_showEurCols?'ON':'OFF')+'</button>';
...
/* ============================================================
   ...
   ============================================================ */
var _wfShowBudget=true; /* toggle budget bars */
...
function renderWorkforcePage(){
  ...
  /* ── Build monthly data ──
     Logic:
     - Budget HM = reference (from initial budget)
     - EAC = rad override if set, else assignments (= what you NEED)
     - Assigned = workers in assignments (what you HAVE)
     - Delta = EAC - Assigned → positive = need to recruit, negative = surplus
  */
  var months=[];
  for(var m=S.ccM+1;m<NM;m++){  /* future months only */
    var mo={m:m,label:ML[m]?ML[m].short:'',isCur:false,isPast:false};
    mo.budHM=0; mo.eac=0; mo.assigned=0;
    S.wbs.forEach(function(w){
      mo.budHM+=bud(w,m);
      /* EAC for this month: rad override if set, else assignment count */
      mo.eac+=ccSlot(m).rad[w.id]!==undefined?+ccSlot(m).rad[w.id]:reelHMFromAssign(w.id,m);
      mo.assigned+=reelHMFromAssign(w.id,m);
    });
    mo.delta=+(mo.eac-mo.assigned).toFixed(1); /* positive = recruit, negative = surplus */
    mo.budDelta=+(mo.budHM-mo.eac).toFixed(1); /* vs budget */
    months.push(mo);
  }

  /* Page header */
  h+='<div style="display:flex;align-items:center;gap:12px;padding:12px 16px;background:var(--white);border-bottom:1px solid var(--border2);flex-wrap:wrap">';
  h+='<span style="font-size:16px">&#128200;</span>';
  h+='<h2 style="font-size:14px;font-weight:700;margin:0">Workforce Plan</h2>';
  h+='<span style="font-size:11px;color:var(--tx3)">Future months — EAC vs Assigned (from CC month onwards)</span>';
  h+='<div style="margin-left:auto;display:flex;gap:8px;flex-wrap:wrap;align-items:center">';
  h+='<button class="btn" onclick="_wfShowBudget=!_wfShowBudget;renderWorkforcePage()" style="font-size:10px;'+(_wfShowBudget?'background:var(--bl);color:var(--blue);border-color:var(--bm)':'')+'">';
  h+='&#128176; Budget '+(_wfShowBudget?'ON':'OFF')+'</button>';
  h+='<span style="font-size:10px;background:#dbeafe;color:#1d4ed8;padding:2px 8px;border-radius:4px">&#9646; Budget HM</span>';
  h+='<span style="font-size:10px;background:#e9d5ff;color:#7c3aed;padding:2px 8px;border-radius:4px">&#9646; EAC (besoin réel)</span>';
  h+='<span style="font-size:10px;background:#dcfce7;color:#15803d;padding:2px 8px;border-radius:4px">&#9646; Assigné</span>';
  h+='<span style="font-size:10px;background:#fee2e2;color:#b91c1c;padding:2px 8px;border-radius:4px">&#9650; À recruter</span>';
  h+='<span style="font-size:10px;background:#f0fdf4;color:#15803d;padding:2px 8px;border-radius:4px">&#9660; Surplus</span>';
  h+='</div></div>';
  ...
  months.forEach(function(mo){
    ...
    var dStr=mo.delta>0?('▲ +'+mo.delta.toFixed(0)+' à recruter'):mo.delta<0?('▼ '+mo.delta.toFixed(0)+' surplus'):'✓ équilibré';
    h+='<div style="background:var(--white);border:1px solid var(--border2);border-radius:10px;padding:10px 14px;min-width:120px">';
  ...
  /* Total project chart */
  h+='<h3 style="font-size:12px;font-weight:700;margin:0 0 10px;color:var(--tx2)">PROJET TOTAL — Besoin vs Assigné</h3>';
  h+=_wfBarChart(months,'total');

  /* By specialty */
  var specMap={};
  S.wbs.forEach(function(w){var sp=w.spec||'other';if(!specMap[sp])specMap[sp]={spec:sp,label:specLabel(sp)||sp,months:[]};});
  Object.keys(specMap).forEach(function(sp){
    for(var m=S.ccM+1;m<NM;m++){
      ...
      specMap[sp].months.push({m:m,label:months.find(function(x){return x.m===m;})?months.find(function(x){return x.m===m;}).label:'',budHM:budHM,eac:eac,assigned:assigned,delta:+(eac-assigned).toFixed(1)});
    }
  });
  var specs=Object.values(specMap).filter(function(s){return s.months.some(function(mo){return mo.budHM>0||mo.eac>0;});});
  ...
  if(specs.length){
    ...
    specs.forEach(function(sp){
      ...
      h+='<div style="font-size:11px;font-weight:700;margin-bottom:8px"><span class="spsel sk-'+sp.spec+'" style="cursor:default;font-size:10px">'+sp.label+'</span></div>';
      h+=_wfBarChart(sp.months,'sp_'+sp.spec);
      h+='</div>';
    });
    h+='</div>';
  }
  ...
  S.groups.forEach(function(g){
    ...
    for(var m=S.ccM+1;m<NM;m++){
      ...
      var lbl=months.find(function(x){return x.m===m;}); 
      gMonths.push({m:m,label:lbl?lbl.label:'',budHM:budHM,eac:eac,assigned:assigned,delta:+(eac-assigned).toFixed(1)});
    }
...
function _wfBarChart(months,key){
  ...
  months.forEach(function(mo){
    if(!mo.budHM&&!mo.eac&&!mo.assigned){h+='';return;}
    ...
    var isOver=mo.delta>0; /* need to recruit */
    ...
    h+='<div style="width:'+barW+'px;height:'+hAsg+'px;background:'+(isOver?'#fca5a5':'#86efac')+';border-radius:2px 2px 0 0" title="Assigné: '+mo.assigned+'"></div>';
    h+='</div>';
    h+='<div style="font-size:8px;font-weight:700;color:'+deltaCol+';height:14px;line-height:14px;text-align:center">'+deltaStr+'</div>';
    h+='<div style="font-size:8px;color:var(--tx2);white-space:nowrap;text-align:center">'+mo.label+'</div>';
    h+='</div>';
  });
  h+='</div></div>';
  return h;
