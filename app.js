...
function renderTopbarR(){
  ...
  if(VIEW==='budget'){
    ...
    else{h+='<button class="btn unlock" onclick="lockBudget()">&#128275; Lock</button>';h+='<button class="btn pri" onclick="openModal(\'wbs\')">+ WBS</button>';h+='<button class="btn soft" onclick="openModal(\'group\')">+ Group</button>';h+='<button class="btn soft" onclick="openModal(\'subgroup\')">+ Subgroup</button>';h+='<button class="btn" onclick="printBudget()">&#128438; Print</button>';}
  }
...
function printBudget(){
  ...
  var rows='';
  function addRow(wbs,lv){
    var bt=addBudget(wbs);
    var pu=wbs.pu?wbs.pu.toFixed(0):'—';
    var indent=lv===1?'20px':'40px';
    var co=getCompany(wbs.companyId); var cn=co?co.name:'';
    var sp=specLabel(wbs.spec);
    rows+='<tr><td style="padding:4px 8px 4px '+indent+';font-weight:'+(lv===1?600:400)+'">'+(wbs.code?'<span style="background:#e2e8f0;padding:1px 4px;border-radius:3px;font-size:10px;margin-right:4px">'+wbs.code+'</span>':'')+wbs.name+'</td>';
    rows+='<td>'+cn+'</td><td>'+sp+'</td><td>'+(wbs.responsible||'')+'</td>';
    rows+='<td style="text-align:right">'+pu+'</td>';
    rows+='<td style="text-align:right;font-weight:700">'+bt.toFixed(1)+'</td></tr>';
    if(wbs.subRows) wbs.subRows.forEach(function(sr){
      var srBt=addSubRowBudget(sr); var srCo=getCompany(sr.companyId||wbs.companyId); var srCn=srCo?srCo.name:'';
      rows+='<tr style="color:#666"><td style="padding:3px 8px 3px 50px;font-size:11px">↳ '+specLabel(sr.spec)+'</td><td>'+srCn+'</td><td>'+specLabel(sr.spec)+'</td><td></td><td style="text-align:right">'+(sr.pu||wbs.pu||'—')+'</td><td style="text-align:right">'+srBt.toFixed(1)+'</td></tr>';
    });
  }
  S.groups.forEach(function(g){
    rows+='<tr style="background:#dbeafe;font-weight:700"><td colspan="6" style="padding:6px 8px">'+g.name+'</td></tr>';
    S.wbs.filter(function(w){return w.parentId===g.id;}).sort(function(a,b){return(a.name||'').localeCompare(b.name||'');}).forEach(function(w){addRow(w,1);});
    g.sub.forEach(function(sg){
      rows+='<tr style="background:#eff6ff;font-weight:600"><td colspan="6" style="padding:5px 8px 5px 20px">◆ '+sg.name+'</td></tr>';
      S.wbs.filter(function(w){return w.parentId===sg.id;}).sort(function(a,b){return(a.name||'').localeCompare(b.name||'');}).forEach(function(w){addRow(w,2);});
    });
  });
  win.document.write('<html><head><title>Budget — Manpower Management Tool</title><style>body{font-family:Arial,sans-serif;font-size:12px}table{border-collapse:collapse;width:100%}th{background:#f1f5f9;text-align:left;padding:6px 8px;border-bottom:2px solid #cbd5e1}td{border-bottom:1px solid #e2e8f0;padding:4px 8px}h2{color:#1e3a5f}@media print{button{display:none}}</style></head><body>');
  win.document.write('<h2>Initial Budget — Manpower Management Tool</h2>');
  win.document.write('<p style="color:#64748b">Printed: '+new Date().toLocaleDateString('fr-FR')+'</p>');
  win.document.write('<table><thead><tr><th>WBS / Name</th><th>Company</th><th>Specialty</th><th>Responsible</th><th>PU</th><th>Total HM</th></tr></thead><tbody>'+rows+'</tbody></table>');
...
function ccGrandTotalRow(page){
  ...
  S.wbs.forEach(function(w){
    ...
    tB+=addBudget(w);tBM+=budTotal(w,page);tRC+=reelCum(w,page);tCumBud+=budTotalCum(w,page);tCostM+=reel(w,page);
    var _wEAC=0;
  ...
  var h='<tr style="background:#276BC4;color:#fff;font-weight:800;font-size:12px">';
...
function ccGrpRow(g,allW,page){
  ...
  for(var m=0;m<NM;m++){
    var mB=0,mC=0;allW.forEach(function(w){mB+=budTotal(w,m);mC+=m<=S.ccM?reel(w,m):rad(w,m);});
...
function wbsCCRows(w,lv,page){
  ...
  if(hasSubRows){
    ...
    h+='<tr class="rwbs '+alt+'" style="background:#f0f4f8">';
    /* p1 */
    ...
    /* Label col */
    h+='<td style="border-right:'+SEP+';background:#f0f4f8;padding:2px 5px">';
    ...
    for(var m=0;m<NM;m++){
      ...
      h+='<td class="'+cls+' num" style="'+bl+'border-right:1px solid var(--border);background:#f0f4f8;vertical-align:top;padding:2px 4px">';
      h+='<div style="font-size:10px;color:var(--blue)">'+(bvT?bvT.toFixed(1):'0')+'</div>';
      h+='<div style="font-size:10px;color:var(--tx2)">'+(avT?avT.toFixed(1):'0')+'</div>';
    ...
    h+='<td style="border-right:'+SEP+';background:#f8fafc;padding:2px 5px">';
    h+='<div style="font-size:8px;color:var(--tx3);text-align:right;line-height:2"><div>Bud</div><div>Real</div><div>Var</div></div></td>';
    for(var m=0;m<NM;m++){
      ...
      else if(m===S.ccM){ av=reel(w,m); avStyle='color:var(--pur);font-weight:700'; avEur=getReelEur(w,m); }
      else {
  ...
  (w.subcontracts||[]).forEach(function(sc){
    ...
    h+=' &nbsp;·&nbsp; Bud: '+fkv(sc.amt)+'€ &nbsp;·&nbsp; Fact: '+fkv(scBilled)+'€ &nbsp;·&nbsp; Reste: <span style="color:'+(scReste<0?'var(--red)':'var(--green)')+'">'+fkv(scReste)+'€</span></td>';
    for(var m=0;m<NM;m++){
      ...
      if(_showEurCols) h+='<td class="'+cls2+'" style="border-right:'+SEP+'"></td>';
    }
    h+='</tr>';
  });
  ...
  (S.contracts||[]).filter(function(ct){return ct.wbsId===wid;}).forEach(function(ct){
    ...
    h+=' &nbsp;·&nbsp; Dép: '+fkv(_cSpent)+'€'+alertBadge+'</td>';
    for(var m=0;m<NM;m++){
      if(!contractMonthActive(ct,m)){h+='<td style="border-right:1px solid var(--border)"></td>';if(_showEurCols)h+='<td style="border-right:'+SEP+'"></td>';continue;}
      ...
      var cls2=m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td';
      var colSpan=_showEurCols?'colspan="2"':'';
      h+='<td '+colSpan+' class="'+cls2+(!_lk2&&!_past2?' edt':'')+' num" style="border-right:'+SEP+';font-size:10px;color:'+(_cIsReal?'#15803d':'var(--tx3)')+(m>S.ccM?';font-style:italic':'')+(!_lk2&&!_past2?';cursor:pointer':'')+'"'
...
function _ccSpecRow(w, spec, companyId, pu, budFn, sr, alt, indLevel, page, wid, SEP, showSummary){
  ...
  h+='<td style="border-right:'+SEP+';background:#fafafa"></td>';
  for(var m=0;m<NM;m++){
...
function ccGrandTotalRow(page){
  ...
  for(var m=0;m<NM;m++){
    ...
    S.wbs.forEach(function(w){mB+=budTotal(w,m);mC+=m<=S.ccM?reel(w,m):rad(w,m);mEUR_B+=getBudEur(w,m);mEUR_C+=m<=S.ccM?getReelEur(w,m):Math.round((m<=S.ccM?reel(w,m):rad(w,m))*26*8*(w.pu||0));});
    ...
    h+='<td class="num" style="border-right:3px solid #1a3a9e;vertical-align:top;padding:2px 4px">'
      ...
      +'</td>';
    if(_showEurCols){h+='<td class="num" style="border-right:3px solid #1a3a9e;vertical-align:top;padding:2px 4px">'
      +'<div style="color:#93c5fd;font-size:9px">'+(mEUR_B?fkv(mEUR_B):''  )+'</div>'
      +'<div style="color:#86efac;font-size:9px">'+(mEUR_C?fkv(mEUR_C):'')+'</div>'
      +'<div style="font-weight:700;font-size:9px;color:'+vEC2+'">'+(mEUR_B>0||mEUR_C>0?(mVarEur>0?'+':'')+fkv(mVarEur):'')+'</div>'
      +'</td>';}
  }
  h+='</tr>';
  return h;
