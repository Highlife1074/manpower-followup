...
function ccHead(page){
  var SEP='3px solid #94a3b8'; /* thick separator between categories */
  var sep1='border-right:'+SEP+';';
  /* Group title row */
  var h='<tr style="background:#e2e8f0;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:#475569;position:sticky;top:0;z-index:16">';
  h+='<th colspan="4" style="text-align:left;padding:4px 10px;'+sep1+'">WBS &amp; Budget</th>';
  h+='<th colspan="3" style="text-align:center;padding:4px 10px;'+sep1+'">Month</th>';
  h+='<th colspan="3" style="text-align:center;padding:4px 10px;'+sep1+'">Cumulated</th>';
  h+='<th colspan="3" style="text-align:center;padding:4px 10px;'+sep1+'">EOS</th>';
  for(var m=0;m<NM;m++){
    var lbl=ML[m]?ML[m].short:(MS[m]);
    var isCur=(m===page);
    var bg=isCur?'rgba(78,69,128,.15)':'';
    var col=isCur?'var(--pur)':'#475569';
    h+='<th colspan="2" style="text-align:center;background:'+bg+';color:'+col+';'+sep1+'padding:4px 6px">'+lbl+(m===S.ccM?' ●':'')+'</th>';
  }
  h+='</tr>';
  /* Sub-header row */
  h+='<tr style="background:var(--bg2);position:sticky;top:28px;z-index:15">';
  h+='<th class="p1" style="min-width:220px;text-align:left;border-right:1px solid var(--border2)">Code / WBS</th>';
  h+='<th class="p2" style="min-width:80px;text-align:left;border-right:1px solid var(--border2)">Company</th>';
  h+='<th class="p3" style="min-width:80px;text-align:left;border-right:1px solid var(--border2)">Specialty</th>';
  h+='<th class="p4" style="min-width:62px;'+sep1+'">Total Bud</th>';
  h+='<th style="min-width:55px;border-right:1px solid var(--border2)">Bud M</th>';
  h+='<th style="min-width:55px;border-right:1px solid var(--border2)">Cost M</th>';
  h+='<th style="min-width:52px;'+sep1+'">Var M</th>';
  h+='<th style="min-width:55px;border-right:1px solid var(--border2)">Cum Bud</th>';
  h+='<th style="min-width:55px;border-right:1px solid var(--border2)">Cum Cost</th>';
  h+='<th style="min-width:52px;'+sep1+'">Cum Var</th>';
  h+='<th style="min-width:55px;border-right:1px solid var(--border2)">EAC</th>';
  h+='<th style="min-width:52px;border-right:1px solid var(--border2)">Var EAC</th>';
  h+='<th style="min-width:52px;'+sep1+'">Var EOS</th>';
  /* Monthly columns — 2 cols: HM | Euros */
  for(var m=0;m<NM;m++){
    var lbl=ML[m]?ML[m].short:MS[m];
    var isCur=(m===page); var isPast=(m<page);
    var bg=isCur?'rgba(78,69,128,.15)':'';
    var col=isCur?'var(--pur)':isPast?'var(--tx3)':'#475569';
    var bl=isCur?'border-left:2px solid var(--pur);':'';
    h+='<th colspan="2" style="text-align:center;'+(bg?'background:'+bg+';':'')+';color:'+col+';'+bl+'border-right:3px solid #94a3b8;padding:4px 6px;font-size:10px;font-weight:700">'+lbl+(m===S.ccM?' ●':'')+'</th>';
  }
  h+='</tr>';
  /* Sub-header: HM | Euros */
  h+='<tr style="background:var(--bg2);position:sticky;top:28px;z-index:14">';
  h+='<th colspan="13" style="border-right:3px solid #94a3b8"></th>';
  for(var m=0;m<NM;m++){
    var isCur=(m===page); var isPast=(m<page);
    var bg2=isCur?'background:rgba(78,69,128,.06);':isPast?'background:var(--bg2);':'';
    var bl2=isCur?'border-left:2px solid var(--pur);':'';
    h+='<th style="font-size:9px;font-weight:700;color:var(--blue);'+bg2+bl2+'border-right:1px solid var(--border);min-width:48px">HM</th>';
    h+='<th style="font-size:9px;font-weight:700;color:#1d4ed8;'+bg2+'border-right:3px solid #94a3b8;min-width:55px">Euros</th>';
  }
  h+='</tr>';
  return h;
}
...
function ccGrpRow(g,allW,page){
  var tB=0,tBM=0,tRC=0,tETC=0,tCumBud=0,tCostM=0;
  allW.forEach(function(w){tB+=addBudget(w);tBM+=bud(w,page);tRC+=reelCum(w,page);tETC+=etcHMFromAssign(w.id,page);tCumBud+=budgetCumAtMonth(w,page);tCostM+=reel(w,page);});
  var tEAC=+(tRC+tETC).toFixed(1);
  var tVarM=+(tBM-tCostM).toFixed(1),tCumVar=+(tCumBud-tRC).toFixed(1),tVarEAC=+(tCumBud-tRC).toFixed(1),tVarEOS=+(tB-tEAC).toFixed(1);
  var h='<tr style="background:#dbeafe;font-weight:800;font-size:12px;cursor:pointer" onclick="tog(\'G\',\''+g.id+'\')">';
  h+='<td class="p1" style="border-right:1px solid var(--border2)"><span style="margin-right:6px">'+(g.col?'▶':'▼')+'</span>'+g.name+'</td>';
  h+='<td class="p2" style="border-right:1px solid var(--border2)"></td>';
  h+='<td class="p3" style="border-right:1px solid var(--border2)"></td>';
  h+='<td class="num p4" style="border-right:1px solid var(--border2);color:var(--blue)">'+(tB?tB.toFixed(1):'')+'</td>';
  h+='<td class="num" style="border-right:1px solid var(--border2);color:var(--blue)">'+(tBM?tBM.toFixed(1):'')+'</td>';
  h+='<td class="num" style="border-right:1px solid var(--border2);color:var(--green)">'+(tCostM?tCostM.toFixed(1):'')+'</td>';
  h+='<td class="num '+pncBC(tVarM)+'" style="border-right:1px solid var(--border2);font-weight:700">'+(tB>0||tCostM>0?(tVarM>0?'+':'')+tVarM.toFixed(1):'')+'</td>';
  h+='<td class="num" style="border-right:1px solid var(--border2)">'+(tCumBud?tCumBud.toFixed(1):'')+'</td>';
  h+='<td class="num" style="border-right:1px solid var(--border2)">'+(tRC?tRC.toFixed(1):'')+'</td>';
  h+='<td class="num '+pncBC(tCumVar)+'" style="border-right:1px solid var(--border2);font-weight:700">'+(tCumBud>0||tRC>0?(tCumVar>0?'+':'')+tCumVar.toFixed(1):'')+'</td>';
  h+='<td class="num" style="border-right:1px solid var(--border2)">'+(tEAC?tEAC.toFixed(1):'')+'</td>';
  h+='<td class="num '+pncBC(tVarEAC)+'" style="border-right:1px solid var(--border2);font-weight:700">'+(tB>0?(tVarEAC>0?'+':'')+tVarEAC.toFixed(1):'')+'</td>';
  h+='<td class="num '+pncBC(tVarEOS)+'" style="border-right:2px solid var(--border3);font-weight:800">'+(tB>0?(tVarEOS>0?'+':'')+tVarEOS.toFixed(1):'')+'</td>';
  for(var m=0;m<NM;m++){
    var mB=0,mC=0;allW.forEach(function(w){mB+=bud(w,m);mC+=m<=S.ccM?reel(w,m):rad(w,m);});
    var mVar=+(mB-mC).toFixed(1),vC=mVar>0?'var(--green)':mVar<0?'var(--red)':'var(--tx3)';
    h+='<td class="num" style="border-right:1px solid var(--border);font-size:10px">'
      +'<div style="color:var(--blue)">'+(mB?mB.toFixed(1):'')+'</div>'
      +'<div style="color:var(--green)">'+(mC?mC.toFixed(1):'')+'</div>'
      +'<div style="font-weight:700;color:'+vC+'">'+(mB>0||mC>0?(mVar>0?'+':'')+mVar.toFixed(1):'')+'</div>'
      +'</td>';
    h+='<td class="num" style="border-right:3px solid #94a3b8"></td>';
  }
  h+='</tr>';
  return h;
}
...
function ccSgRow(sg,sgW,page){
  var tB=0,tBM=0,tRC=0,tETC=0,tCumBud=0,tCostM=0;
  sgW.forEach(function(w){tB+=addBudget(w);tBM+=bud(w,page);tRC+=reelCum(w,page);tETC+=etcHMFromAssign(w.id,page);tCumBud+=budgetCumAtMonth(w,page);tCostM+=reel(w,page);});
  var tEAC=+(tRC+tETC).toFixed(1);
  var tVarM=+(tBM-tCostM).toFixed(1),tCumVar=+(tCumBud-tRC).toFixed(1),tVarEOS=+(tB-tEAC).toFixed(1);
  var h='<tr style="background:#eff6ff;font-weight:700;font-size:11px;color:#4338ca;cursor:pointer" onclick="tog(\'SG\',\''+sg.id+'\')">';
  ...
  h+='<td class="p2" style="border-right:1px solid var(--border2)"></td>';
  h+='<td class="p3" style="border-right:1px solid var(--border2)"></td>';
  h+='<td class="num p4" style="border-right:1px solid var(--border2)">'+(tB?tB.toFixed(1):'')+'</td>';
  h+='<td class="num" style="border-right:1px solid var(--border2)">'+(tBM?tBM.toFixed(1):'')+'</td>';
  h+='<td class="num" style="border-right:1px solid var(--border2)">'+(tCostM?tCostM.toFixed(1):'')+'</td>';
  h+='<td class="num '+pncBC(tVarM)+'" style="border-right:1px solid var(--border2)">'+(tB>0||tCostM>0?(tVarM>0?'+':'')+tVarM.toFixed(1):'')+'</td>';
  h+='<td class="num" style="border-right:1px solid var(--border2)">'+(tCumBud?tCumBud.toFixed(1):'')+'</td>';
  h+='<td class="num" style="border-right:1px solid var(--border2)">'+(tRC?tRC.toFixed(1):'')+'</td>';
  h+='<td class="num '+pncBC(tCumVar)+'" style="border-right:1px solid var(--border2)">'+(tCumBud>0||tRC>0?(tCumVar>0?'+':'')+tCumVar.toFixed(1):'')+'</td>';
  h+='<td class="num" style="border-right:1px solid var(--border2)">'+(tEAC?tEAC.toFixed(1):'')+'</td>';
  h+='<td class="num" style="border-right:1px solid var(--border2)"></td>';
  h+='<td class="num '+pncBC(tVarEOS)+'" style="border-right:2px solid var(--border3)">'+(tB>0?(tVarEOS>0?'+':'')+tVarEOS.toFixed(1):'')+'</td>';
  for(var m=0;m<NM;m++){
    var mB=0,mC=0;sgW.forEach(function(w){mB+=bud(w,m);mC+=m<=S.ccM?reel(w,m):rad(w,m);});
    var mVar=+(mB-mC).toFixed(1),vC=mVar>0?'var(--green)':mVar<0?'var(--red)':'var(--tx3)';
    h+='<td class="num" style="border-right:1px solid var(--border);font-size:10px">'
      +'<div style="color:var(--blue)">'+(mB?mB.toFixed(1):'')+'</div>'
      +'<div style="color:var(--green)">'+(mC?mC.toFixed(1):'')+'</div>'
      +'<div style="color:'+vC+';font-weight:700">'+(mB>0||mC>0?(mVar>0?'+':'')+mVar.toFixed(1):'')+'</div>'
      +'</td>';
    h+='<td class="num" style="border-right:3px solid #94a3b8"></td>';
  }
  h+='</tr>';
...
function ccBody(page){
  ...
  for(var gi=0;gi<S.groups.length;gi++){
    var g=S.groups[gi];
    var dw=S.wbs.filter(function(w){return w.parentId===g.id;});
    var allW=dw.slice();
    ...
    if(g.col){h+='<tr class="wbs-sep-row"><td colspan="'+(13+NM*2)+'"></td></tr>'; continue;}
    for(var wi=0;wi<dw.length;wi++) h+=wbsCCRows(dw[wi],1,page,false);
    for(var si=0;si<g.sub.length;si++){
      var sg=g.sub[si];
      var sgW=S.wbs.filter(function(w){return w.parentId===sg.id;});
      if(!sgW.length) continue;
      h+=ccSgRow(sg,sgW,page);
      if(!sg.col) for(var wi=0;wi<sgW.length;wi++) h+=wbsCCRows(sgW[wi],2,page,wi===sgW.length-1);
    }
    h+='<tr style="height:4px"><td colspan="'+(13+NM*2)+'" style="background:var(--bg);border-top:3px solid #8a8070"></td></tr>';
  }
  return h;
}
...
function wbsCCRows(w,lv,page,isLast){
  var ind=lv===1?12:28;
  var bt=addBudget(w);
  var rc2=reelCum(w,page);      /* Cumulated actual */
  var etc=etcHMFromAssign(w.id,page);
  var eac=+(rc2+etc).toFixed(1);
  var varEAC=+(bt-rc2).toFixed(1);    /* Cum Var = Budget - CumCost */
  var varEOS=+(bt-eac).toFixed(1);    /* Var EOS = Total Budget - EAC */
  var budM=bud(w,page);
  var costM=reel(w,page);
  var varM=+(budM-costM).toFixed(1);
  var cumBud=budgetCumAtMonth(w,page);
  var wid=w.id;
  ...
  if(window._wbsIdx===undefined) window._wbsIdx=0;
  var cls=(window._wbsIdx%2===0)?'wbs-even':'wbs-odd';
  window._wbsIdx++;

  /* Budget EUR total */
  var budEurTot=addBudgetEurSlots(w);
  /* Act EUR cum */
  var actEurCum=reelEurCum(w,page);
  /* Add contract billings to EUR */
  var scEurCum=0;
  (w.subcontracts||[]).forEach(function(sc){Object.keys(sc.billing||{}).forEach(function(k){scEurCum+=+(sc.billing[k]||0);});});
  (S.contracts||[]).filter(function(ct){return ct.wbsId===wid;}).forEach(function(ct){
    for(var _m=0;_m<=S.ccM;_m++){if(!contractMonthActive(ct,_m))continue;scEurCum+=ct.billing&&ct.billing[mkey(_m)]!==undefined?+(ct.billing[mkey(_m)]):0;}
  });

  var h='<tr class="rwbs '+cls+'">';
  /* p1: name + responsible */
  h+='<td class="p1" style="padding-left:'+ind+'px;border-right:1px solid var(--border2)">';
  if(w.code)h+='<span class="codb">'+w.code+'</span> ';
  h+='<span style="font-weight:600">'+w.name+'</span>';
  if(w.responsible)h+='<div style="font-size:9px;color:var(--blue)">&#128203; '+w.responsible+'</div>';
  h+='<div style="margin-top:3px;display:flex;flex-direction:column;gap:2px"><span style="font-size:8px;color:var(--tx3);line-height:1.7">Bud</span><span style="font-size:8px;color:var(--tx3);line-height:1.7">Real</span><span style="font-size:8px;color:var(--tx3);line-height:1.7">Var</span></div>';
  /* p2: company */
  h+='</td>';
  ...
  /* Fixed summary cols */
  var pnF=pncBC; /* Budget-Cost: positive=green */
  var _SEP='border-right:3px solid #94a3b8;';
  h+='<td class="num p4" style="'+_SEP+'color:var(--blue)">'+(bt?bt.toFixed(1):'')+'</td>';
  h+='<td class="num" style="border-right:1px solid var(--border2);color:var(--blue)">'+(budM?budM.toFixed(1):'')+'</td>';
  h+='<td class="num" style="border-right:1px solid var(--border2);color:var(--green)">'+(costM?costM.toFixed(1):'')+'</td>';
  h+='<td class="num '+pnF(varM)+'" style="'+_SEP+'font-weight:700">'+(bt>0||costM>0?(varM>0?'+':'')+varM.toFixed(1):'')+'</td>';
  h+='<td class="num" style="border-right:1px solid var(--border2);color:var(--tx3)">'+(cumBud?cumBud.toFixed(1):'')+'</td>';
  h+='<td class="num" style="border-right:1px solid var(--border2);color:var(--green)">'+(rc2?rc2.toFixed(1):'')+'</td>';
  var cumVar=+(cumBud-rc2).toFixed(1);
  h+='<td class="num '+pnF(cumVar)+'" style="'+_SEP+'font-weight:700">'+(cumBud>0||rc2>0?(cumVar>0?'+':'')+cumVar.toFixed(1):'')+'</td>';
  /* EAC = show when there's actual or ETC, not just when bt>0 */
  h+='<td class="num" style="border-right:1px solid var(--border2);color:var(--tx2)">'+(eac>0?eac.toFixed(1):'')+'</td>';
  h+='<td class="num '+pnF(varEAC)+'" style="border-right:1px solid var(--border2)">'+(bt>0||rc2>0?(varEAC>0?'+':'')+varEAC.toFixed(1):'')+'</td>';
  h+='<td class="num '+pnF(varEOS)+'" style="'+_SEP+'font-weight:800">'+(bt>0||eac>0?(varEOS>0?'+':'')+varEOS.toFixed(1):'')+'</td>';

  /* Monthly cells: 2 cols (HM stacked 3 rows | Euros stacked 3 rows) */
  for(var m=0;m<NM;m++){
    var isCur=(m===page); var isPast=(m<S.ccM);
    var bg=isCur?'background:rgba(78,69,128,.06);':isPast?'background:var(--bg2);':'';
    var bl=isCur?'border-left:2px solid var(--pur);':'';
    var bv=bud(w,m); var budEurM=getBudEur(w,m);
    var av,avEur;
    if(m<=S.ccM){
      av=reel(w,m); avEur=getReelEur(w,m);
      ...
      var _hasMR=ccSlot(m).rad[w.id]!==undefined;
      var _asgFut=reelHMFromAssign(w.id,m);
      av=_hasMR?+ccSlot(m).rad[w.id]:_asgFut;
    ...
    var varHM=+(bv-av).toFixed(1); var varEur=+(budEurM-avEur).toFixed(0);
    var vHMc=varHM>0?'var(--green)':varHM<0?'var(--red)':'var(--tx3)';
    var vEUc=varEur>0?'var(--green)':varEur<0?'var(--red)':'var(--tx3)';
    /* HM col: Budget / Realcost / Var stacked */
    var _ccLkM=isCCMonthLocked(m);
    var realHM,realStyle;
    if(m<=S.ccM){ realHM=av?av.toFixed(1):'0'; realStyle='color:'+(m===S.ccM?'var(--pur)':'var(--tx2)')+';font-weight:'+(m===S.ccM?700:400); }
    else{ var _d=ccSlot(m).rad[w.id]!==undefined?+(av-reelHMFromAssign(w.id,m)).toFixed(1):0; realHM=(av?av.toFixed(1):'0')+(_d!==0?'<span style="font-size:8px;color:'+(_d>0?'#15803d':'#b91c1c')+'">'+(  _d>0?'+':'')+_d+'</span>':''); realStyle='color:'+(ccSlot(m).rad[w.id]!==undefined?'#c2410c':'var(--tx3)')+';font-style:'+(ccSlot(m).rad[w.id]!==undefined?'normal':'italic'); }
    var hmClickAttr=m>S.ccM&&!_ccLkM?' onclick="ecCCCell(this,\''+wid+'\','+m+',\'rad\',' +av+')" style="cursor:pointer;text-decoration:underline dotted;'+realStyle+'"':' style="'+realStyle+'"';
    /* HM col with labels: Budget / Realcost / Var */
    var hmCol='<div style="text-align:right;line-height:1.7;font-size:10px;color:var(--blue)">'+(bv?bv.toFixed(1):'')+'</div>'
      +'<div style="text-align:right;line-height:1.7;font-size:10px"'+hmClickAttr+'>'+realHM+'</div>'
      +'<div style="text-align:right;line-height:1.7;font-size:10px;font-weight:700;color:'+vHMc+'">'+(bv>0||av>0?(varHM>0?'+':'')+varHM.toFixed(1):'')+'</div>';
    /* EUR col with labels: Budget / Realcost / Var */
    var _eurV2=getReelEur(w,m);
    var _eurCA=m<=S.ccM&&!_ccLkM?' onclick="ecCCCell(this,\''+wid+'\','+m+',\'cc-reel-eur\',' +_eurV2+')" style="cursor:pointer;text-decoration:underline dotted;color:#15803d;font-size:10px"':' style="color:'+(m>S.ccM?'#1d4ed8':'#15803d')+';font-size:10px;font-style:'+(m>S.ccM?'italic':'normal')+'"';
    var eurCol='<div style="text-align:right;line-height:1.7;font-size:9px;color:#1d4ed8">'+(budEurM?fkv(budEurM):'')+'</div>'
      +'<div style="text-align:right;line-height:1.7;font-size:9px"><span'+_eurCA+'>'+(avEur?fkv(avEur):'')+'</span></div>'
      +'<div style="text-align:right;line-height:1.7;font-size:9px;font-weight:700;color:'+vEUc+'">'+(budEurM>0||avEur>0?(varEur>0?'+':'')+fkv(varEur):'')+'</div>';
    h+='<td style="text-align:right;padding:2px 5px;vertical-align:top;'+bl+bg+'border-right:1px solid var(--border)">'+hmCol+'</td>';
    h+='<td style="text-align:right;padding:2px 5px;vertical-align:top;border-right:3px solid #94a3b8">'+eurCol+'</td>';
  }
  h+='</tr>';
  ...
  (w.subcontracts||[]).forEach(function(sc){
    ...
    h+='<tr class="'+cls+'"><td class="p1" style="padding-left:'+(ind+10)+'px;border-right:1px solid var(--border2);background:#eff6ff;font-size:10px;color:var(--blue)" colspan="13">&#128196; <strong>'+sc.ref+'</strong>'+(sc.company?' — '+sc.company:'')+' &nbsp;·&nbsp; Bud: '+fkv(sc.amt)+'€ &nbsp;·&nbsp; Fact: '+fkv(scBilled)+'€</td>';
    for(var m=0;m<NM;m++){
      var k2=mkey(m);var sv=+(sc.billing[k2]||0);
      var cls2=m<S.ccM?'col-past-td':m===S.ccM?'col-cur-td':'col-rad-td';
      var _scLk=isCCMonthLocked(m);var _scPast=m<S.ccM;
      h+='<td colspan="2" style="text-align:right;padding:3px 6px;font-family:var(--mono);font-size:10px;background:#eff6ff;border-right:3px solid #94a3b8;color:var(--blue);'+(!_scLk&&!_scPast?'cursor:pointer':'')+'"';
      if(!_scLk&&!_scPast) h+=' onclick="openScBillingCell(this,this.dataset.wid,this.dataset.scid,this.dataset.k,'+sv+')" data-wid="'+w.id+'" data-scid="'+sc.id+'" data-k="'+k2+'"';
      h+='>'+(sv?fkv(sv):(m>=S.ccM?'<span style="color:var(--border2);font-size:9px">+</span>':''))+'</td>';
    }
    h+='</tr>';
  });
  (S.contracts||[]).filter(function(ct){return ct.wbsId===wid;}).forEach(function(ct){
    var _ctBud=ct.totalAmount>0?ct.totalAmount:+(ct.monthlyForecast*contractActiveMonths(ct)).toFixed(0);
    var _cSpent=0;for(var _m=0;_m<=S.ccM;_m++){if(!contractMonthActive(ct,_m))continue;_cSpent+=ct.billing&&ct.billing[mkey(_m)]!==undefined?+(ct.billing[mkey(_m)]):0;}
    var _cFcastRem=0;for(var _m=S.ccM+1;_m<NM;_m++){if(!contractMonthActive(ct,_m))continue;var _mk=mkey(_m);_cFcastRem+=ct.billing&&ct.billing[_mk]!==undefined?+(ct.billing[_mk]):+(ct.adjustedForecast||ct.monthlyForecast);}
    var _cProj=_cSpent+_cFcastRem;
    var _cOver=_ctBud>0&&_cProj>_ctBud;
    var alertBadge=_cOver?'<span style="background:#fee2e2;color:#b91c1c;font-weight:700;padding:1px 6px;border-radius:4px;margin-left:6px">⚠ +'+fkv(_cProj-_ctBud)+'€</span>':(_ctBud>0?'<span style="background:#dcfce7;color:#15803d;padding:1px 6px;border-radius:4px;margin-left:6px">✓ reste '+fkv(_ctBud-_cProj)+'€</span>':'');
    h+='<tr class="'+cls+'"><td class="p1" style="padding-left:'+(ind+10)+'px;border-right:1px solid var(--border2);background:#faf5ff;font-size:10px;color:var(--pur)" colspan="13">&#128196; <strong>'+ct.ref+'</strong>'+(ct.company?' — '+ct.company:'')+(_ctBud?' &nbsp;·&nbsp; Bud: '+fkv(_ctBud)+'€':'')+' &nbsp;·&nbsp; Dép: '+fkv(_cSpent)+'€'+alertBadge+'</td>';
    for(var m=0;m<NM;m++){
      if(!contractMonthActive(ct,m)){h+='<td colspan="3" style="border-right:2px solid var(--border2)"></td>';continue;}
      var _ck=mkey(m);var _cIsReal=ct.billing&&ct.billing[_ck]!==undefined;
      var _cFcast=+(ct.adjustedForecast||ct.monthlyForecast);
      var _cBilled=_cIsReal?+(ct.billing[_ck]):0;
      var _cDisp=m<=S.ccM?(_cIsReal?_cBilled:0):(ct.billing&&ct.billing[_ck]!==undefined?_cBilled:_cFcast);
      var _ccLkM=isCCMonthLocked(m);var _isPast=m<S.ccM;
      h+='<td colspan="2" style="text-align:right;padding:3px 6px;font-family:var(--mono);font-size:10px;background:#faf5ff;border-right:3px solid #94a3b8;color:'+(_cIsReal?'#15803d':'var(--tx3)')+(_cDisp&&m>S.ccM?';font-style:italic':'')+(!_ccLkM&&!_isPast?';cursor:pointer':'')+'"';
      if(!_ccLkM&&!_isPast) h+=' onclick="openContractCellEdit(this)" data-cid="'+ct.id+'" data-m="'+m+'" data-v="'+_cDisp+'"';
      h+='>'+(_cDisp?fkv(_cDisp):(m>=S.ccM?'<span style="color:var(--border2);font-size:9px">+</span>':''))+'</td>';
    }
    h+='</tr>';
  });
  return h;
...
function reel_all(page){var t=0;S.wbs.forEach(function(w){t+=reel(w,page);});return t;}
function ccGrandTotalRow(page){
  var tB=0,tBM=0,tRC=0,tETC=0,tCumBud=0;
  S.wbs.forEach(function(w){
    tB+=addBudget(w);tBM+=bud(w,page);tRC+=reelCum(w,page);
    tETC+=etcHMFromAssign(w.id,page);tCumBud+=budgetCumAtMonth(w,page);
  });
  var tEAC=+(tRC+tETC).toFixed(1);
  var tVarM=+(tBM-reel_all(page)).toFixed(1);
  var tCumCost=tRC;
  var tCumVar=+(tCumBud-tCumCost).toFixed(1);
  var tVarEAC=+(tCumBud-tRC).toFixed(1);
  var tVarEOS=+(tB-tEAC).toFixed(1);
  var tCostM=0; S.wbs.forEach(function(w){tCostM+=reel(w,page);});
  tVarM=+(tBM-tCostM).toFixed(1);

  var h='<tr style="background:#1d4ed8;color:#fff;font-weight:800;font-size:12px">';
  h+='<td class="p1" style="border-right:1px solid #1a3a9e">PROJECT TOTAL</td>';
  h+='<td class="p2" style="border-right:1px solid #1a3a9e"><span style="background:#e0f2fe;color:#0369a1;font-size:9px;padding:2px 6px;border-radius:4px">Budget</span></td>';
  h+='<td class="p3" style="border-right:1px solid #1a3a9e"></td>';
  h+='<td class="num p4" style="border-right:1px solid #1a3a9e">'+tB.toFixed(1)+'</td>';
  h+='<td class="num" style="border-right:1px solid #1a3a9e">'+tBM.toFixed(1)+'</td>';
  h+='<td class="num" style="border-right:1px solid #1a3a9e">'+tCostM.toFixed(1)+'</td>';
  var vmC=tVarM>=0?'#86efac':'#fca5a5';
  h+='<td class="num" style="border-right:1px solid #1a3a9e;color:'+vmC+'">'+(tVarM>0?'+':'')+tVarM.toFixed(1)+'</td>';
  h+='<td class="num" style="border-right:1px solid #1a3a9e">'+tCumBud.toFixed(1)+'</td>';
  h+='<td class="num" style="border-right:1px solid #1a3a9e">'+tRC.toFixed(1)+'</td>';
  var cvC=tCumVar>=0?'#86efac':'#fca5a5';
  h+='<td class="num" style="border-right:1px solid #1a3a9e;color:'+cvC+'">'+(tCumVar>0?'+':'')+tCumVar.toFixed(1)+'</td>';
  h+='<td class="num" style="border-right:1px solid #1a3a9e">'+tEAC.toFixed(1)+'</td>';
  var veC=tVarEAC>=0?'#86efac':'#fca5a5';
  h+='<td class="num" style="border-right:1px solid #1a3a9e;color:'+veC+'">'+(tVarEAC>0?'+':'')+tVarEAC.toFixed(1)+'</td>';
  var vosC=tVarEOS>=0?'#86efac':'#fca5a5';
  h+='<td class="num" style="border-right:2px solid #1a3a9e;color:'+vosC+'">'+(tVarEOS>0?'+':'')+tVarEOS.toFixed(1)+'</td>';
  for(var m=0;m<NM;m++){
    var mB=0,mC=0,mEUR_B=0,mEUR_C=0;
    S.wbs.forEach(function(w){mB+=bud(w,m);mC+=m<=S.ccM?reel(w,m):rad(w,m);mEUR_B+=getBudEur(w,m);mEUR_C+=m<=S.ccM?getReelEur(w,m):Math.round((m<=S.ccM?reel(w,m):rad(w,m))*26*8*(w.pu||0));});
    var mVar=+(mB-mC).toFixed(1);
    var vC=mVar>=0?'#86efac':'#fca5a5';
    var mVarEur=+(mEUR_B-mEUR_C).toFixed(0); var vEC2=mVarEur>=0?'#86efac':'#fca5a5';
    h+='<td class="num" style="border-right:1px solid #1a3a9e;font-size:10px">'
      +'<div style="color:#93c5fd">'+(mB?mB.toFixed(1):'')+'</div>'
      +'<div style="color:#fff">'+(mC?mC.toFixed(1):'')+'</div>'
      +'<div style="font-weight:700;color:'+vC+'">'+(mB>0||mC>0?(mVar>0?'+':'')+mVar.toFixed(1):'')+'</div>'
      +'</td>';
    h+='<td class="num" style="border-right:3px solid #1a3a9e;font-size:9px">'
      +'<div style="color:#93c5fd">'+(mEUR_B?fkv(mEUR_B):'')+'</div>'
      +'<div style="color:#86efac">'+(mEUR_C?fkv(mEUR_C):'')+'</div>'
      +'<div style="font-weight:700;color:'+vEC2+'">'+(mEUR_B>0||mEUR_C>0?(mVarEur>0?'+':'')+fkv(mVarEur):'')+'</div>'
      +'</td>';
  }
  h+='</tr>';
  return h;
