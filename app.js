...
function renderTabBar(){
  ...
  h+='<div class="tab'+(VIEW==="contracts"?" active":"")+'" onclick="goContracts()" style="color:var(--pur)">&#128196; Contracts ('+(S.contracts?S.contracts.length:0)+')</div>';  
...
function wbsCCRows(w,lv,page,isLast){
  var ind=lv===1?16:36; var bt=addBudget(w); var rc2=reelCum(w,page); var va=+(bt-rc2).toFixed(1); /* Budget-Cost: positive=under budget=green */
  ...
  h+='<td class="p4 num" style="color:var(--blue);border-right:1px solid var(--border2)">'+budMonth.toFixed(1)+'</td>';
  h+='<td class="p5 num" style="color:var(--green);font-weight:700;border-right:1px solid var(--border2)">'+rc2.toFixed(1)+'</td>';
  h+='<td class="num '+pncBC(va)+'" style="font-weight:700;border-right:1px solid var(--border2)">'+(va>0?'+':'')+va.toFixed(1)+'</td>';
  ...
  h+='<td class="p2" style="border-right:1px solid var(--border2)"></td><td class="p3" style="border-right:1px solid var(--border2)"></td><td class="p4" style="border-right:2px solid var(--border2)"></td>';
  h+='<td class="p5 num" style="color:var(--blue);font-weight:700;border-right:1px solid var(--border2)">'+bt.toFixed(1)+'</td><td style="border-right:1px solid var(--border2)"></td><td style="border-right:2px solid var(--border3)"></td>';
  ...
  h+='</tr>';
  /* Ref Budget EUR - only if EUR rows visible */
  if(_showEurRows){
  ...
  h+='<td class="p2" style="border-right:1px solid var(--border2)"></td><td class="p3" style="border-right:1px solid var(--border2)"></td><td class="p4" style="border-right:2px solid var(--border2)"></td>';
  h+='<td class="p5 num" style="color:var(--green);font-weight:700;border-right:1px solid var(--border2)">'+rc2.toFixed(1)+'</td><td style="border-right:1px solid var(--border2)"></td><td style="border-right:2px solid var(--border3)"></td>';
  for(var m=0;m<NM;m++){
    var cv=ccVal(w,m,page); var cls=m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td';
    var wid=w.id;
    if(cv.type==='past'){
      /* Past month actual = directly from assignments, read-only */
      h+='<td class="'+cls+' num" style="color:var(--tx2)">'+f1(cv.v)+'</td>';
    } else if(cv.type==='cur'){
      /* CC month actual = from assignments, read-only (shown in purple) */
      var dv=cv.v?cv.v.toFixed(1):'0';
      h+='<td class="'+cls+' num" style="color:var(--pur);font-weight:700">'+dv+'</td>';
    } else {
      /* Future month ETC — default = assignments, manual override shows delta */
      var hasManualRad=ccSlot(m).rad[w.id]!==undefined;
      var _assignedFut=reelHMFromAssign(w.id,m);
      var _radDisplayVal=hasManualRad?+ccSlot(m).rad[w.id]:_assignedFut;
      var dv2=_radDisplayVal?_radDisplayVal.toFixed(1):'0';
      var _radDelta=hasManualRad?+(_radDisplayVal-_assignedFut).toFixed(1):0;
      var _radDeltaStr=(_radDelta!==0)?'<span style="font-size:9px;color:'+(_radDelta>0?'#15803d':'#b91c1c')+'"> ('+(_radDelta>0?'+':'')+_radDelta+')</span>':'';
      var radStyle=hasManualRad?'color:var(--ora);font-weight:700':'color:var(--tx2);font-style:italic';
      var _ccMonthLocked=isCCMonthLocked(m);
      if(_ccMonthLocked){
        h+='<td class="'+cls+' num" style="'+radStyle+';background:var(--ambl)!important" title="CC locked">'+dv2+_radDeltaStr+' &#128274;</td>';
      } else {
        h+='<td class="'+cls+' edt num" style="'+radStyle+'" data-wid="'+wid+'" data-key="r'+m+'" data-type="cc-rad" data-m="'+m+'" onclick="ecCC(this,\''+wid+'\','+m+',\'rad\','+_radDisplayVal+')">'+dv2+_radDeltaStr+'</td>';
      }
    }
  }
  h+='</tr>';
  ...
  h+='<td class="p2" style="border-right:1px solid var(--border2)"></td><td class="p3" style="border-right:1px solid var(--border2)"></td><td class="p4" style="border-right:2px solid var(--border2)"></td>';
  h+='<td class="num '+pncBC(va)+'" style="font-weight:700;border-right:1px solid var(--border2)">'+(va>0?'+':'')+va.toFixed(1)+'</td><td style="border-right:1px solid var(--border2)"></td><td style="border-right:2px solid var(--border3)"></td>';
  for(var m=0;m<NM;m++){var bv=bud(w,m);var rv2=m<=page?reel(w,m):rad(w,m);var va2=+(bv-rv2).toFixed(1);/* Budget-Cost */var cls=m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td';h+='<td class="'+cls+' num '+pncBC(va2)+'">'+(va2!==0?(va2>0?'+':'')+va2.toFixed(1):'')+'</td>';}
  h+='</tr>';

  /* ── Consommé EUR row — only shown if EUR toggle is on ── */
  if(_showEurRows){
  ...
  /* ── Subcontract rows in CC ── */
  if(w.subcontracts&&w.subcontracts.length>0){
    w.subcontracts.forEach(function(sc){
      var scBilled=0;
      Object.keys(sc.billing||{}).forEach(function(k){scBilled+=+(sc.billing[k]||0);});
      var scMonthVal=+(sc.billing[mkey(page)]||0);
      var scReste=+(sc.amt-scBilled).toFixed(0);
      h+='<tr class="'+wbsCls+'"><td class="p1" colspan="6" style="padding:3px 6px 3px '+(ind+12)+'px;border-right:2px solid var(--border2);background:#f0f4ff">';
      h+='<span style="font-size:10px;color:var(--pur)">&#128196; <strong>'+sc.ref+'</strong>';
      if(sc.company) h+=' — '+sc.company;
      h+=' &nbsp;·&nbsp; Budget: <strong>'+sc.amt.toLocaleString()+' €</strong>';
      h+=' &nbsp;·&nbsp; Facturé: <strong>'+scBilled.toLocaleString()+' €</strong>';
      h+=' &nbsp;·&nbsp; Reste: <strong style="color:'+(scReste<0?'var(--red)':'var(--green)')+'">'+(scReste<0?'':'')+scReste.toLocaleString()+' €</strong>';
      h+='</span></td>';
      /* Monthly billing cells */
      for(var m=0;m<NM;m++){
        var k2=mkey(m);
        var sv=+(sc.billing[k2]||0);
        var cls2=m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td';
        var _scLocked=isCCMonthLocked(m);
        if(m<=S.ccM&&!_scLocked){
          h+='<td class="'+cls2+' edt num" style="color:var(--pur);font-size:10px;cursor:pointer" onclick="openScBillingCell(this,this.dataset.wid,this.dataset.scid,this.dataset.k,'+sv+')" data-wid="'+w.id+'" data-scid="'+sc.id+'" data-k="'+k2+'">'+(sv?sv.toLocaleString():'')+'</td>';
        } else {
          h+='<td class="'+cls2+' lk num" style="color:var(--pur);font-size:10px">'+(sv?sv.toLocaleString():'')+'</td>';
        }
      }
      h+='</tr>';
    });
  }
  /* ── Global contracts attached to this WBS ── */
  (S.contracts||[]).filter(function(c){return c.wbsId===wid;}).forEach(function(c){
    var _isCCLk=isCCMonthLocked(page);
    /* Contract summary row */
    var _cbTotal=0;Object.keys(c.billing||{}).forEach(function(k){_cbTotal+=+(c.billing[k]||0);});
    var _cnM=contractActiveMonths(c);
    var _ctBud=+(c.monthlyForecast*_cnM).toFixed(0);
    var _cEAC=contractEAC(c);
    var _cOver=_cEAC>_ctBud&&_ctBud>0;
    var _cExh=contractExhaustDate(c);
    h+='<tr class="'+wbsCls+'"><td class="p1" colspan="7" style="padding:3px 6px 3px '+(ind+12)+'px;border-right:2px solid var(--border3);background:#faf5ff">';
    h+='<span style="font-size:10px;color:var(--pur)">&#128196; <strong>'+c.ref+'</strong>';
    if(c.company) h+=' — '+c.company;
    h+=' &nbsp;·&nbsp; Budget: <strong>'+_ctBud.toLocaleString()+' €</strong>';
    h+=' &nbsp;·&nbsp; EAC: <strong style="color:'+(_cOver?'var(--red)':'var(--green)')+'">'+_cEAC.toLocaleString()+' €</strong>';
    if(_cOver&&_cExh) h+=' &nbsp;·&nbsp; <span style="color:var(--red);font-weight:700">⚠ Budget exhausted: '+_cExh+'</span>';
    h+='</span></td>';
    /* Monthly cells */
    for(var m=0;m<NM;m++){
      if(!contractMonthActive(c,m)){
        var _ccls=m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td';
        h+='<td class="'+_ccls+'"></td>'; continue;
      }
      var _ck=mkey(m);
      var _cActual=m<=S.ccM?+(c.billing[_ck]||0):null;
      var _cFcast=+(c.adjustedForecast||c.monthlyForecast);
      var _cDisp=_cActual!==null?_cActual:_cFcast;
      var _ccls2=m<page?'col-past-td':m===page?'col-cur-td':'col-rad-td';
      var _cIsReal=_cActual!==null&&c.billing[_ck]!==undefined;
      var _cStyle='color:'+(_cIsReal?'var(--green)':'var(--tx3)')+';font-size:10px;';
      if(m<=S.ccM&&!_ccLk){
        h+='<td class="'+_ccls2+' edt num" style="'+_cStyle+';cursor:pointer" onclick="openContractCellEdit(this)" data-cid="'+c.id+'" data-m="'+m+'" data-v="'+(_cActual||0)+'">'+(_cDisp?_cDisp.toLocaleString():'')+'</td>';
      } else {
        h+='<td class="'+_ccls2+' num" style="'+_cStyle+'">'+(_cDisp?_cDisp.toLocaleString():'')+'</td>';
      }
    }
    h+='</tr>';
  });
...
function wkEditSelected(){
  ...
  h+='<div class="ma"><button class="btn" onclick="closeModal()">Cancel</button><button class="btn pri" onclick="wkApplyBulkEdit('+JSON.stringify(ids)+')">Apply to '+ids.length+' workers</button></div>';
  box.innerHTML=h;
  document.getElementById('mbg').style.display='flex';
}

function wkApplyBulkEdit(ids){
...
function wkEditResponsible(e, wkId){
  ...
  /* Build unique list of existing responsibles */
  var uniq = [];
...
function renderAssignmentsPage(){
  ...
  function asTotalRow(wbsList, label, indent, bgColor, textColor, startCol, nCols, page){
    ...
    for(var m = startCol; m < NM; m++){
      ...
      var dispStr = '';
      if(need > 0 || total > 0){
        var col = total >= need ? '#15803d' : total > 0 ? '#c2410c' : '#b91c1c';
  ...
  for(var m=startCol;m<NM;m++){
    ...
    var ptDisp='';
    if(_ptN>0||_ptW>0){
      var ptCol=_ptW>=_ptN?'#15803d':_ptW>0?'#c2410c':'#b91c1c';
  ...
  for(var gi = 0; gi < S.groups.length; gi++){
    ...
    /* Group total row (header) */
    html += asTotalRow(allGroupWBS, g.name.toUpperCase(), 12, '#d8d2c8', '#2d2926', startCol, nCols, page);
    /* Collapse toggle — inject into that row via a workaround: add onclick to td */
    /* Actually rewrite the total row for group to include toggle */
    /* Simpler: pop and rewrite */
    /* Re-do group header with toggle */
    html = html.slice(0, html.lastIndexOf('<tr style="background:#d8d2c8'));
    ...
    var _grH = '<tr style="background:#d8d2c8;cursor:pointer;" onclick="tog(\'G\',\'' + g.id + '\')">';
    _grH += '<td style="position:sticky;left:0;z-index:5;background:#d8d2c8;padding:6px 12px;font-weight:800;font-size:12px;letter-spacing:.01em;text-transform:uppercase;border-bottom:2px solid var(--border3);white-space:nowrap;">';
    ...
    _grH += '<td style="position:sticky;left:220px;z-index:5;background:#d8d2c8;padding:3px 6px;font-family:var(--mono);font-size:10px;font-weight:700;text-align:right;border-bottom:2px solid var(--border3);line-height:1.6;">'
    ...
    for(var m = startCol; m < NM; m++){
      ...
      var gDisp = '';
      if(_gn > 0 || _gt > 0){
        var gCol = _gt >= _gn ? '#15803d' : _gt > 0 ? '#c2410c' : '#b91c1c';
        gDisp = '<span style="color:' + gCol + ';font-weight:800">' + _gt + '</span>';
        if(_gn > 0) gDisp += '<span style="color:rgba(0,0,0,.4);font-weight:400">/</span><span style="color:rgba(0,0,0,.6);font-weight:600">' + _gn + '</span>';
      }
    ...
    for(var si = 0; si < g.sub.length; si++){
      ...
      var _sgH = '<tr style="background:#e2ddd4;cursor:pointer;" onclick="tog(\'SG\',\'' + sg.id + '\')">';
      _sgH += '<td style="position:sticky;left:0;z-index:5;background:#e2ddd4;padding:5px 12px 5px 24px;font-weight:800;font-size:12px;color:#4a3a80;border-bottom:1px solid var(--border3);white-space:nowrap;">';
      ...
      _sgH += '<td style="position:sticky;left:220px;z-index:5;background:#e2ddd4;padding:3px 6px;font-family:var(--mono);font-size:10px;font-weight:700;text-align:right;border-bottom:1px solid var(--border3);line-height:1.6;">'
      ...
      for(var m = startCol; m < NM; m++){
        ...
        var sgDisp = '';
        if(_sn > 0 || _st > 0){
          var sgCol = _st >= _sn ? '#15803d' : _st > 0 ? '#c2410c' : '#b91c1c';
          sgDisp = '<span style="color:' + sgCol + ';font-weight:800">' + _st + '</span>';
          if(_sn > 0) sgDisp += '<span style="color:rgba(74,58,128,.4);font-weight:400">/</span><span style="color:rgba(74,58,128,.8);font-weight:600">' + _sn + '</span>';
        }
...
function asWBSRow(wbs, ind, startCol, page, nCols){
  ...
  for(var m = startCol; m < NM; m++){
    ...
    if(budHM > 0){
      ...
      countHtml = '<div style="font-family:var(--mono);font-size:12px;font-weight:700;color:var(--green);text-align:center;">' + nW + '</div>';
    } else {
      /* No budget, no workers — show dash only if month is within project range */
      countHtml = '';
    }
...
function renderContractsPage(){
  ...
  h+='<th style="text-align:right;padding:7px 10px;border:1px solid var(--border2);min-width:100px">Total budget</th>';
  ...
  S.contracts.forEach(function(c){
    ...
    var totalBudget=+(c.monthlyForecast*nMonths).toFixed(0);
    ...
    h+='<td style="padding:6px 10px;border:1px solid var(--border2);text-align:right;font-family:var(--mono)">'+totalBudget.toLocaleString()+' €</td>';
...
function openContractModal(id){
  ...
  h+='<div><label>Company</label><input id="cCo" value="'+(c?c.company:'')+'" placeholder="Subcontractor name"></div>';
  h+='<div><label>Start Date</label><input id="cStart" type="month" value="'+(c?c.startDate:'')+'" ></div>';
  h+='<div><label>End Date</label><input id="cEnd" type="month" value="'+(c?c.endDate:'')+'" ></div>';
  h+='<div><label>Monthly Cost Forecast (€)</label><input id="cFcast" type="number" step="500" value="'+(c?c.monthlyForecast:'')+'" placeholder="0"></div>';
  h+='<div><label>Attach to WBS</label><select id="cWBS">'+wbsOpts+'</select></div>';
  h+='</div>';
...
function saveContract(id){
  ...
  if(id){
    for(var i=0;i<S.contracts.length;i++) if(S.contracts[i].id===id){
      var c=S.contracts[i];
      c.ref=ref;c.company=co;c.startDate=start;c.endDate=end;c.monthlyForecast=fcast;c.wbsId=wbsId;
      break;
    }
  } else {
    S.contracts.push({id:uid(),ref:ref,company:co,startDate:start,endDate:end,monthlyForecast:fcast,wbsId:wbsId,billing:{},adjustedForecast:fcast});
  }
...
function printAssignments(){
  ...
  win.document.write('th{background:#f1f5f9;font-weight:700;font-size:9px;text-transform:uppercase;}');
  win.document.write('.n-grp{background:#d8d2c8!important;font-weight:800;font-size:11px;}');
  win.document.write('.n-sg{background:#e2ddd4!important;font-weight:700;color:#4a3a80;}');
...
function renderCCByResponsible(page){
  ...
  Object.keys(respMap).sort().forEach(function(resp){
    ...
    rows+='<tr>'
      ...
      +'<td style="text-align:right;padding:6px 10px;border:1px solid #e2e8f0;font-family:monospace;color:#c2410c">'+r.depPrevue.toFixed(1)+'</td>'
...
function openContractCellEdit(td){
  ...
  inp.addEventListener('keydown',function(e){
    ...
    if(e.key==='Escape'){saved=true;td.textContent=cur?cur.toLocaleString():'';}
  });
}
