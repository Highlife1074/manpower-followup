...
function wbsCCRows(w,lv,page,isLast){
  ...
  (w.subcontracts||[]).forEach(function(sc){
    ...
    for(var m=0;m<NM;m++){
      ...
      if(m<=S.ccM&&!_scLk){
        h+='<td class="'+cls2+' edt num" style="color:var(--blue);font-size:10px;cursor:pointer" onclick="openScBillingCell(this,this.dataset.wid,this.dataset.scid,this.dataset.k,'+sv+')" data-wid="'+w.id+'" data-scid="'+sc.id+'" data-k="'+k2+'">'+(sv?fkv(sv):'')+'</td>';
      } else {
  ...
  (S.contracts||[]).filter(function(ct){return ct.wbsId===wid;}).forEach(function(ct){
    ...
    for(var m=0;m<NM;m++){
      ...
      if(!_ccLkM && !_isPast){
        ...
        h+='<td class="'+_ccls2+' edt num" style="'+_cSty+_overBg+';cursor:pointer'+(m>S.ccM?';font-style:italic':'')+'" onclick="openContractCellEdit(this)" data-cid="'+ct.id+'" data-m="'+m+'" data-v="'+(_cDisp||0)+'">'+(_cDisp?fkv(_cDisp):'')+'</td>';
      } else {
