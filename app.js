...
function reelAtSlot(w,i){
  ...
  return reelHMFromAssign(w.id,i) + getOpenPositions(w.id,i);
}
...
function renderTabBar(){
  ...
  h+='<div class="tab'+(VIEW==="workforce"?" active":"")+' " onclick="switchTab(\'workforce\')">&#128200; Workforce</div>';
...
function ccHead(page){
  ...
  h+='<th colspan="5" style="text-align:left;padding:4px 8px;border-right:'+SEP+'">WBS &amp; Budget</th>';
...
function wbsCCRows(w,lv,page){
  ...
  var eac=0;
  for(var _mf=page+1;_mf<NM;_mf++) eac+=radAtSlot(w,_mf);
  eac=+eac.toFixed(1);
  ...
  if(hasSubRows){
    ...
    /* Monthly recap cells */
    for(var m=0;m<NM;m++){
      var bvT=budTotal(w,m); var avT=m<=S.ccM?reel(w,m):(radAtSlot(w,m));
      ...
      var _avStyle=m===S.ccM?'color:var(--pur);font-weight:700':m<S.ccM?'color:var(--tx2)':'color:var(--tx2);font-style:italic';
    ...
    var eacs=0; for(var _mf=page+1;_mf<NM;_mf++) eacs+=radAtSlot(w,_mf);
    eacs=+eacs.toFixed(1);
    ...
    for(var m=0;m<NM;m++){
      ...
      if(m<S.ccM){ av=reel(w,m); avStyle='color:var(--tx2)'; avEur=getReelEur(w,m); }
        ...
        var asgCC=reelHMFromAssign(w.id,m)+getOpenPositions(w.id,m);
        av=asgCC; /* CC read-only: always assignments + open positions, ignore legacy overrides */
        ...
        var asgFut=reelHMFromAssign(w.id,m)+getOpenPositions(w.id,m);
        av=asgFut; /* CC read-only: always assignments + open positions */
        avStyle='color:var(--tx2);font-style:italic';
        var delta=0;
      ...
      var editable=false; /* CC fully read-only — use Assignment tab */
      var _ecType='rad'; /* only future ETC overrides allowed */
...
function setCCSupervisor(wbsId,m,workerId){
  ...
  var _savedView=VIEW; /* preserve current tab */
  saveS();
  /* Re-render current tab only */
  VIEW=_savedView;
  if(_savedView==='assignments') renderAssignmentsPage();
  else if(_savedView==='orgchart') renderOrgChartPage();
  else if(_savedView==='cc'||typeof _savedView==='number') renderCCPage(S.ccM);
  /* else stay wherever we are */
}
...
function renderAssignmentsPage(){
  ...
  html += '<button class="btn" style="font-size:11px;display:inline-flex;color:var(--blue);border-color:var(--blue)" onclick="openPositionDialog()">&#9711; Open Pos.</button>';
  ...
  function asTotalRow(wbsList, label, indent, bgColor, textColor, startCol, nCols, page){
    ...
    for(var m = startCol; m < NM; m++){
      ...
      var tdBg = isCur ? 'rgba(78,69,128,.08)' : isPast ? 'var(--bg2)' : '#fffbf0';
...
function asWBSRow(wbs, ind, startCol, page, nCols){
  ...
  for(var m = startCol; m < NM; m++){
    ...
    var alertStyle = '';
    if(isSel){
      alertStyle = 'background:rgba(61,90,128,.18)!important;outline:2px solid var(--blue)!important;';
    }
    ...
    /* Supervisor + FM always computed regardless of budget */
    var _hasFm=!!getForeman(wbs.id,m);
    var _openP=getOpenPositions(wbs.id,m);
    ...
    var countHtml = '';
    if(budHM > 0 || _supObjCell || _hasFm || _openP > 0){
      ...
      var _totalW=nW+_openP;
      if(nW > 0 || budHM > 0){
        countHtml += '<span style="color:' + (_totalW >= need ? 'var(--green)' : (_totalW>0?'#c2410c':'var(--red)')) + ';">' + nW + '</span>';
      }
      if(_openP > 0){
        countHtml += '<div style="color:var(--blue);font-size:9px;font-weight:700">◯ '+_openP+'</div>';
      }
...
function showMultiCellPopup(e, pairs){
  ...
  /* Use modal instead of floating div — avoids popup-closing-on-dropdown-click */
  window._multiPairs=pairs;
  openMultiAssignModal(pairs);
}

function openMultiAssignModal(pairs){
  var wbsNames=[];
  pairs.forEach(function(p){
    var w=S.wbs.find(function(x){return x.id===p.wbsId;});
    var lbl=(w?w.name:p.wbsId)+' '+(ML[p.m]?ML[p.m].short:'m'+p.m);
    if(wbsNames.indexOf(lbl)<0) wbsNames.push(lbl);
  });
  
  var h='<h3>&#128203; '+pairs.length+' cell'+(pairs.length>1?'s':'')+' selected</h3>';
  h+='<p style="font-size:10px;color:var(--tx3);margin-bottom:12px">'+wbsNames.slice(0,5).join(' · ')+(wbsNames.length>5?' …':'')+'</p>';
  
  /* Supervisor — searchable list */
  h+='<div style="margin-bottom:14px">';
  h+='<label style="font-size:11px;font-weight:700">&#128119; Supervisor</label>';
  h+='<input id="supSearch" placeholder="Search supervisor…" oninput="filterMultiList(\'supList\',this.value)" style="width:100%;margin:4px 0;font-size:11px">';
  h+='<div id="supList" style="max-height:150px;overflow-y:auto;border:1px solid var(--border2);border-radius:6px">';
  h+='<div style="padding:6px 10px;cursor:pointer;background:#fff8;border-bottom:1px solid var(--border2)" onclick="setMultiSupervisor(\'_none\',window._multiPairs);closeModal()">— Remove supervisor —</div>';
  (S.supervisors||[]).forEach(function(sup){
    h+='<div style="padding:6px 10px;cursor:pointer;border-bottom:1px solid var(--border2)" onclick="setMultiSupervisor(\''+sup.id+'\',window._multiPairs);closeModal()" data-name="'+sup.name.toLowerCase()+'">&#128119; '+sup.name+'</div>';
  });
  if(!(S.supervisors||[]).length) h+='<div style="padding:6px 10px;color:var(--tx3);font-style:italic">No supervisors — add them in Settings</div>';
  h+='</div></div>';
  
  /* Foreman — searchable list */
  h+='<div style="margin-bottom:14px">';
  h+='<label style="font-size:11px;font-weight:700">&#128736; Foreman</label>';
  h+='<input id="fmSearch" placeholder="Search worker…" oninput="filterMultiList(\'fmList\',this.value)" style="width:100%;margin:4px 0;font-size:11px">';
  h+='<div id="fmList" style="max-height:150px;overflow-y:auto;border:1px solid var(--border2);border-radius:6px">';
  h+='<div style="padding:6px 10px;cursor:pointer;background:#fff8;border-bottom:1px solid var(--border2)" onclick="setMultiForeman(\'_none\',window._multiPairs);closeModal()">— Remove foreman —</div>';
  S.workers.forEach(function(wk){
    var c=specColor(wk.spec||'');
    h+='<div style="padding:6px 10px;cursor:pointer;border-bottom:1px solid var(--border2);display:flex;align-items:center;gap:6px" onclick="setMultiForeman(\''+wk.id+'\',window._multiPairs);closeModal()" data-name="'+wk.name.toLowerCase()+'">';
    h+='<span style="width:8px;height:8px;border-radius:50%;background:'+c+';flex-shrink:0"></span>'+wk.name+'</div>';
  });
  h+='</div></div>';
  ...
  box.innerHTML=h; box.className='modal';
  document.getElementById('mbg').style.display='flex';
  setTimeout(function(){var el=document.getElementById('supSearch');if(el)el.focus();},50);
}

function filterMultiList(listId, query){
  var q=query.toLowerCase().trim();
  var list=document.getElementById(listId);
  if(!list) return;
  var items=list.querySelectorAll('div[data-name]');
  items.forEach(function(item){
    item.style.display=(!q||item.dataset.name.indexOf(q)>=0)?'':'none';
  });
}
...
function renderCCByResponsible(page){
  ...
  S.wbs.forEach(function(w){
    ...
    var depP=0; for(var m=page+1;m<NM;m++) depP+=radAtSlot(w,m);
...
function printCCByResponsible(){
  ...
  Object.keys(respMap).sort().forEach(function(resp){
    ...
    wbsList.forEach(function(w){
      ...
      var ef=0;for(var _m=S.ccM+1;_m<NM;_m++)ef+=radAtSlot(w,_m);
      ...
      for(var m=0;m<NM;m++){
        var bv=budTotal(w,m);
        var av=m<=S.ccM?reel(w,m):radAtSlot(w,m); /* rad = assignments + open positions */
...
function printCC(){
  ...
  S.wbs.forEach(function(w){
    ...
    for(var _m=S.ccM+1;_m<NM;_m++) tEAC_f+=radAtSlot(w,_m);
  });
  ...
  S.groups.forEach(function(g){
    ...
    allW.forEach(function(w){
      ...
      for(var _m=S.ccM+1;_m<NM;_m++)gEF+=radAtSlot(w,_m);
    });
    ...
    g.sub.forEach(function(sg){
      ...
      sgW.forEach(function(w){
        ...
        for(var _m=S.ccM+1;_m<NM;_m++)sEF+=radAtSlot(w,_m);
      });
...
function renderOrgChartPage(){
  ...
  Object.keys(supGroups).sort().forEach(function(supName){
    ...
    cards.forEach(function(card){
      ...
      var _openAssign=getOpenPositions(w.id,m);
      var _openBudget=Math.max(0,card.budHM-n-_openAssign);
      for(var _oi=0;_oi<_openAssign;_oi++){
        h+='<div style="display:flex;align-items:center;gap:6px;padding:3px 0;font-size:11px;border-bottom:1px dashed #bfdbfe"><span style="width:8px;height:8px;border-radius:50%;border:1.5px solid var(--blue);flex-shrink:0"></span><span style="flex:1;color:var(--blue);font-style:italic">&#9711; Open position</span></div>';
      }
      for(var _ob=0;_ob<_openBudget;_ob++){
        h+='<div style="display:flex;align-items:center;gap:6px;padding:3px 0;font-size:11px;border-bottom:1px dashed #e2e8f0"><span style="width:8px;height:8px;border-radius:50%;border:1.5px dashed #cbd5e1;flex-shrink:0"></span><span style="flex:1;color:#94a3b8;font-style:italic">Unallocated slot</span></div>';
      }
      if(!card.workers.length&&!_openAssign&&!_openBudget) h+='<div style="font-size:10px;color:var(--tx3);font-style:italic;padding:4px 0">No workers</div>';
      h+='</div></div>';
    });
    h+='</div></div></div>';
  });
  ...
  el.innerHTML=h;
    ...
    el.innerHTML='<div style="padding:20px;color:#dc2626;background:#fef2f2;border-radius:8px;margin:16px"><strong>Org Chart Error:</strong><br>'+e.message+'<br><small>'+e.stack+'</small></div>';
  }
}
...
/* ── Open Positions ── */
function getOpenPositions(wbsId,m){
  if(!S.openPositions) return 0;
  return parseInt(S.openPositions[wbsId+'_'+m]||0);
}
function setOpenPositions(wbsId,m,n){
  if(!S.openPositions) S.openPositions={};
  if(n>0) S.openPositions[wbsId+'_'+m]=n;
  else delete S.openPositions[wbsId+'_'+m];
}
...
function openPositionDialog(){
  ...
  var pairs=[];
  Object.keys(window._asSel||{}).forEach(function(wid){
    Object.keys(window._asSel[wid]||{}).forEach(function(m){
      if(window._asSel[wid][m]) pairs.push({wbsId:wid,m:parseInt(m)});
    });
  });
  if(!pairs.length){showToast('Select cells first','warn');return;}
  
  var h='<h3>&#9711; Open Positions</h3>';
  h+='<p style="font-size:11px;color:var(--tx2);margin-bottom:12px">'+pairs.length+' cell'+(pairs.length>1?'s':'')+' selected</p>';
  h+='<div style="margin-bottom:14px">';
  h+='<label style="font-size:12px;font-weight:600">Number of open positions to add:</label>';
  h+='<input id="openPosCount" type="number" min="0" max="50" value="1" style="width:100%;margin-top:6px;font-size:14px;text-align:center">';
  h+='</div>';
  
  /* Show current open positions per cell */
  h+='<div style="max-height:120px;overflow-y:auto;margin-bottom:12px">';
  pairs.slice(0,8).forEach(function(p){
    var w=S.wbs.find(function(x){return x.id===p.wbsId;});
    var cur=getOpenPositions(p.wbsId,p.m);
    h+='<div style="font-size:10px;padding:2px 0;color:var(--tx3)">'+(w?w.name:p.wbsId)+' '+(ML[p.m]?ML[p.m].short:'')+' — current: '+cur+'</div>';
  });
  if(pairs.length>8) h+='<div style="font-size:10px;color:var(--tx3)">…and '+(pairs.length-8)+' more</div>';
  h+='</div>';
  
  window._openPosPairs=pairs;
  h+='<div style="display:flex;gap:8px"><button class="btn pri" onclick="confirmOpenPositions(window._openPosPairs)">Set</button>';
  h+='<button class="btn" onclick="clearOpenPositions(window._openPosPairs);closeModal()">Clear all</button>';
  h+='<button class="btn" onclick="closeModal()">Cancel</button></div>';
  
  var box=document.getElementById('mbox');
  box.innerHTML=h; box.className='modal';
  document.getElementById('mbg').style.display='flex';
  setTimeout(function(){var el=document.getElementById('openPosCount');if(el){el.focus();el.select();}},50);
}

function confirmOpenPositions(pairs){
  var n=parseInt(document.getElementById('openPosCount').value)||0;
  pairs.forEach(function(p){ setOpenPositions(p.wbsId,p.m,n); });
  window._asSel={}; /* clear selection */
  closeModal();
  saveS();
  renderAssignmentsPage();
  showToast((n>0?n+' open position'+(n>1?'s':''):' Open positions cleared')+' for '+pairs.length+' cell'+(pairs.length>1?'s':''),'ok');
}

function clearOpenPositions(pairs){
  pairs.forEach(function(p){ setOpenPositions(p.wbsId,p.m,0); });
  saveS();
}
...
function renderWorkforcePage(){
  ...
  for(var m=0;m<NM;m++){
    ...
    filteredWBS.forEach(function(w){
      ...
      mo.eac+=m<=S.ccM?reel(w,m):(radAtSlot(w,m));
    ...
    specs.forEach(function(sp){
      ...
      for(var m=0;m<NM;m++){
        ...
        filteredWBS.filter(function(w){return(w.spec||'other')===sp.spec;}).forEach(function(w){
          budHM+=budTotal(w,m);
          eac+=m<=S.ccM?reel(w,m):(radAtSlot(w,m));
